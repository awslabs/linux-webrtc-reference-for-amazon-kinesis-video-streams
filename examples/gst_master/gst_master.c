...
static int32_t OnMediaSinkHook(void *pCustom, WebrtcFrame_t *pFrame);
static int32_t
InitializeGstMediaSource(AppContext_t *pAppContext,
...
static int32_t InitTransceiver(void *pMediaCtx,
                               ...
                               Transceiver_t *pTranceiver) {
  int32_t ret = 0;
  GstMediaSourcesContext_t *pMediaSourceContext =
      (GstMediaSourcesContext_t *)pMediaCtx;

  if ((pMediaCtx == NULL) || (pTranceiver == NULL)) {
    LogError(("Invalid input, pMediaCtx: %p, pTranceiver: %p", pMediaCtx,
              pTranceiver));
    ret = -1;
  ...
  if (ret == 0) {
    ...
    case TRANSCEIVER_TRACK_KIND_VIDEO:
      ret =
          GstMediaSource_InitVideoTransceiver(pMediaSourceContext, pTranceiver);
      break;
    case TRANSCEIVER_TRACK_KIND_AUDIO:
      ret =
          GstMediaSource_InitAudioTransceiver(pMediaSourceContext, pTranceiver);
      break;
    default:
      LogError(("Invalid track kind: %d", trackKind));
      ret = -3;
  ...
  if (ret == 0) {
    ...
    for (i = 0; i < AWS_MAX_VIEWER_NUM; i++) {
      if (pFrame->trackKind == TRANSCEIVER_TRACK_KIND_VIDEO) {
        pTransceiver = &pAppContext->appSessions[i]
                            .transceivers[DEMO_TRANSCEIVER_MEDIA_INDEX_VIDEO];
      } else if (pFrame->trackKind == TRANSCEIVER_TRACK_KIND_AUDIO) {
        pTransceiver = &pAppContext->appSessions[i]
                            .transceivers[DEMO_TRANSCEIVER_MEDIA_INDEX_AUDIO];
      } else {
      ...
      if (pAppContext->appSessions[i].peerConnectionSession.state ==
        ...
        peerConnectionResult = PeerConnection_WriteFrame(
            &pAppContext->appSessions[i].peerConnectionSession, pTransceiver,
            &peerConnectionFrame);

        if (peerConnectionResult != PEER_CONNECTION_RESULT_OK) {
          LogError(("Fail to write %s frame, result: %d",
                    (pFrame->trackKind == TRANSCEIVER_TRACK_KIND_VIDEO)
                        ? "video"
                        : "audio",
...
InitializeGstMediaSource(AppContext_t *pAppContext,
  ...
  if ((pAppContext == NULL) || (pGstMediaSourceContext == NULL)) {
    LogError(("Invalid input, pAppContext: %p, pGstMediaSourceContext: %p",
              pAppContext, pGstMediaSourceContext));
    ret = -1;
  }

  if (ret == 0) {
    ret = GstMediaSource_Init(pGstMediaSourceContext, OnMediaSinkHook,
                              pAppContext);
  }
