...
static int OnWssMessageReceived(char *pMessage, size_t messageLength,
                                void *pUserData);
...
static uint8_t
AreCredentialsExpired(SignalingControllerContext_t *pCtx,
                      const SignalingControllerConnectInfo_t *pConnectInfo);

static SignalingControllerResult_t
FetchTemporaryCredentials(SignalingControllerContext_t *pCtx,
                          const AwsIotCredentials_t *pAwsIotCredentials);

static SignalingControllerResult_t
DescribeSignalingChannel(SignalingControllerContext_t *pCtx,
                         const SignalingChannelName_t *pChannelName);

static SignalingControllerResult_t
GetSignalingChannelEndpoints(SignalingControllerContext_t *pCtx,
                             uint8_t enableSessionStorage);

static SignalingControllerResult_t
GetIceServerConfigs(SignalingControllerContext_t *pCtx);

static SignalingControllerResult_t
ConnectToWssEndpoint(SignalingControllerContext_t *pCtx);

static SignalingControllerResult_t
ConnectToSignalingService(SignalingControllerContext_t *pCtx,
                          const SignalingControllerConnectInfo_t *pConnectInfo);

static void LogSignalingInfo(SignalingControllerContext_t *pCtx);

static SignalingControllerResult_t
JoinStorageSession(SignalingControllerContext_t *pCtx);
...
static int OnWssMessageReceived(char *pMessage, size_t messageLength,
  ...
  SignalingControllerContext_t *pCtx =
      (SignalingControllerContext_t *)pUserData;
  SignalingMessage_t signalingMessage;
  Base64Result_t base64Result;

  signalingResult =
      Signaling_ParseWssRecvMessage(pMessage, messageLength, &(wssRecvMessage));
  ...
  if (ret == 0) {
    ...
    if (base64Result != BASE64_RESULT_OK) {
      LogError(
          ("Failed to decode signaling message. Result: %d!", base64Result));
      ret = 1;
    }
  }
  ...
  if (ret == 0) {
    ...
    case SIGNALING_TYPE_MESSAGE_STATUS_RESPONSE: {
      if (strcmp(wssRecvMessage.statusResponse.pStatusCode, "200") != 0) {
        LogWarn(("Failed to deliver message. Correlation ID: %.*s, Error Type: "
                 "%.*s, Error Code: %.*s, Description: %.*s!",
  ...
  if ((ret == 0) && (pCtx->messageReceivedCallback != NULL)) {
    ...
    signalingMessage.pCorrelationId =
        wssRecvMessage.statusResponse.pCorrelationId;
    signalingMessage.correlationIdLength =
...
static SignalingControllerResult_t HttpSend(SignalingControllerContext_t *pCtx,
  ...
  for (i = 0; i < SIGNALING_CONTROLLER_HTTP_NUM_RETRIES; i++) {
    networkingResult =
        Networking_HttpSend(&(pCtx->httpContext), pRequest, &(awsCreds),
                            &(pCtx->awsConfig), pResponse);
...
FetchTemporaryCredentials(SignalingControllerContext_t *pCtx,
  ...
  signalingResult = Signaling_ConstructFetchTempCredsRequestForAwsIot(
      pAwsIotCredentials->pIotCredentialsEndpoint,
      pAwsIotCredentials->iotCredentialsEndpointLength,
      pAwsIotCredentials->pRoleAlias, pAwsIotCredentials->roleAliasLength,
      &(signalingRequest));

  if (signalingResult != SIGNALING_RESULT_OK) {
    LogError(("Failed to construct Fetch Temporary Credential request. Return "
              "= 0x%x!",
              signalingResult));
    ret = SIGNALING_CONTROLLER_RESULT_FAIL;
  }
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    ...
    httpResponse.contentMaxCapacity =
        SIGNALING_CONTROLLER_HTTP_RESPONSE_BUFFER_LENGTH;

    if (Networking_HttpSend(&(pCtx->httpContext), &(httpRequest), NULL,
                            &(pCtx->awsConfig),
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    signalingResult = Signaling_ParseFetchTempCredsResponseFromAwsIot(
        httpResponse.pContent, httpResponse.contentLength,
        &(signalingCredentials));

    if (signalingResult != SIGNALING_RESULT_OK) {
      LogError(("Fail to parse fetch credentials response, return=0x%x, "
                "response(%lu): %.*s",
                signalingResult, httpResponse.contentLength,
                (int)httpResponse.contentLength, httpResponse.pContent));
      ret = SIGNALING_CONTROLLER_RESULT_FAIL;
    } else {
      LogDebug(("Access Key ID: %.*s \n\n Secret Access Key ID : %.*s \n\n"
                "Session Token: %.*s \n \n Expiration: %.*s",
                (int)signalingCredentials.accessKeyIdLength,
                signalingCredentials.pAccessKeyId,
                (int)signalingCredentials.secretAccessKeyLength,
                signalingCredentials.pSecretAccessKey,
                (int)signalingCredentials.sessionTokenLength,
                signalingCredentials.pSessionToken,
                (int)signalingCredentials.expirationLength,
                (char *)signalingCredentials.pExpiration));
    }
  }

  if ((ret == SIGNALING_CONTROLLER_RESULT_OK) &&
      (signalingCredentials.pAccessKeyId != NULL)) {
    memcpy(&(pCtx->accessKeyId[0]), signalingCredentials.pAccessKeyId,
  ...
  if ((ret == SIGNALING_CONTROLLER_RESULT_OK) &&
      (signalingCredentials.pSecretAccessKey != NULL)) {
    memcpy(&(pCtx->secretAccessKey[0]), signalingCredentials.pSecretAccessKey,
  ...
  if ((ret == SIGNALING_CONTROLLER_RESULT_OK) &&
      (signalingCredentials.pSessionToken != NULL)) {
    memcpy(&(pCtx->sessionToken[0]), signalingCredentials.pSessionToken,
  ...
  if ((ret == SIGNALING_CONTROLLER_RESULT_OK) &&
      (signalingCredentials.pExpiration != NULL)) {
    pCtx->expirationSeconds = NetworkingUtils_GetTimeFromIso8601(
        signalingCredentials.pExpiration,
...
DescribeSignalingChannel(SignalingControllerContext_t *pCtx,
  ...
  signalingResult = Signaling_ConstructDescribeSignalingChannelRequest(
      &(awsRegion), (SignalingChannelName_t *)pChannelName,
      &(signalingRequest));

  if (signalingResult != SIGNALING_RESULT_OK) {
    LogError(
        ("Fail to construct describe signaling channel request, return=0x%x",
         signalingResult));
    ret = SIGNALING_CONTROLLER_RESULT_FAIL;
  }
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    signalingResult = Signaling_ParseDescribeSignalingChannelResponse(
        httpResponse.pContent, httpResponse.contentLength,
        &(signalingChannelInfo));

    if (signalingResult != SIGNALING_RESULT_OK) {
      LogError(("Fail to parse describe signaling channel response, "
                "return=0x%x, response(%lu): %.*s",
                signalingResult, httpResponse.contentLength,
                (int)httpResponse.contentLength, httpResponse.pContent));
      ret = SIGNALING_CONTROLLER_RESULT_FAIL;
    }
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    if ((signalingChannelInfo.pChannelStatus == NULL) ||
        (strncmp(signalingChannelInfo.pChannelStatus, "ACTIVE",
  ...
  if ((ret == SIGNALING_CONTROLLER_RESULT_OK) &&
      (signalingChannelInfo.channelArn.pChannelArn != NULL)) {
    ...
    pCtx->signalingChannelArn[signalingChannelInfo.channelArn
                                  .channelArnLength] = '\0';
...
GetSignalingChannelEndpoints(SignalingControllerContext_t *pCtx,
  ...
  endpointRequestInfo.channelArn.channelArnLength =
      pCtx->signalingChannelArnLength;
  endpointRequestInfo.protocols =
      SIGNALING_PROTOCOL_WEBSOCKET_SECURE | SIGNALING_PROTOCOL_HTTPS;
  ...
  signalingResult = Signaling_ConstructGetSignalingChannelEndpointRequest(
      &(awsRegion), &(endpointRequestInfo), &(signalingRequest));

  if (signalingResult != SIGNALING_RESULT_OK) {
    LogError(("Failed to construct Get Signaling Channel Endpoint request. "
              "return=0x%x!",
              signalingResult));
    ret = SIGNALING_CONTROLLER_RESULT_FAIL;
  }
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    signalingResult = Signaling_ParseGetSignalingChannelEndpointResponse(
        httpResponse.pContent, httpResponse.contentLength,
        &(signalingEndpoints));

    if (signalingResult != SIGNALING_RESULT_OK) {
      LogError(("Failed to parse Get Signaling Channel Endpoint response. "
                "return=0x%x!",
                signalingResult));
      ret = SIGNALING_CONTROLLER_RESULT_FAIL;
    }
  ...
  if ((ret == SIGNALING_CONTROLLER_RESULT_OK) &&
      (signalingEndpoints.httpsEndpoint.pEndpoint != NULL)) {
    if (signalingEndpoints.httpsEndpoint.endpointLength >
        SIGNALING_CONTROLLER_ENDPOINT_BUFFER_LENGTH) {
      LogError(("HTTPS endpoint (%lu) does not fit in the buffer!",
      ...
      strncpy(&(pCtx->httpsEndpoint[0]),
              ...
              signalingEndpoints.httpsEndpoint.endpointLength);
      pCtx->httpsEndpoint[signalingEndpoints.httpsEndpoint.endpointLength] =
          '\0';
      pCtx->httpsEndpointLength =
          signalingEndpoints.httpsEndpoint.endpointLength;
    }
  }

  if ((ret == SIGNALING_CONTROLLER_RESULT_OK) &&
      (signalingEndpoints.wssEndpoint.pEndpoint != NULL)) {
    if (signalingEndpoints.wssEndpoint.endpointLength >
        SIGNALING_CONTROLLER_ENDPOINT_BUFFER_LENGTH) {
      LogError(("WSS endpoint (%lu) does not fit in the buffer!",
                signalingEndpoints.wssEndpoint.endpointLength));
      ret = SIGNALING_CONTROLLER_RESULT_FAIL;
    } else {
      strncpy(&(pCtx->wssEndpoint[0]), signalingEndpoints.wssEndpoint.pEndpoint,
              signalingEndpoints.wssEndpoint.endpointLength);
  ...
  if ((ret == SIGNALING_CONTROLLER_RESULT_OK) &&
      (signalingEndpoints.webrtcEndpoint.pEndpoint != NULL)) {
    if (signalingEndpoints.webrtcEndpoint.endpointLength >
        SIGNALING_CONTROLLER_ENDPOINT_BUFFER_LENGTH) {
      LogError(("WebRTC endpoint (%lu) does not fit in the buffer!",
      ...
      strncpy(&(pCtx->webrtcEndpoint[0]),
              ...
              signalingEndpoints.webrtcEndpoint.endpointLength);
      pCtx->webrtcEndpoint[signalingEndpoints.webrtcEndpoint.endpointLength] =
          '\0';
      pCtx->webrtcEndpointLength =
          signalingEndpoints.webrtcEndpoint.endpointLength;
    }
  }
...
JoinStorageSession(SignalingControllerContext_t *pCtx) {
  ...
  joinSessionRequestInfo.channelArn.pChannelArn =
      &(pCtx->signalingChannelArn[0]);
  joinSessionRequestInfo.channelArn.channelArnLength =
      pCtx->signalingChannelArnLength;
  ...
  signalingResult = Signaling_ConstructJoinStorageSessionRequest(
      &(webrtcEndpoint), &(joinSessionRequestInfo), &(signalingRequest));

  if (signalingResult != SIGNALING_RESULT_OK) {
    LogError(("Failed to construct join storage session request, return=0x%x",
              signalingResult));
    ret = SIGNALING_CONTROLLER_RESULT_FAIL;
  }
...
GetIceServerConfigs(SignalingControllerContext_t *pCtx) {
  ...
  SignalingIceServer_t
      iceServers[SIGNALING_CONTROLLER_ICE_SERVER_MAX_CONFIG_COUNT];
  ...
  getIceServerConfigRequestInfo.channelArn.pChannelArn =
      &(pCtx->signalingChannelArn[0]);
  getIceServerConfigRequestInfo.channelArn.channelArnLength =
      pCtx->signalingChannelArnLength;
  getIceServerConfigRequestInfo.pClientId = "ProducerMaster";
  getIceServerConfigRequestInfo.clientIdLength = strlen("ProducerMaster");

  signalingResult = Signaling_ConstructGetIceServerConfigRequest(
      &(signalingChannelHttpEndpoint), &(getIceServerConfigRequestInfo),
      &(signalingRequest));

  if (signalingResult != SIGNALING_RESULT_OK) {
    LogError(("Failed to construct Get ICE Server Config request. Return=0x%x!",
              signalingResult));
    ret = SIGNALING_CONTROLLER_RESULT_FAIL;
  }
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    signalingResult = Signaling_ParseGetIceServerConfigResponse(
        httpResponse.pContent, httpResponse.contentLength, &(iceServers[0]),
        &(iceServersCount));

    if (signalingResult != SIGNALING_RESULT_OK) {
      LogError(("Failed to parse Get ICE Server Config response. Return=0x%x!",
                signalingResult));
      ret = SIGNALING_CONTROLLER_RESULT_FAIL;
    }
  }
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    for (i = 0; i < iceServersCount; i++) {
      if (i >= SIGNALING_CONTROLLER_ICE_SERVER_MAX_CONFIG_COUNT) {
        LogWarn(("Cannot store all the ICE server configs! Consider increasing "
                 "SIGNALING_CONTROLLER_ICE_SERVER_MAX_CONFIG_COUNT. "));
        break;
      }

      if (iceServers[i].userNameLength >=
          SIGNALING_CONTROLLER_ICE_SERVER_USER_NAME_BUFFER_LENGTH) {
        LogError(("ICE server username (%lu) does not fit in the buffer!",
                  iceServers[i].userNameLength));
        ret = SIGNALING_CONTROLLER_RESULT_FAIL;
        break;
      }

      if (iceServers[i].passwordLength >=
          SIGNALING_CONTROLLER_ICE_SERVER_PASSWORD_BUFFER_LENGTH) {
        LogError(("ICE server password (%lu) does not fit in the buffer!",
                  iceServers[i].passwordLength));
        ret = SIGNALING_CONTROLLER_RESULT_FAIL;
        break;
      }

      memcpy(&(pCtx->iceServerConfigs[i].userName[0]), iceServers[i].pUserName,
             iceServers[i].userNameLength);
      pCtx->iceServerConfigs[i].userNameLength = iceServers[i].userNameLength;

      memcpy(&(pCtx->iceServerConfigs[i].password[0]), iceServers[i].pPassword,
             iceServers[i].passwordLength);
      ...
      for (j = 0; j < iceServers[i].urisNum; j++) {
        if (j >= SIGNALING_CONTROLLER_ICE_SERVER_MAX_URIS_COUNT) {
          LogWarn(("Cannot store all the ICE server URIs! Consider increasing "
                   "SIGNALING_CONTROLLER_ICE_SERVER_MAX_URIS_COUNT. "));
          break;
        }

        if (iceServers[i].urisLength[j] >=
            SIGNALING_CONTROLLER_ICE_SERVER_URI_BUFFER_LENGTH) {
          LogError(("ICE server URI ( %lu) does not fit in the buffer!",
        ...
        memcpy(&(pCtx->iceServerConfigs[i].iceServerUris[j].uri[0]),
               iceServers[i].pUris[j], iceServers[i].urisLength[j]);
        pCtx->iceServerConfigs[i].iceServerUris[j].uriLength =
            iceServers[i].urisLength[j];
      }
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    ...
    if (minTtl < SIGNALING_CONTROLLER_ICE_CONFIG_REFRESH_GRACE_PERIOD_SEC) {
      ...
      pCtx->iceServerConfigExpirationSec =
          iceServerConfigTimeSec +
...
ConnectToWssEndpoint(SignalingControllerContext_t *pCtx) {
  ...
  wssEndpointRequestInfo.channelArn.channelArnLength =
      pCtx->signalingChannelArnLength;
  wssEndpointRequestInfo.role = SIGNALING_ROLE_MASTER;

  signalingResult = Signaling_ConstructConnectWssEndpointRequest(
      &(wssEndpoint), &(wssEndpointRequestInfo), &(signalingRequest));

  if (signalingResult != SIGNALING_RESULT_OK) {
    LogError(("Failed to construct Connect WSS Endpoint Request. Return=0x%x!",
              signalingResult));
    ret = SIGNALING_CONTROLLER_RESULT_FAIL;
  }
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    ...
    networkingResult = Networking_WebsocketConnect(
        &(pCtx->websocketContext), &(wssConnectInfo), &(awsCreds),
        &(pCtx->awsConfig));
...
static SignalingControllerResult_t ConnectToSignalingService(
    SignalingControllerContext_t *pCtx,
    const SignalingControllerConnectInfo_t *pConnectInfo) {
  SignalingControllerResult_t ret = SIGNALING_CONTROLLER_RESULT_OK;
...
#endif
    ret =
        GetSignalingChannelEndpoints(pCtx, pConnectInfo->enableStorageSession);
#if METRIC_PRINT_ENABLED
    ...
    if ((pCtx->expirationSeconds == 0) ||
        (currentTimeSeconds >=
         pCtx->expirationSeconds -
             SIGNALING_CONTROLLER_FETCH_CREDS_GRACE_PERIOD_SEC)) {
...
static void LogSignalingInfo(SignalingControllerContext_t *pCtx) {
  ...
  LogInfo(("WebRTC Endpoint: %s", pCtx->webrtcEndpointLength == 0
                                      ? "N/A"
                                      : &(pCtx->webrtcEndpoint[0])));
  ...
  for (i = 0; i < pCtx->iceServerConfigsCount; i++) {
    ...
    LogInfo(
        ("    URI Count: %lu", pCtx->iceServerConfigs[i].iceServerUriCount));

    for (j = 0; j < pCtx->iceServerConfigs[i].iceServerUriCount; j++) {
      LogInfo(("        URI: %s",
               &(pCtx->iceServerConfigs[i].iceServerUris[j].uri[0])));
    }
  }
}
...
SignalingController_Init(SignalingControllerContext_t *pCtx,
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    networkingResult = Networking_HttpInit(&(pCtx->httpContext), pSslCreds);
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    networkingResult =
        Networking_WebsocketInit(&(pCtx->websocketContext), pSslCreds);
...
SignalingControllerResult_t SignalingController_StartListening(
    SignalingControllerContext_t *pCtx,
    const SignalingControllerConnectInfo_t *pConnectInfo) {
  SignalingControllerResult_t ret = SIGNALING_CONTROLLER_RESULT_OK;
  NetworkingResult_t networkingResult;
    ...
    for (;;) {
      ...
      if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
        while (networkingResult == NETWORKING_RESULT_OK) {
          networkingResult =
              Networking_WebsocketSignal(&(pCtx->websocketContext));
...
SignalingControllerResult_t
SignalingController_SendMessage(SignalingControllerContext_t *pCtx,
  ...
  if ((pCtx == NULL) || (pSignalingMessage == NULL) ||
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    pthread_mutex_lock(&(pCtx->signalingTxMutex));
    {
      ...
      pCtx->signalingIntermediateMessageLength =
          SIGNALING_CONTROLLER_MESSAGE_BUFFER_LENGTH;
      ...
      if (base64Result != BASE64_RESULT_OK) {
        LogError(("Failed to base64 encode signaling message. Result: %d!",
                  base64Result));
        ret = SIGNALING_CONTROLLER_RESULT_FAIL;
      }
      ...
      if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
        ...
        wssSendMessage.pBase64EncodedMessage =
            &(pCtx->signalingIntermediateMessageBuffer[0]);
        wssSendMessage.base64EncodedMessageLength =
            pCtx->signalingIntermediateMessageLength;
        wssSendMessage.pCorrelationId = pSignalingMessage->pCorrelationId;
        wssSendMessage.correlationIdLength =
            pSignalingMessage->correlationIdLength;
        wssSendMessage.pRecipientClientId = pSignalingMessage->pRemoteClientId;
        wssSendMessage.recipientClientIdLength =
            pSignalingMessage->remoteClientIdLength;

        pCtx->signalingTxMessageLength =
            SIGNALING_CONTROLLER_MESSAGE_BUFFER_LENGTH;
        signalingResult = Signaling_ConstructWssMessage(
            &(wssSendMessage), &(pCtx->signalingTxMessageBuffer[0]),
            &(pCtx->signalingTxMessageLength));

        if (signalingResult != SIGNALING_RESULT_OK) {
          LogError(("Failed to construct signaling Wss message. Result: %d!",
                    signalingResult));
          ret = SIGNALING_CONTROLLER_RESULT_FAIL;
        }
      }
      ...
      if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
        ...
        networkingResult = Networking_WebsocketSend(
            &(pCtx->websocketContext), &(pCtx->signalingTxMessageBuffer[0]),
            pCtx->signalingTxMessageLength);

        if (networkingResult != NETWORKING_RESULT_OK) {
          LogError(("Failed to send signaling Wss message. Result: %d!",
                    networkingResult));
          ret = SIGNALING_CONTROLLER_RESULT_FAIL;
        }
      }
...
SignalingControllerResult_t SignalingController_QueryIceServerConfigs(
    SignalingControllerContext_t *pCtx, IceServerConfig_t **ppIceServerConfigs,
    size_t *pIceServerConfigsCount) {
  SignalingControllerResult_t ret = SIGNALING_CONTROLLER_RESULT_OK;
  uint64_t currentTimeSec;

  if ((pCtx == NULL) || (ppIceServerConfigs == NULL) ||
...
SignalingControllerResult_t SignalingController_RefreshIceServerConfigs(
    SignalingControllerContext_t *pCtx) {
  SignalingControllerResult_t ret = SIGNALING_CONTROLLER_RESULT_OK;
...
SignalingController_ExtractSdpOfferFromSignalingMessage(
    const char *pSignalingMessage, size_t signalingMessageLength,
    const char **ppSdpMessage, size_t *pSdpMessageLength) {
  SignalingControllerResult_t ret = SIGNALING_CONTROLLER_RESULT_OK;
  ...
  if ((pSignalingMessage == NULL) || (signalingMessageLength == 0) ||
      (ppSdpMessage == NULL) || (pSdpMessageLength == NULL)) {
    ret = SIGNALING_CONTROLLER_RESULT_BAD_PARAM;
  }
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    ...
    if (jsonResult != JSONSuccess) {
      LogError(("Signaling message is not valid JSON. Result: %d, "
                "message(%lu): %.*s!",
                jsonResult, signalingMessageLength, (int)signalingMessageLength,
                pSignalingMessage));
      ret = SIGNALING_CONTROLLER_RESULT_FAIL;
    }
  }
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    jsonResult = JSON_Iterate(pSignalingMessage, signalingMessageLength,
                              &(start), &(next), &(pair));
  ...
  if ((ret == SIGNALING_CONTROLLER_RESULT_OK) && (sdpOfferFound == 0)) {
    LogError(("SDP offer not found in signaling message(%lu): %.*s",
              signalingMessageLength, (int)signalingMessageLength,
              pSignalingMessage));
...
SignalingControllerResult_t SignalingController_DeserializeSdpContentNewline(
    const char *pSdpMessage, size_t sdpMessageLength, char *pFormalSdpMessage,
  ...
  if ((pSdpMessage == NULL) || (sdpMessageLength == 0) ||
      (pFormalSdpMessage == NULL) || (pFormalSdpMessageLength == NULL)) {
    ret = SIGNALING_CONTROLLER_RESULT_BAD_PARAM;
  }
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    ...
    while ((pNext = strstr(pCurSdp, "\\n")) != NULL) {
      ...
      if ((lineLength >= 2) && (pCurSdp[lineLength - 2] == '\\') &&
          (pCurSdp[lineLength - 1] == 'r')) {
        lineLength -= 2;
      }

      if (*pFormalSdpMessageLength < outputLength + lineLength + 2) {
        LogError(("Buffer space is not enough to store formal SDP message, "
                  "buffer size: %lu, SDP message(%lu): %.*s",
                  *pFormalSdpMessageLength, sdpMessageLength,
                  (int)sdpMessageLength, pSdpMessage));
...
SignalingControllerResult_t SignalingController_SerializeSdpContentNewline(
    const char *pSdpMessage, size_t sdpMessageLength, char *pEventSdpMessage,
  ...
  if ((pSdpMessage == NULL) || (sdpMessageLength == 0) ||
      (pEventSdpMessage == NULL) || (pEventSdpMessageLength == NULL)) {
    ret = SIGNALING_CONTROLLER_RESULT_BAD_PARAM;
  }
  ...
  if (ret == SIGNALING_CONTROLLER_RESULT_OK) {
    ...
    while ((pNext = memchr(pCurSdp, '\n', pTail - pCurSdp)) != NULL) {
      ...
      if ((lineLength > 0) && (pCurSdp[lineLength - 1] == '\r')) {
        lineLength--;
      }

      if (*pEventSdpMessageLength < outputLength + lineLength + 4) {
        LogError(("The output buffer length(%lu) is too small to store "
                  "serialized %lu bytes message.",
                  *pEventSdpMessageLength, sdpMessageLength));
      ...
      writtenLength =
          snprintf(pCurOutput, *pEventSdpMessageLength - outputLength,
                   "%.*s\\r\\n", (int)lineLength, pCurSdp);
