...
#define PEER_CONNECTION_JITTER_BUFFER_INCREASE_WITH_WRAP(x, y, max)            \
  (PEER_CONNECTION_JITTER_BUFFER_WRAP((x) + (y), max))
#define PEER_CONNECTION_JITTER_BUFFER_DECREASE_WITH_WRAP(x, y, max)            \
  (PEER_CONNECTION_JITTER_BUFFER_WRAP((x) - (y), max))
...
static void DiscardPackets(PeerConnectionJitterBuffer_t *pJitterBuffer,
                           uint16_t startSeq, uint16_t endSeq,
  ...
  if (pJitterBuffer != NULL) {
    ...
    for (i = startSeq; i != (uint16_t)(endSeq + 1); i++) {
      index = PEER_CONNECTION_JITTER_BUFFER_WRAP(
          i, PEER_CONNECTION_JITTER_BUFFER_MAX_ENTRY_NUM);
...
ParseFramesInJitterBuffer(PeerConnectionJitterBuffer_t *pJitterBuffer,
                          int32_t isClosing) {
  ...
  int32_t firstTimestampIndex = -1, popingPacketStartIndex = -1,
  ...
  if (pJitterBuffer == NULL) {
    ...
    LogError(("Invalid input, onFrameReadyCallbackFunc: %p, "
              "onFrameDropCallbackFunc: %p",
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    earliestBufferTimestamp = pJitterBuffer->newestReceivedTimestamp -
                              pJitterBuffer->tolerenceRtpTimeStamp;
    /* Note that newest sequence is probably less than oldest sequence due to
     * wrapping. */
    prev = pJitterBuffer->oldestReceivedSequenceNumber;
    for (i = pJitterBuffer->oldestReceivedSequenceNumber;
         i != (uint16_t)(pJitterBuffer->newestReceivedSequenceNumber + 1);
         i++) {
      index = PEER_CONNECTION_JITTER_BUFFER_WRAP(
          i, PEER_CONNECTION_JITTER_BUFFER_MAX_ENTRY_NUM);
      pPacket = &pJitterBuffer->rtpPackets[index];
      if (pPacket->isPushed == 0U) {
        ...
        /* The packet is earilier than tolerence timestamp, pop it or discard
         * it. */
        ...
        if (poppingTimestamp != currentTimestamp) {
          ...
          if (firstTimestampIndex != -1) {
            ...
            if ((popingPacketStartIndex != -1) &&
              ...
              /* We now have an full frame ready between start index and end
               * index. */
              ret = pJitterBuffer->onFrameReadyCallbackFunc(
                  pJitterBuffer->pOnFrameReadyCallbackContext,
                  popingPacketStartIndex, popingPacketEndIndex);
              if (ret != PEER_CONNECTION_RESULT_OK) {
                LogError(("Terminating parsing jitter buffer by frame ready "
                          "callback function, result: %d",
                          ret));
                break;
              }
            }

            DiscardPackets(pJitterBuffer, firstTimestampIndex, prev,
                           currentTimestamp);
          }
        ...
        if (pJitterBuffer->getPacketPropertyFunc &&
            (pJitterBuffer->getPacketPropertyFunc(pPacket, &isStart) ==
             PEER_CONNECTION_RESULT_OK)) {
          if ((popingPacketStartIndex == -1) && isStart) {
            popingPacketStartIndex = i;
          }
        } else {
          /* No get properties callback function or it returns failure. This
           * packet is invalid, drop it. */
          isFrameDataContinuous = 0;
          LogInfo(
              ("Fail to get property, dumping RTP payload, 0x%x 0x%x 0x%x 0x%x",
               pPacket->pPacketBuffer[0], pPacket->pPacketBuffer[1],
               pPacket->pPacketBuffer[2], pPacket->pPacketBuffer[3]));
          DiscardPackets(pJitterBuffer, (uint16_t)firstTimestampIndex, i,
                         currentTimestamp);
        ...
        /* The following packets are still in tolerence timestamp, not necessary
         * to parse them for now. */
        break;
      }
...
static PeerConnectionResult_t
ShouldAcceptPacket(PeerConnectionJitterBuffer_t *pJitterBuffer,
                   PeerConnectionJitterBufferPacket_t *pPacket) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  ...
  if ((pJitterBuffer == NULL) || (pPacket == NULL)) {
    LogError(("Invalid input, pJitterBuffer: %p, pPacket: %p", pJitterBuffer,
              pPacket));
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  }
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    earliestTolerenceRtpSeq =
        (uint16_t)((pJitterBuffer->newestReceivedSequenceNumber -
                    rtpSeqOffset) &
                   0xFFFF);
    ...
    if (lastestTolerenceRtpSeq >= earliestTolerenceRtpSeq) {
      /* The most common scenario, the tolerence range is between earliest and
       * lastest, otherwise return false. */
      if ((pPacket->sequenceNumber < earliestTolerenceRtpSeq) ||
        ...
        LogInfo(("Dropping packet with seq: %u because of out of tolerence seq "
                 "range, earliest: %u, lastest: %u",
                 pPacket->sequenceNumber, earliestTolerenceRtpSeq,
                 lastestTolerenceRtpSeq));
      }
    } else {
      ...
      if ((pPacket->sequenceNumber < earliestTolerenceRtpSeq) &&
        ...
        ret = PEER_CONNECTION_RESULT_PACKET_OUTDATED;
        LogInfo(("Dropping packet with seq: %u because of out of tolerence seq "
                 "range, earliest seq: %u, lastest seq: %u",
                 pPacket->sequenceNumber, earliestTolerenceRtpSeq,
                 lastestTolerenceRtpSeq));
      }
    }
...
static PeerConnectionResult_t
UpdateJitterBufferAddPacket(PeerConnectionJitterBuffer_t *pJitterBuffer,
                            PeerConnectionJitterBufferPacket_t *pPacket) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    } else if (((pJitterBuffer->newestReceivedSequenceNumber +
                 PEER_CONNECTION_JITTER_BUFFER_SEQ_WRAPPER_THRESHOLD) &
                0xFFFF) <= pPacket->sequenceNumber) {
      /* Overflow is happening, we update the newest sequence number only if
       * it's in the range of threshold. */
      pJitterBuffer->newestReceivedSequenceNumber = pPacket->sequenceNumber;
    } else {
    ...
    } else if (((pJitterBuffer->oldestReceivedSequenceNumber -
                 PEER_CONNECTION_JITTER_BUFFER_SEQ_WRAPPER_THRESHOLD) &
                0xFFFF) <= pPacket->sequenceNumber) {
      /* Overflow is happening, we update the newest sequence number only if
       * it's in the range of threshold. */
    ...
    if (pPacket->rtpTimestamp > pJitterBuffer->newestReceivedTimestamp) {
      ...
      pJitterBuffer->newestReceivedTimestamp = pPacket->rtpTimestamp;
        ...
        ((uint64_t)(pJitterBuffer->newestReceivedTimestamp +
                    (PEER_CONNECTION_JITTER_BUFFER_TIMESTAMP_WRAPPER_THRESHOLD_SEC *
                     pJitterBuffer->clockRate)) &
         0xFFFFFFFF) <= pPacket->rtpTimestamp) {
      /* Overflow is happening, we update the newest timestamp only if it's in
...
static void DiscardPacket(PeerConnectionJitterBuffer_t *pJitterBuffer,
  ...
  if (pPacket && (pPacket->pPacketBuffer != NULL)) {
    free(pPacket->pPacketBuffer);
    memset(pPacket, 0, sizeof(PeerConnectionJitterBufferPacket_t));
  }
}

PeerConnectionResult_t PeerConnectionJitterBuffer_Create(
    PeerConnectionJitterBuffer_t *pJitterBuffer,
    ...
    uint32_t codec, uint32_t clockRate) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;

  if ((pJitterBuffer == NULL) || (codec == 0)) {
    LogError(
        ("Invalid input, pJitterBuffer: %p, codec: %u", pJitterBuffer, codec));
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  }
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    pJitterBuffer->lastPopTick = 0xFFFFFFFFFFFFFFFF;
    ...
    LogInfo(("Creating jitter buffer with tolerence RTP timestamp: %u",
             pJitterBuffer->tolerenceRtpTimeStamp));
  }
...
void PeerConnectionJitterBuffer_Free(
    PeerConnectionJitterBuffer_t *pJitterBuffer) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    DiscardPackets(pJitterBuffer, 0,
                   PEER_CONNECTION_JITTER_BUFFER_MAX_ENTRY_NUM - 1, 0U);
  }
}

PeerConnectionResult_t PeerConnectionJitterBuffer_AllocateBuffer(
    PeerConnectionJitterBuffer_t *pJitterBuffer,
    PeerConnectionJitterBufferPacket_t **ppOutPacket, size_t packetBufferSize,
    uint16_t rtpSeq) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  int index = PEER_CONNECTION_JITTER_BUFFER_WRAP(
      rtpSeq, PEER_CONNECTION_JITTER_BUFFER_MAX_ENTRY_NUM);

  if ((pJitterBuffer == NULL) || (ppOutPacket == NULL)) {
    LogError(("Invalid input, pJitterBuffer: %p, ppOutPacket: %p",
              pJitterBuffer, ppOutPacket));
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  } else if (packetBufferSize == 0) {
    LogError(
        ("Invalid input, the input buffer length should be set correctly"));
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  } else {
    /* Empty else marker. */
  }
...
PeerConnectionResult_t PeerConnectionJitterBuffer_GetPacket(
    PeerConnectionJitterBuffer_t *pJitterBuffer, uint16_t rtpSeq,
...
PeerConnectionJitterBuffer_Push(PeerConnectionJitterBuffer_t *pJitterBuffer,
                                PeerConnectionJitterBufferPacket_t *pPacket) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    /* Parse the jitter buffer to check if any frames are ready for decoding or
     * if any packets need to be dropped. */
    ret = ParseFramesInJitterBuffer(pJitterBuffer, 0);
  }

  return ret;
}

PeerConnectionResult_t PeerConnectionJitterBuffer_FillFrame(
  ...
  if ((pJitterBuffer == NULL) || (pOutBuffer == NULL) ||
      (pOutBufferLength == NULL) || (pRtpTimestamp == NULL)) {
    LogError(("Invalid input, pJitterBuffer: %p, pOutBuffer: %p, "
              "pOutBufferLength: %p, pRtpTimestamp: %p",
              pJitterBuffer, pOutBuffer, pOutBufferLength, pRtpTimestamp));
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  }

  if (ret == PEER_CONNECTION_RESULT_OK) {
    if (pJitterBuffer->fillFrameFunc) {
      ret = pJitterBuffer->fillFrameFunc(pJitterBuffer, rtpSeqStart, rtpSeqEnd,
                                         pOutBuffer, pOutBufferLength,
                                         pRtpTimestamp);
    } else {
      LogWarn(
          ("No fill frame function pointer for this jitter buffer, codec: 0x%x",
           pJitterBuffer->codec));
    }
  }
