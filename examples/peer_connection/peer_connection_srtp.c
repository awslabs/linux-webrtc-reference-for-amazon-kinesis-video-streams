...
/* At write frame, we reserve 2 bytes at the beginning of payload buffer for
 * re-transmission if RTX is enabled. */
...
static PeerConnectionResult_t OnJitterBufferFrameReady(void *pCustomContext,
  ...
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK,
                         retFillFrame = PEER_CONNECTION_RESULT_OK;
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    /* Return fail only when hitting critical issues. If fill fram API returns
     * fail, we still return OK to the jitter buffer to release these packet
     * normally. */
    retFillFrame = PeerConnectionJitterBuffer_FillFrame(
        &pSrtpReceiver->rxJitterBuffer, startSequence, endSequence,
        pSrtpReceiver->frameBuffer, &frameBufferLength, &rtpTimestamp);
    LogDebug(
        ("Fill frame with result: %d, length: %lu, start seq: %u, end seq: %u",
         retFillFrame, frameBufferLength, startSequence, endSequence));
  }
  ...
  if (retFillFrame == PEER_CONNECTION_RESULT_OK) {
    if (pSrtpReceiver->onFrameReadyCallbackFunc) {
      ...
      frame.presentationUs =
          PEER_CONNECTION_SRTP_CONVERT_RTP_TIMESTAMP_TO_TIME_US(
              pSrtpReceiver->rxJitterBuffer.clockRate, rtpTimestamp);
      frame.pData = pSrtpReceiver->frameBuffer;
      frame.dataLength = frameBufferLength;
      pSrtpReceiver->onFrameReadyCallbackFunc(
          pSrtpReceiver->pOnFrameReadyCallbackCustomContext, &frame);
    }
  }
...
PeerConnectionResult_t PeerConnectionSrtp_ConstructSrtpPacket(
    PeerConnectionSession_t *pSession, RtpPacket_t *pPacketRtp,
    uint8_t *pOutputSrtpPacket, size_t *pOutputSrtpPacketLength) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  ...
  if ((pSession == NULL) || (pPacketRtp == NULL) ||
      (pOutputSrtpPacket == NULL) || (pOutputSrtpPacketLength == NULL)) {
    LogError(("Invalid input, pSession: %p, pPacketRtp: %p, pOutputSrtpPacket: "
              "%p, pOutputSrtpPacketLength: %p",
              pSession, pPacketRtp, pOutputSrtpPacket,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    resultRtp = Rtp_Serialize(&pSession->pCtx->rtpContext, pPacketRtp,
                              pOutputSrtpPacket, &rtpBufferLength);
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    if (pSession->srtpTransmitSession != NULL) {
      errorStatus = srtp_protect(pSession->srtpTransmitSession,
                                 pOutputSrtpPacket, rtpBufferLength,
                                 pOutputSrtpPacket, pOutputSrtpPacketLength, 0);
      if (errorStatus != srtp_err_status_ok) {
        LogError(
            ("Fail to encrypt Tx SRTP packet, errorStatus: %d", errorStatus));
...
PeerConnectionSrtp_Init(PeerConnectionSession_t *pSession) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    switch (pSession->dtlsSession.xNetworkCredentials.dtlsKeyingMaterial
                .srtpProfile) {
    ...
    default:
      LogError(
          ("Unknown SRTP profile: %d", pSession->dtlsSession.xNetworkCredentials
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    if (pthread_mutex_lock(&(pSession->srtpSessionMutex)) == 0) {
      ...
      LogError(
          ("Fail to take SRTP session mutex to create SRTP session instance."));
      ret = PEER_CONNECTION_RESULT_FAIL_TAKE_SRTP_MUTEX;
    }
  }
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    receivePolicy.key = pSession->dtlsSession.xNetworkCredentials
                            .dtlsKeyingMaterial.serverWriteKey;
    ...
    if (errorStatus != srtp_err_status_ok) {
      LogError(
          ("Fail to create Rx SRTP session, errorStatus: %d", errorStatus));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    transmitPolicy.key = pSession->dtlsSession.xNetworkCredentials
                             .dtlsKeyingMaterial.clientWriteKey;
    transmitPolicy.ssrc.type = ssrc_any_outbound;
    transmitPolicy.next = NULL;

    errorStatus =
        srtp_create(&(pSession->srtpTransmitSession), &transmitPolicy);
    if (errorStatus != srtp_err_status_ok) {
      LogError(
          ("Fail to create Tx SRTP session, errorStatus: %d", errorStatus));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    for (i = 0; i < pSession->transceiverCount; i++) {
      if ((pSession->pTransceivers[i]->trackKind ==
        ...
        if ((pSession->rtpConfig.videoCodecRtxPayload != 0) &&
          ...
          /* If we're using different payload type in re-transmission, we create
           * the rolling buffer just for RTP payload. */
          maxSizePerPacket = PEER_CONNECTION_SRTP_RTP_PAYLOAD_MAX_LENGTH;
        }
        ret = PeerConnectionRollingBuffer_Create(
            &pSession->videoSrtpSender.txRollingBuffer,
            pSession->pTransceivers[i]->rollingbufferBitRate, // bps
            pSession->pTransceivers[i]
                ->rollingbufferDurationSec, // duration in seconds
            maxSizePerPacket);
        ...
        ret = PeerConnectionRollingBuffer_Create(
            &pSession->audioSrtpSender.txRollingBuffer,
            pSession->pTransceivers[i]->rollingbufferBitRate, // bps
            pSession->pTransceivers[i]
                ->rollingbufferDurationSec, // duration in seconds
            maxSizePerPacket);
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    for (i = 0; i < pSession->transceiverCount; i++) {
      if ((pSession->pTransceivers[i]->trackKind ==
        ...
        ret = PeerConnectionJitterBuffer_Create(
            &pSrtpReceiver->rxJitterBuffer, OnJitterBufferFrameReady,
            pSrtpReceiver, OnJitterBufferFrameDrop, pSrtpReceiver,
            PEER_CONNECTION_SRTP_JITTER_BUFFER_TOLERENCE_TIME_SECOND, // buffer
                                                                      // time in
                                                                      // seconds
...
PeerConnectionResult_t
PeerConnectionSrtp_DeInit(PeerConnectionSession_t *pSession) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  srtp_err_status_t errorStatus;
  uint8_t isLocked = 0U;
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    if (pSession->srtpReceiveSession != NULL) {
      ...
      if (errorStatus != srtp_err_status_ok) {
        LogError(("Fail to deallocate Rx SRTP session, errorStatus: %d",
                  errorStatus));
      }
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    if (pSession->srtpTransmitSession != NULL) {
      ...
      if (errorStatus != srtp_err_status_ok) {
        LogError(("Fail to deallocate Tx SRTP session, errorStatus: %d",
                  errorStatus));
      }
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    if (pthread_mutex_lock(&(pSession->videoSrtpSender.senderMutex)) == 0) {
      PeerConnectionRollingBuffer_Free(
          &pSession->videoSrtpSender.txRollingBuffer);
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    memset(pSession->videoSrtpReceiver.frameBuffer, 0,
           PEER_CONNECTION_FRAME_BUFFER_SIZE);
    PeerConnectionJitterBuffer_Free(
        &pSession->videoSrtpReceiver.rxJitterBuffer);

    /* Clean up Audio SRTP Receiver */
    memset(pSession->audioSrtpReceiver.frameBuffer, 0,
           PEER_CONNECTION_FRAME_BUFFER_SIZE);
    PeerConnectionJitterBuffer_Free(
        &pSession->audioSrtpReceiver.rxJitterBuffer);
  }
...
PeerConnectionResult_t
PeerConnectionSrtp_HandleSrtpPacket(PeerConnectionSession_t *pSession,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    if (pSession->srtpReceiveSession != NULL) {
      errorStatus = srtp_unprotect(pSession->srtpReceiveSession, pBuffer,
                                   bufferLength, rtpBuffer, &rtpBufferLength);
      if (errorStatus != srtp_err_status_ok) {
        LogError(
            ("Fail to decrypt Rx SRTP packet, errorStatus: %d", errorStatus));
        ret = PEER_CONNECTION_RESULT_FAIL_DECRYPT_SRTP_RTP_PACKET;
      } else {
        LogVerbose(("Decrypt SRTP packet successfully, decrypted length: %lu",
                    rtpBufferLength));
      }
    } else {
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    resultRtp = Rtp_DeSerialize(&pSession->pCtx->rtpContext, rtpBuffer,
                                rtpBufferLength, &rtpPacket);
    if (resultRtp != RTP_RESULT_OK) {
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ret = PeerConnectionJitterBuffer_AllocateBuffer(
        &pSrtpReceiver->rxJitterBuffer, &pJitterBufferPacket,
        rtpPacket.payloadLength, rtpPacket.header.sequenceNumber);
  }

  if (ret == PEER_CONNECTION_RESULT_OK) {
    memcpy(pJitterBufferPacket->pPacketBuffer, rtpPacket.pPayload,
    ...
    // LogInfo( ( "Dumping RTP payload: %u, seq: %u, timestamp: %lu",
    // rtpPacket.payloadLength, rtpPacket.header.sequenceNumber,
    // rtpPacket.header.timestamp ) ); for( int i = 0; i <
    // rtpPacket.payloadLength; i++ )
    // {
