...
static void OnSCTPSessionOutboundPacket(void *customData, uint8_t *pPacket,
                                        uint32_t packetLen);
static void OnSCTPSessionDataChannelOpen(void *customData, uint16_t channelId,
                                         const uint8_t *pName,
                                         uint16_t nameLen);
...
PeerConnectionResult_t PeerConnectionSCTP_DeallocateDataChannel(
...
PeerConnectionResult_t PeerConnectionSCTP_CreateDataChannel(
    PeerConnectionSession_t *pSession, char *pcDataChannelName,
  ...
  if ((pSession == NULL) || (ppChannel == NULL) ||
    ...
    if (pChannel == NULL) {
      ...
      ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
    } else if (pSession->uKvsDataChannelCount <
               PEER_CONNECTION_MAX_SCTP_DATA_CHANNELS_PER_PEER) {
      /* Set channel name */
      strncpy(pChannel->ucDataChannelName, pcDataChannelName,
              MAX_DATA_CHANNEL_NAME_LEN);

      /* Set channel settings */
      if (pDataChannelInitInfo != NULL) {
        memcpy(&(pChannel->dataChannelInitInfo), pDataChannelInitInfo,
               sizeof(SctpDataChannelInitInfo_t));
      } else {
        memset(&(pChannel->dataChannelInitInfo), 0,
               sizeof(SctpDataChannelInitInfo_t));
        /* Use default. */
      ...
      *ppChannel = pChannel;
      ...
      LogError(("Sessions has more than "
                "PEER_CONNECTION_MAX_SCTP_DATA_CHANNELS_PER_PEER data channels "
                "opened."));
      ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
    }
  }
...
PeerConnectionSCTP_DataChannelSend(PeerConnectionDataChannel_t *pChannel,
                                   uint8_t isBinary, uint8_t *pMessage,
                                   uint32_t pMessageLen) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  ...
  if ((pMessage == NULL) || (pChannel == NULL)) {
    LogError(("No message or pDataChannel received in "
    ...
    if (Sctp_SendMessage(pSctpSession, &(pChannel->dataChannel), isBinary,
                         pMessage, pMessageLen) != SCTP_UTILS_RESULT_OK) {
...
static void OnDataChannelMessage(PeerConnectionDataChannel_t *pDataChannel,
  ...
  if (retStatus != PEER_CONNECTION_RESULT_OK) {
    LogInfo(("[KVS Master] dataChannelSend(): operation returned status code: "
             "0x%08x \n",
...
PeerConnectionResult_t
PeerConnectionSCTP_AllocateSCTP(PeerConnectionSession_t *pSession) {

  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;

  /* Create the SCTP Session */
  pSession->sctpSession.sctpSessionCallbacks.outboundPacketCallback =
      OnSCTPSessionOutboundPacket;
  pSession->sctpSession.sctpSessionCallbacks.dataChannelMessageCallback =
      OnSCTPSessionDataChannelMessage;
  pSession->sctpSession.sctpSessionCallbacks.dataChannelOpenCallback =
      OnSCTPSessionDataChannelOpen;
  pSession->sctpSession.sctpSessionCallbacks.dataChannelOpenAckCallback =
      OnSCTPSessionDataChannelAckOpen;
  ...
  if (Sctp_CreateSession(&(pSession->sctpSession), 0) == SCTP_UTILS_RESULT_OK) {
    ...
    while (pxIterator != NULL) {
      pxIterator->dataChannelInitInfo.pChannelName =
          &(pxIterator->ucDataChannelName[0]);
      pxIterator->dataChannelInitInfo.channelNameLen =
          strlen(pxIterator->ucDataChannelName);

      if (Sctp_OpenDataChannel(
              &(pSession->sctpSession), &(pxIterator->dataChannelInitInfo),
        ...
        {
          pxIterator->onDataChannelMessage =
              PeerConnectionSCTP_SetChannelOnMessageCallbackHook(
                  pSession, pxIterator->dataChannel.channelId,
                  (uint8_t *)pxIterator->ucDataChannelName,
...
void PeerConnectionSCTP_ProcessSCTPData(PeerConnectionSession_t *pSession,
                                        uint8_t *receiveBuffer, int readBytes) {

  if (Sctp_ProcessMessage(&(pSession->sctpSession), receiveBuffer, readBytes) !=
...
pxGetDataChannelWithID(PeerConnectionSession_t *pSession, uint32_t channelId) {
...
static void OnSCTPSessionOutboundPacket(void *customData, uint8_t *pPacket,
  ...
  if (DTLS_Send(&(pPeerConnectionSession->dtlsSession.xDtlsNetworkContext),
                pPacket, packetLen) < 0) {
...
static void OnSCTPSessionDataChannelMessage(void *customData,
                                            ...
                                            uint8_t isBinary, uint8_t *pMessage,
                                            uint32_t pMessageLen) {
  PeerConnectionSession_t *pPeerConnectionSession =
      (PeerConnectionSession_t *)customData;
  PeerConnectionDataChannel_t *pChannel = NULL;
...
static void OnSCTPSessionDataChannelOpen(void *customData, uint16_t channelId,
  ...
  if ((pPeerConnectionSession == NULL) ||
      ((uKVSDataChannelCount >= MAX_SCTP_DATA_CHANNELS))) {
    LogError(("No context found or not enough data channel remaining"));
    return;
  }

  if ((pPeerConnectionSession->uKvsDataChannelCount <
       PEER_CONNECTION_MAX_SCTP_DATA_CHANNELS_PER_PEER) &&
...
#if DATACHANNEL_CUSTOM_CALLBACK_HOOK
    {
      pChannel->onDataChannelMessage =
          PeerConnectionSCTP_SetChannelOnMessageCallbackHook(
              pPeerConnectionSession, channelId, pName, nameLen);
    }
#else
    {
    ...
    LogError(("All %d data channel handles are open, no free handles available",
              MAX_SCTP_DATA_CHANNELS));
  }
}
/*-----------------------------------------------------------*/

PeerConnectionResult_t
PeerConnectionSCTP_CloseDataChannel(PeerConnectionDataChannel_t *pChannel) {
  PeerConnectionSession_t *pPeerConnectionSession = pChannel->pPeerConnection;
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;

  if (Sctp_CloseDataChannel(&(pPeerConnectionSession->sctpSession),
                            &(pChannel->dataChannel)) != SCTP_UTILS_RESULT_OK) {
    ret = PEER_CONNECTION_RESULT_FAIL_SCTP_CLOSE;
  } else {

    PeerConnectionDataChannel_t *pxIterator =
        pPeerConnectionSession->pDataChannels;
...
PeerConnectionResult_t
PeerConnectionSCTP_DeallocateSCTP(PeerConnectionSession_t *pSession) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  PeerConnectionDataChannel_t *pxIterator = pSession->pDataChannels;
