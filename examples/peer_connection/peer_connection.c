...
static void OnCloseRequest( PeerConnectionSession_t * pSession )
{
    if( pSession == NULL )
    {
        ...
        switch( requestMsg.requestType )
        {
            ...
            case PEER_CONNECTION_SESSION_REQUEST_TYPE_RTCP_SENDER_REPORT:
                ( void ) PeerConnection_OnRtcpSenderReportCallback( pSession,
                                                                    &requestMsg );
                break;
            case PEER_CONNECTION_SESSION_REQUEST_TYPE_CLOSE:
                OnCloseRequest( pSession );
...
PeerConnectionResult_t PeerConnection_CloseSession( PeerConnectionSession_t * pSession )
{
    ...
    if( ret == PEER_CONNECTION_RESULT_OK )
    {
        ret = SendPeerConnectionEvent( pSession, PEER_CONNECTION_SESSION_REQUEST_TYPE_CLOSE, NULL );
    }
                                                                         ...
                                                                         PeerConnectionSessionRequestMessage_t * pRequestMessage )
{
    ...
    else
    {
        ...
        if( ret == PEER_CONNECTION_RESULT_OK )
        {
            ...
            {
                LogInfo( ( "Send RTCP Sender Report with Status : %u  to SSRC :  %u, NTP Time :  %lu, RTP Time:  %u,  PacketCount : %u, OctetCount : %u",ret,
