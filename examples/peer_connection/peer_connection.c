...
#define PEER_CONNECTION_SESSION_RX_TASK_NAME                                   \
  "PcRxTsk" // For Ice controller to monitor socket Rx path
...
static PeerConnectionResult_t
SendPeerConnectionEvent(PeerConnectionSession_t *pSession,
                        PeerConnectionSessionRequestType_t requestType,
                        void *pRequestContent, size_t contentLength);
static PeerConnectionResult_t
HandleRequest(PeerConnectionSession_t *pSession,
              MessageQueueHandler_t *pRequestQueue);
static PeerConnectionResult_t HandleAddRemoteCandidateRequest(
    PeerConnectionSession_t *pSession,
    PeerConnectionSessionRequestMessage_t *pRequestMessage);
static PeerConnectionResult_t HandleProcessIceCandidatesAndPairs(
    PeerConnectionSession_t *pSession,
    PeerConnectionSessionRequestMessage_t *pRequestMessage);
static PeerConnectionResult_t HandlePeriodConnectionCheck(
    PeerConnectionSession_t *pSession,
    PeerConnectionSessionRequestMessage_t *pRequestMessage);
static PeerConnectionResult_t
HandleIceClosing(PeerConnectionSession_t *pSession,
                 PeerConnectionSessionRequestMessage_t *pRequestMessage);

static PeerConnectionResult_t PeerConnection_OnRtcpSenderReportCallback(
    PeerConnectionSession_t *pSession,
    PeerConnectionSessionRequestMessage_t *pRequestMessage);
...
static void *PeerConnection_SessionTask(void *pParameter) {
  ...
  for (;;) {
    ...
    retRead =
        read(pSession->startupBarrier, &barrierResult, sizeof(barrierResult));
    if (retRead != sizeof(uint64_t)) {
      LogError(("Unexpected return value from start up barrier, retRead: %ld",
                retRead));
      continue;
    }
...
static void SessionProcessEndlessLoop(PeerConnectionSession_t *pSession) {
  ...
  for (; !skipProcess;) {
    result = HandleRequest(pSession, &pSession->requestQueue);
    if ((result != PEER_CONNECTION_RESULT_OK) &&
        (result != PEER_CONNECTION_RESULT_CLOSING)) {
      LogError(
          ("Unexpected result while handling request, result: %d", result));
      break;
    ...
    if ((pSession->state == PEER_CONNECTION_SESSION_STATE_CONNECTION_READY) &&
        ((NetworkingUtils_GetCurrentTimeUs(NULL) / 1000) >
         pSession->inactiveConnectionTimeoutMs)) {
      LogInfo(
          ("Detect inactive connection, closing peer connection session: %s.",
  ...
  if (pMessageQueue == NULL) {
    ...
    do {
      ...
      if (result == MESSAGE_QUEUE_RESULT_MQ_HAVE_MESSAGE) {
        ...
        (void)MessageQueue_Recv(pMessageQueue, &requestMsg, &requestMsgLength);
      }
...
SendPeerConnectionEvent(PeerConnectionSession_t *pSession,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    if (retMessageQueue == MESSAGE_QUEUE_RESULT_MQ_IS_FULL) {
      LogWarn(("The message queue in peer connection session: %p is full, "
               "dropping request type: %d",
               pSession, requestType));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    if (pRequestContent != NULL) {
      memcpy(&requestMessage.peerConnectionSessionRequestContent,
             pRequestContent, contentLength);
    }

    retMessageQueue =
        MessageQueue_Send(&pSession->requestQueue, &requestMessage,
...
static void OnRtcpSenderReportAudioTimerExpire(void *pParameter) {
  ...
  for (i = 0; i < pSession->transceiverCount; i++) {
    if (pSession->pTransceivers[i]->trackKind == TRANSCEIVER_TRACK_KIND_AUDIO) {
      ...
      (void)SendPeerConnectionEvent(
          pSession, PEER_CONNECTION_SESSION_REQUEST_TYPE_RTCP_SENDER_REPORT,
          &requestMessage.peerConnectionSessionRequestContent.rtcpContent,
          sizeof(
              requestMessage.peerConnectionSessionRequestContent.rtcpContent));
      break;
...
static void OnCloseSessionTimerExpire(void *pParameter) {
  ...
  if ((pSession != NULL) &&
    ...
    LogInfo(("Detect long time no SDP message scenario, closing peer "
             "connection session, pSession: %p",
             pSession));
    EmptyMessageQueue(&pSession->requestQueue);

    (void)SendPeerConnectionEvent(
        pSession, PEER_CONNECTION_SESSION_REQUEST_TYPE_PEER_CONNECTION_CLOSE,
        NULL, 0U);

    /* Wake peer connection session to free resources. */
    retWrite = write(pSession->startupBarrier, &signalStartUpBarrier,
                     sizeof(signalStartUpBarrier));
    if (retWrite != sizeof(uint64_t)) {
      LogError(("Fail to signal start up barrier, errno(%d): %s.", errno,
                strerror(errno)));
    }
  }
}
...
HandleRequest(PeerConnectionSession_t *pSession,
  ...
  requestMsgLength = sizeof(PeerConnectionSessionRequestMessage_t);
  retMessageQueue =
      MessageQueue_Recv(pRequestQueue, &requestMsg, &requestMsgLength);
  if (retMessageQueue == MESSAGE_QUEUE_RESULT_OK) {
    /* Received message, process it. */
    LogDebug(("Peer connection receives request with type: %d",
              requestMsg.requestType));
    switch (requestMsg.requestType) {
    case PEER_CONNECTION_SESSION_REQUEST_TYPE_ADD_REMOTE_CANDIDATE:
      (void)HandleAddRemoteCandidateRequest(pSession, &requestMsg);
      break;
    case PEER_CONNECTION_SESSION_REQUEST_TYPE_PROCESS_ICE_CANDIDATES_AND_PAIRS:
      (void)HandleProcessIceCandidatesAndPairs(pSession, &requestMsg);
      break;
    case PEER_CONNECTION_SESSION_REQUEST_TYPE_PERIOD_CONNECTION_CHECK:
      (void)HandlePeriodConnectionCheck(pSession, &requestMsg);
    ...
    case PEER_CONNECTION_SESSION_REQUEST_TYPE_ICE_CLOSING:
      (void)HandleIceClosing(pSession, &requestMsg);
      break;
    ...
    case PEER_CONNECTION_SESSION_REQUEST_TYPE_RTCP_SENDER_REPORT:
      (void)PeerConnection_OnRtcpSenderReportCallback(pSession, &requestMsg);
      break;
    case PEER_CONNECTION_SESSION_REQUEST_TYPE_PEER_CONNECTION_CLOSE:
      PeerConnection_CloseSession(pSession);

      /* Reset the state to init for next peer since ICE negotiation has not
       * started yet */
      OnClosePeerConnection(pSession);
      break;
    default:
...
static PeerConnectionResult_t HandleAddRemoteCandidateRequest(
  ...
  IceControllerCandidate_t *pRemoteCandidate =
      (IceControllerCandidate_t *)&pRequestMessage
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    if (pSession->state == PEER_CONNECTION_SESSION_STATE_FIND_CONNECTION) {
      ...
      iceControllerResult = IceController_AddRemoteCandidate(
          &pSession->iceControllerContext, &remoteCandidateInfo);
      if (iceControllerResult != ICE_CONTROLLER_RESULT_OK) {
        /* Skip unsupported candidate types (e.g. IPv6 or TCP candidates) - this
         * is expected behavior. */
        LogDebug(
            ("Fail to add remote candidate, result: %d.", iceControllerResult));
        ret = PEER_CONNECTION_RESULT_FAIL_ICE_CONTROLLER_ADD_REMOTE_CANDIDATE;
      }
    } else {
      LogDebug(("Cannot process candidate | Type=%d | Connection State=%d | "
                "Reason=Invalid connection state",
                pRemoteCandidate->candidateType, pSession->state));
    }
  }
...
static PeerConnectionResult_t HandleProcessIceCandidatesAndPairs(
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    iceControllerResult = IceController_ProcessIceCandidatesAndPairs(
        &pSession->iceControllerContext);
    if (iceControllerResult != ICE_CONTROLLER_RESULT_OK) {
      LogError(("Fail to process ICE candidates and pairs, result: %d",
                iceControllerResult));
      ret =
...
static PeerConnectionResult_t HandlePeriodConnectionCheck(
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    iceControllerResult =
        IceController_PeriodConnectionCheck(&pSession->iceControllerContext);
    if (iceControllerResult != ICE_CONTROLLER_RESULT_OK) {
...
HandleIceClosing(PeerConnectionSession_t *pSession,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    iceControllerResult =
        IceController_AddressClosing(&pSession->iceControllerContext);
    if (iceControllerResult != ICE_CONTROLLER_RESULT_OK) {
...
static PeerConnectionResult_t
SendRemoteCandidateRequest(PeerConnectionSession_t *pSession,
                           IceControllerCandidate_t *pRemoteCandidate) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  TimerControllerResult_t retTimer;

  if ((pSession == NULL) || (pRemoteCandidate == NULL)) {
    LogError(("Invalid input, pSession: %p, pRemoteCandidate: %p", pSession,
              pRemoteCandidate));
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  }
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ret = SendPeerConnectionEvent(
        pSession, PEER_CONNECTION_SESSION_REQUEST_TYPE_ADD_REMOTE_CANDIDATE,
        pRemoteCandidate, sizeof(IceControllerCandidate_t));
  }
...
static int32_t
OnIceEventProcessIceCandidatesAndPairs(PeerConnectionSession_t *pSession) {
  int32_t ret = 0;
  ...
  if (ret == 0) {
    result = SendPeerConnectionEvent(
        pSession,
...
static int32_t
OnIceEventPeriodicConnectionCheck(PeerConnectionSession_t *pSession) {
  int32_t ret = 0;
  PeerConnectionResult_t result;
  ...
  if (ret == 0) {
    result = SendPeerConnectionEvent(
        pSession, PEER_CONNECTION_SESSION_REQUEST_TYPE_PERIOD_CONNECTION_CHECK,
        NULL, 0U);
    if (result != PEER_CONNECTION_RESULT_OK) {
      ret = -11;
    }
...
static int32_t OnIceEventClosing(PeerConnectionSession_t *pSession) {
  ...
  if (ret == 0) {
    result = SendPeerConnectionEvent(
        pSession, PEER_CONNECTION_SESSION_REQUEST_TYPE_ICE_CLOSING, NULL, 0U);
    if (result != PEER_CONNECTION_RESULT_OK) {
      ret = -11;
    }
  }
...
static int32_t OnIceEventClosed(PeerConnectionSession_t *pSession) {
  ...
  if (ret == 0) {
    result = SendPeerConnectionEvent(
        pSession, PEER_CONNECTION_SESSION_REQUEST_TYPE_ICE_CLOSED, NULL, 0U);
    if (result != PEER_CONNECTION_RESULT_OK) {
      ret = -11;
    }
  }
...
static int32_t HandleDtlsTermination(PeerConnectionSession_t *pSession) {
  ...
  if (ret == 0) {
    ...
    /* Close the socket context to avoid any input packets triggering unexpected
     * RTP/RTCP handling. */
    PeerConnection_CloseSession(pSession);
  }
...
static int32_t OnDtlsSendHook(void *pCustomContext, const uint8_t *pBuffer,
                              size_t bufferLength) {
  int32_t ret = 0;
  ...
  resultIceController = IceController_SendToRemotePeer(
      &pSession->iceControllerContext, pBuffer, bufferLength);
...
static int32_t
HandleIceEventCallback(void *pCustomContext, IceControllerCallbackEvent_t event,
  ...
  if (pCustomContext == NULL) {
    ...
    ret = -1;
  } else if ((event == ICE_CONTROLLER_CB_EVENT_NONE) ||
             (event >= ICE_CONTROLLER_CB_EVENT_MAX)) {
      ...
      if (pEventMsg != NULL) {
        if (pSession->onIceCandidateReadyCallbackFunc != NULL) {
          pLocalCandidateReadyMsg =
              (PeerConnectionIceLocalCandidate_t *)&pEventMsg
                  ->iceControllerCallbackContent.localCandidateReadyMsg;
          pSession->onIceCandidateReadyCallbackFunc(
              pSession->pOnLocalCandidateReadyCallbackCustomContext,
              pLocalCandidateReadyMsg);
        } else {
          LogError(("No proper callback function to handle local candidate "
                    "ready message."));
        }
      } else {
...
/* Start DTLS handshaking. */
      ...
      /* This must set after StartDtlsHandshake, or the other thread might
       * execute handshake earlier than expectation. */
...
static int32_t StartDtlsHandshake(PeerConnectionSession_t *pSession) {
  ...
  if (ret == 0) {
    ...
    pDtlsSession->xDtlsNetworkContext.pParams =
        &pDtlsSession->xDtlsTransportParams;

    /* Assign transport send callback function/context for different connection
     * types. */
    memset(&pDtlsSession->xDtlsTransportParams, 0,
           sizeof(DtlsTransportParams_t));
    pDtlsSession->xDtlsTransportParams.onDtlsSendHook = OnDtlsSendHook;
    pDtlsSession->xDtlsTransportParams.pOnDtlsSendCustomContext =
        (void *)pSession;
  ...
  if (ret == 0) {
    if (NULL == pSession->pCtx->dtlsContext.localCert.raw.p) {
      LogError(("Fail to get answer cert: NULL == "
                "pSession->pCtx->dtlsContext.localCert.raw.p"));
      ret = -23;
    } else {
      /* Assign local cert to the DTLS session. */
      pDtlsSession->xNetworkCredentials.pClientCert =
          &pSession->pCtx->dtlsContext.localCert;

      // /* Assign local key to the DTLS session. */
      pDtlsSession->xNetworkCredentials.pPrivateKey =
          &pSession->pCtx->dtlsContext.localKey;

      /* Attempt to create a DTLS connection. */
      xNetworkStatus = DTLS_Init(&pDtlsSession->xDtlsNetworkContext,
                                 &pDtlsSession->xNetworkCredentials, 0U);

      if (xNetworkStatus != DTLS_SUCCESS) {
        LogError(("Fail to initialize the DTLS session with return %d ",
                  xNetworkStatus));
        ret = -24;
      }
    }
  ...
  if (ret == 0) {
    pSession->dtlsHandshakingTimeoutMs =
        (NetworkingUtils_GetCurrentTimeUs(NULL) / 1000) +
        PEER_CONNECTION_DTLS_HANDSHAKING_TIMEOUT_MS;
...
static int32_t ExecuteDtlsHandshake(PeerConnectionSession_t *pSession) {
  ...
  if (ret == 0) {
    ...
    if (xNetworkStatus == DTLS_HANDSHAKE_COMPLETE) {
      ...
      LogError(("Error happens when executing DTLS handshake, return %d",
                xNetworkStatus));
      ret = -31;
    } else {
      /* This condition means DTLS handshaking is not complete yet. Wait for Rx
       * packet. */
    }
  }
  ...
  if (ret == 0) {
    if ((NetworkingUtils_GetCurrentTimeUs(NULL) / 1000) >
        pSession->dtlsHandshakingTimeoutMs) {
...
static void PeerConnection_SetTimer(PeerConnectionSession_t *pSession) {
  ...
  for (i = 0; i < PEER_CONNECTION_TRANSCEIVER_MAX_COUNT; i++) {
    if (pSession->pTransceivers[i]->trackKind == TRANSCEIVER_TRACK_KIND_AUDIO) {
      retTimer =
          TimerController_IsTimerSet(&pSession->rtcpAudioSenderReportTimer);
      if (retTimer == TIMER_CONTROLLER_RESULT_NOT_SET) {
        /* The timer is not set before, send the request immendiately and start
         * rtcp audio Sender Report timer. */
        LogDebug(("Trigger rtcp audio Sender Report timer."));
        retTimer = TimerController_SetTimer(
            &pSession->rtcpAudioSenderReportTimer,
            PEER_CONNECTION_RTCP_REPORT_TIMER_INTERVAL_MS + (rand() % 200),
            PEER_CONNECTION_RTCP_REPORT_TIMER_INTERVAL_MS + (rand() % 200));
        if (retTimer != TIMER_CONTROLLER_RESULT_OK) {
          LogError(("Fail to start RTCP Audio Sender Report timer, result: %d",
                    retTimer));
        }
      }
    } else if (pSession->pTransceivers[i]->trackKind ==
               TRANSCEIVER_TRACK_KIND_VIDEO) {
      retTimer =
          TimerController_IsTimerSet(&pSession->rtcpVideoSenderReportTimer);
      if (retTimer == TIMER_CONTROLLER_RESULT_NOT_SET) {
        /* The timer is not set before, send the request immendiately and start
         * rtcp video Sender Report timer. */
        LogDebug(("Trigger rtcp video Sender Report timer."));
        retTimer = TimerController_SetTimer(
            &pSession->rtcpVideoSenderReportTimer,
            PEER_CONNECTION_RTCP_REPORT_TIMER_INTERVAL_MS + (rand() % 200),
            PEER_CONNECTION_RTCP_REPORT_TIMER_INTERVAL_MS + (rand() % 200));
        if (retTimer != TIMER_CONTROLLER_RESULT_OK) {
          LogError(("Fail to start RTCP Video Sender Report timer, result: %d",
                    retTimer));
        }
      }
    } else {
...
static int32_t OnDtlsHandshakeComplete(PeerConnectionSession_t *pSession) {
   ...
   */
  xNetworkStatus = DTLS_VerifyRemoteCertificateFingerprint(
      &pSession->dtlsSession.xDtlsNetworkContext.pParams->dtlsSslContext,
      pSession->remoteCertFingerprint, pSession->remoteCertFingerprintLength);

  if (xNetworkStatus != DTLS_SUCCESS) {
    LogError(("Fail to DTLS_VerifyRemoteCertificateFingerprint with return %d ",
              xNetworkStatus));
    ret = -0x1001;
  }
  ...
  if (ret == 0) {
    ...
    xNetworkStatus = DTLS_PopulateKeyingMaterial(
        &pSession->dtlsSession.xDtlsNetworkContext.pParams->dtlsSslContext,
        &pSession->dtlsSession.xNetworkCredentials.dtlsKeyingMaterial);

    if (xNetworkStatus != DTLS_SUCCESS) {
      LogError(("Fail to DTLS_PopulateKeyingMaterial with return %d ",
                xNetworkStatus));
      ret = -0x1002;
    } else {
      LogDebug(("DTLS_PopulateKeyingMaterial with key_length: %i ",
                pSession->dtlsSession.xNetworkCredentials.dtlsKeyingMaterial
                    .key_length));
    }
  }
  ...
  if (ret == 0) {
    ...
    pSession->inactiveConnectionTimeoutMs =
        (NetworkingUtils_GetCurrentTimeUs(NULL) / 1000) +
        PEER_CONNECTION_INACTIVE_CONNECTION_TIMEOUT_MS;
    for (i = 0; i < pSession->transceiverCount; i++) {
      if (pSession->pTransceivers[i]->onPcEventCallbackFunc) {
        pSession->pTransceivers[i]->onPcEventCallbackFunc(
            pSession->pTransceivers[i]->pOnPcEventCustomContext,
            TRANSCEIVER_CB_EVENT_REMOTE_PEER_READY, NULL);
      }
    }
  }
...
static int32_t ProcessDtlsPacket(PeerConnectionSession_t *pSession,
  ...
  size_t dtlsDecryptBufferLength =
      PEER_CONNECTION_MAX_DTLS_DECRYPTED_DATA_LENGTH;

  xNetworkStatus = DTLS_ProcessPacket(
      &pSession->dtlsSession.xDtlsNetworkContext, pDtlsEncryptData,
      dtlsEncryptDataLength, dtlsDecryptBuffer, &dtlsDecryptBufferLength);
  ...
  if (xNetworkStatus == DTLS_HANDSHAKE_COMPLETE) {
    ...
    {
      if (pSession->state == PEER_CONNECTION_SESSION_STATE_CONNECTION_READY) {
        PeerConnectionSCTP_ProcessSCTPData(pSession, dtlsDecryptBuffer,
                                           dtlsDecryptBufferLength);
      }
    }
#endif /* ENABLE_SCTP_DATA_CHANNEL */
  } else {
    LogInfo(("Error happens when process the DTLS packet, return %d",
             xNetworkStatus));
...
static int32_t HandleNonStunPackets(void *pCustomContext, uint8_t *pBuffer,
                                    size_t bufferLength) {
  int32_t ret = 0;
  PeerConnectionSession_t *pSession = (PeerConnectionSession_t *)pCustomContext;
  ...
  if ((pCustomContext == NULL) || (pBuffer == NULL)) {
    LogWarn(("Invalid input, pCustomContext: %p, pBuffer: %p", pCustomContext,
             pBuffer));
    ret = -1;
  }
  ...
  if (ret == 0) {
    if (bufferLength < 2) {
      ...
      if ((pBuffer[1] >= 192) && (pBuffer[1] <= 223)) {
        /* RTCP packet. */
        resultPeerConnection = PeerConnectionSrtp_HandleSrtcpPacket(
            pSession, pBuffer, bufferLength);
        if (resultPeerConnection != PEER_CONNECTION_RESULT_OK) {
          LogWarn(("Failed to handle SRTCP packets, result: %d",
                   resultPeerConnection));
          ret = -2;
        } else {
          pSession->inactiveConnectionTimeoutMs =
              (NetworkingUtils_GetCurrentTimeUs(NULL) / 1000) +
        ...
        /* RTP packet */
        resultPeerConnection = PeerConnectionSrtp_HandleSrtpPacket(
            pSession, pBuffer, bufferLength);
        if (resultPeerConnection != PEER_CONNECTION_RESULT_OK) {
          LogWarn(("Failed to handle SRTP packets, result: %d",
                   resultPeerConnection));
          ret = -2;
        } else {
          pSession->inactiveConnectionTimeoutMs =
...
static PeerConnectionResult_t
InitializeIceController(PeerConnectionSession_t *pSession,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    initConfig.natTraversalConfigBitmap =
        pSessionConfig->natTraversalConfigBitmap;
    ...
    iceControllerResult =
        IceController_Init(&pSession->iceControllerContext, &initConfig);
    if (iceControllerResult != ICE_CONTROLLER_RESULT_OK) {
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    iceControllerResult = IceController_AddIceServerConfig(
        &pSession->iceControllerContext, &iceServerConfig);
    if (iceControllerResult != ICE_CONTROLLER_RESULT_OK) {
...
static PeerConnectionResult_t
DestroyIceController(PeerConnectionSession_t *pSession) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  IceControllerResult_t iceControllerResult;
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    iceControllerResult =
        IceController_Destroy(&pSession->iceControllerContext);
    if (iceControllerResult != ICE_CONTROLLER_RESULT_OK) {
...
static PeerConnectionResult_t
PeerConnection_ResetTimer(PeerConnectionSession_t *pSession) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    timerControllerResult =
        TimerController_IsTimerSet(&pSession->closeSessionTimer);
...
static PeerConnectionResult_t
AllocateTransceiver(PeerConnectionSession_t *pSession,
...
static PeerConnectionResult_t
InitializeDtlsContext(PeerConnectionDtlsContext_t *pDtlsContext) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  DtlsTransportStatus_t xNetworkStatus = DTLS_SUCCESS;
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    xNetworkStatus = DTLS_CreateCertificateAndKey(GENERATED_CERTIFICATE_BITS, 0,
                                                  &pDtlsContext->localCert,
                                                  &pDtlsContext->localKey);
    if (xNetworkStatus != DTLS_SUCCESS) {
      LogError(
          ("Fail to DTLS_CreateCertificateAndKey, return %d", xNetworkStatus));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    xNetworkStatus = DTLS_CreateCertificateFingerprint(
        &pDtlsContext->localCert, pDtlsContext->localCertFingerprint,
        CERTIFICATE_FINGERPRINT_LENGTH);
    if (xNetworkStatus != DTLS_SUCCESS) {
      LogError(("Fail to dtlsCertificateFingerprint answer cert, return %d",
                xNetworkStatus));
...
static PeerConnectionResult_t
SetDefaultPayloadTypes(PeerConnectionSession_t *pSession) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  int i;
  ...
  LogInfo(("Setting default payload types for session, transceiverCount: %u",
           pSession->transceiverCount));
  for (i = 0;
       i < pSession->transceiverCount && ret == PEER_CONNECTION_RESULT_OK;
       i++) {
    pTransceiver = pSession->pTransceivers[i];

    if (pTransceiver == NULL) {
      LogError(("This is not expected, keep this condition here to avoid NULL "
                "accessing, transceiverCount: %u, current index: %d",
                pSession->transceiverCount, i));
...
PeerConnection_Init(PeerConnectionSession_t *pSession,
                    PeerConnectionSessionConfiguration_t *pSessionConfig) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  ...
  if ((pSession == NULL) || (pSessionConfig == NULL)) {
    LogError(("Invalid input, pSession: %p, pSessionConfig: %p", pSession,
              pSessionConfig));
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  }
  ...
  if ((ret == PEER_CONNECTION_RESULT_OK) &&
    ...
    peerConnectionContext.localUserName[PEER_CONNECTION_USER_NAME_LENGTH] =
        '\0';
    ...
    if (peerConnectionContext.dtlsContext.isInitialized == 0) {
      ...
      pDtlsSession = &pSession->dtlsSession;
      memset(pDtlsSession, 0, sizeof(DtlsSession_t));

      /* pCtx->dtlsContext.isInitialized would be set to 1 in
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    (void)snprintf(tempName, sizeof(tempName), "%s%02d",
                   PEER_CONNECTION_MESSAGE_QUEUE_NAME, initSeq);

    /* Delete message queue from previous round. */
    MessageQueue_Destroy(NULL, tempName);

    retMessageQueue =
        MessageQueue_Create(&pSession->requestQueue, tempName,
                            sizeof(PeerConnectionSessionRequestMessage_t),
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    (void)snprintf(tempName, sizeof(tempName), "%s%02d",
                   PEER_CONNECTION_SESSION_TASK_NAME, initSeq);

    if (pthread_create(&(pSession->pTaskHandler), NULL,
                       PeerConnection_SessionTask, pSession) != 0) {
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    (void)snprintf(tempName, sizeof(tempName), "%s%02d",
                   PEER_CONNECTION_SESSION_RX_TASK_NAME, initSeq++);

    if (pthread_create(&(pSession->pSocketListener), NULL,
...
#if ENABLE_TWCC_SUPPORT
  if (ret == PEER_CONNECTION_RESULT_OK) {
    resultRtcpTwccManager = RtcpTwccManager_Init(
        &pSession->pCtx->rtcpTwccManager, pSession->pCtx->twccPacketInfo,
        PEER_CONNECTION_RTCP_TWCC_MAX_ARRAY);
    if (resultRtcpTwccManager != RTCP_TWCC_MANAGER_RESULT_OK) {
      LogError(("Fail to Initialize RTCP TWCC Manager, result: %d",
                resultRtcpTwccManager));
      ret = PEER_CONNECTION_RESULT_FAIL_RTCP_TWCC_INIT;
    }
  }

  if (ret == PEER_CONNECTION_RESULT_OK) {
    if (pthread_mutex_init(&(pSession->twccMetaData.twccBitrateMutex), NULL) !=
        0) {
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    retTimer =
        TimerController_Create(&pSession->rtcpAudioSenderReportTimer,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    retTimer =
        TimerController_Create(&pSession->rtcpVideoSenderReportTimer,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    retTimer = TimerController_Create(&pSession->closeSessionTimer,
                                      OnCloseSessionTimerExpire, pSession);
    if (retTimer != TIMER_CONTROLLER_RESULT_OK) {
      LogError(("TimerController_Create return fail, result: %d", retTimer));
      ret = PEER_CONNECTION_RESULT_FAIL_TIMER_INIT;
    } else {
...
PeerConnectionResult_t PeerConnection_SetLocalDescription(
    PeerConnectionSession_t *pSession,
    const PeerConnectionBufferSessionDescription_t *pBufferSessionDescription) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;

  if ((pSession == NULL) || (pBufferSessionDescription == NULL)) {
    LogError(("Invalid input, pSession: %p, pBufferSessionDescription: %p",
              pSession, pBufferSessionDescription));
  }
...
PeerConnectionResult_t PeerConnection_SetRemoteDescription(
    PeerConnectionSession_t *pSession,
    const PeerConnectionBufferSessionDescription_t *pBufferSessionDescription) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  IceControllerResult_t iceControllerResult;
  ...
  if ((pSession == NULL) || (pBufferSessionDescription == NULL)) {
    ...
    LogError(("Invalid input, pBufferSessionDescription->pSdpBuffer: %p, "
              "pBufferSessionDescription->sdpBufferLength: %lu",
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    memcpy(pTargetRemoteSdp->pSdpBuffer, pBufferSessionDescription->pSdpBuffer,
           pTargetRemoteSdp->sdpBufferLength);
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    if ((pTargetRemoteSdp->sdpDescription.quickAccess.pIceUfrag == NULL) ||
        (pTargetRemoteSdp->sdpDescription.quickAccess.iceUfragLength +
             PEER_CONNECTION_USER_NAME_LENGTH >
         (PEER_CONNECTION_USER_NAME_LENGTH << 1))) {
      LogWarn(("Remote user name is too long to store, length: %lu",
               pTargetRemoteSdp->sdpDescription.quickAccess.iceUfragLength));
      ret = PEER_CONNECTION_RESULT_INVALID_REMOTE_USERNAME;
    } else if ((pTargetRemoteSdp->sdpDescription.quickAccess.pIceUfrag ==
                NULL) ||
               (pTargetRemoteSdp->sdpDescription.quickAccess.icePwdLength >
                PEER_CONNECTION_PASSWORD_LENGTH)) {
      LogWarn(("Remote user password is too long to store, length: %lu",
               pTargetRemoteSdp->sdpDescription.quickAccess.icePwdLength));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    memcpy(pSession->remoteUserName,
           ...
           pTargetRemoteSdp->sdpDescription.quickAccess.iceUfragLength);
    pSession->remoteUserName[pTargetRemoteSdp->sdpDescription.quickAccess
                                 .iceUfragLength] = '\0';
    memcpy(pSession->remotePassword,
           pTargetRemoteSdp->sdpDescription.quickAccess.pIcePwd,
           pTargetRemoteSdp->sdpDescription.quickAccess.icePwdLength);
    pSession->remotePassword[pTargetRemoteSdp->sdpDescription.quickAccess
                                 .icePwdLength] = '\0';
    snprintf(pSession->combinedName,
             (PEER_CONNECTION_USER_NAME_LENGTH << 1) + 2, "%.*s:%.*s",
             (int)pTargetRemoteSdp->sdpDescription.quickAccess.iceUfragLength,
             pSession->remoteUserName, PEER_CONNECTION_USER_NAME_LENGTH,
    ...
    pSession->remoteCertFingerprint[pTargetRemoteSdp->sdpDescription.quickAccess
                                        .fingerprintLength] = '\0';
    pSession->remoteCertFingerprintLength =
        pTargetRemoteSdp->sdpDescription.quickAccess.fingerprintLength;

    iceControllerResult = IceController_Start(
        &pSession->iceControllerContext, peerConnectionContext.localUserName,
        PEER_CONNECTION_USER_NAME_LENGTH, peerConnectionContext.localPassword,
        PEER_CONNECTION_PASSWORD_LENGTH, pSession->remoteUserName,
        pTargetRemoteSdp->sdpDescription.quickAccess.iceUfragLength,
        pSession->remotePassword,
        pTargetRemoteSdp->sdpDescription.quickAccess.icePwdLength,
        pSession->combinedName, strlen(pSession->combinedName));
    if (iceControllerResult != ICE_CONTROLLER_RESULT_OK) {
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    pSession->rtpConfig.twccId =
        (uint16_t)pTargetRemoteSdp->sdpDescription.quickAccess.twccExtId;
    pSession->rtpConfig.remoteVideoSsrc =
        pTargetRemoteSdp->sdpDescription.quickAccess.videoSsrc;
    pSession->rtpConfig.remoteAudioSsrc =
        pTargetRemoteSdp->sdpDescription.quickAccess.audioSsrc;
  }
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    LogVerbose(
        ("REMOTE Candidates Count : %d",
         pTargetRemoteSdp->sdpDescription.quickAccess.remoteCandidateCount));

    if (pTargetRemoteSdp->sdpDescription.quickAccess.remoteCandidateCount !=
        0) {
      for (i = 0;
           i <
           pTargetRemoteSdp->sdpDescription.quickAccess.remoteCandidateCount;
           i++) {
        ret = PeerConnection_AddRemoteCandidate(
            pSession,
            pTargetRemoteSdp->sdpDescription.quickAccess.pRemoteCandidates[i],
            pTargetRemoteSdp->sdpDescription.quickAccess
                .remoteCandidateLengths[i]);

        if (ret != PEER_CONNECTION_RESULT_OK) {
          LogError(("Fail to add remote candidate from SDP offer, ret: %d, "
                    "candidate(%lu): %.*s",
                    ret,
                    pTargetRemoteSdp->sdpDescription.quickAccess
                        .remoteCandidateLengths[i],
                    (int)pTargetRemoteSdp->sdpDescription.quickAccess
                        .remoteCandidateLengths[i],
                    pTargetRemoteSdp->sdpDescription.quickAccess
                        .pRemoteCandidates[i]));
          ...
          LogDebug(
              ("Added remote candidate from SDP offer, candidate(%lu): %.*s",
               pTargetRemoteSdp->sdpDescription.quickAccess
                   .remoteCandidateLengths[i],
               (int)pTargetRemoteSdp->sdpDescription.quickAccess
                   .remoteCandidateLengths[i],
...
PeerConnectionResult_t
PeerConnection_SetVideoOnFrame(PeerConnectionSession_t *pSession,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    pSession->videoSrtpReceiver.onFrameReadyCallbackFunc =
        onFrameReadyCallbackFunc;
    pSession->videoSrtpReceiver.pOnFrameReadyCallbackCustomContext =
        pOnFrameReadyCallbackCustomContext;
  }

  return ret;
}
PeerConnectionResult_t
PeerConnection_SetAudioOnFrame(PeerConnectionSession_t *pSession,
                               OnFrameReadyCallback_t onFrameReadyCallbackFunc,
                               void *pOnFrameReadyCallbackCustomContext) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    pSession->audioSrtpReceiver.onFrameReadyCallbackFunc =
        onFrameReadyCallbackFunc;
    pSession->audioSrtpReceiver.pOnFrameReadyCallbackCustomContext =
        pOnFrameReadyCallbackCustomContext;
  }
...
PeerConnectionResult_t
PeerConnection_AddRemoteCandidate(PeerConnectionSession_t *pSession,
  ...
  if ((pSession == NULL) || (pDecodeMessage == NULL)) {
    LogError(("Invalid input, pSession: %p, pDecodeMessage: %p", pSession,
              pDecodeMessage));
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  }
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    iceControllerResult = IceController_DeserializeIceCandidate(
        pDecodeMessage, decodeMessageLength, &candidate);
    if (iceControllerResult != ICE_CONTROLLER_RESULT_OK) {
      LogWarn(("IceController_DeserializeIceCandidate fail, result: %d, "
               "dropping ICE candidate.",
               iceControllerResult));
...
PeerConnectionResult_t
PeerConnection_AddTransceiver(PeerConnectionSession_t *pSession,
                              Transceiver_t *pTransceiver) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
...
#if ENABLE_SCTP_DATA_CHANNEL
PeerConnectionResult_t
PeerConnection_AddDataChannel(PeerConnectionSession_t *pSession) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
...
PeerConnectionResult_t
PeerConnection_MatchTransceiverBySsrc(PeerConnectionSession_t *pSession,
                                      uint32_t ssrc,
  ...
  if ((pSession == NULL) || (ppTransceiver == NULL)) {
    LogError(("Invalid input, pSession: %p, ppTransceiver: %p", pSession,
              ppTransceiver));
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  }
    ...
    *ppTransceiver = NULL;
    for (i = 0; i < pSession->transceiverCount; i++) {
      if ((ssrc == pSession->pTransceivers[i]->ssrc) ||
          (ssrc == pSession->pTransceivers[i]->rtxSsrc)) {
...
PeerConnectionResult_t
PeerConnection_CloseSession(PeerConnectionSession_t *pSession) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  int i;
    ...
    if (notifyTransceiver != 0U) {
      for (i = 0; i < pSession->transceiverCount; i++) {
        if (pSession->pTransceivers[i]->onPcEventCallbackFunc) {
          pSession->pTransceivers[i]->onPcEventCallbackFunc(
              pSession->pTransceivers[i]->pOnPcEventCustomContext,
              TRANSCEIVER_CB_EVENT_REMOTE_PEER_CLOSED, NULL);
        }
      }
    }
  }
...
PeerConnectionResult_t
PeerConnection_WriteFrame(PeerConnectionSession_t *pSession,
                          Transceiver_t *pTransceiver,
                          const PeerConnectionFrame_t *pFrame) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;

  if ((pSession == NULL) || (pTransceiver == NULL) || (pFrame == NULL)) {
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    if (pSession->state < PEER_CONNECTION_SESSION_STATE_CONNECTION_READY) {
      LogInfo(("This session is not ready for sending frames, state: %d.",
               pSession->state));
    } else if (
        TRANSCEIVER_IS_CODEC_ENABLED(
      ...
      LogError(("Codec is not supported, codec bit map: 0x%x",
...
PeerConnectionResult_t PeerConnection_CreateOffer(
    PeerConnectionSession_t *pSession,
  ...
  if ((pSession == NULL) || (pOutputBufferSessionDescription == NULL) ||
      (pOutputSerializedSdpMessage == NULL) ||
      (pOutputSerializedSdpMessageLength == NULL)) {
    LogError(("Invalid input, pSession: %p, pOutputBufferSessionDescription: "
              "%p, pOutputSerializedSdpMessage: %p, "
              "pOutputSerializedSdpMessageLength: %p",
              pSession, pOutputBufferSessionDescription,
              pOutputSerializedSdpMessage, pOutputSerializedSdpMessageLength));
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  } else if (pOutputBufferSessionDescription->pSdpBuffer == NULL) {
    LogError(("Invalid input, pOutputBufferSessionDescription->pSdpBuffer: %p",
              pOutputBufferSessionDescription->pSdpBuffer));
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  } else {
    /* Empty else marker. */
  }
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ret = PeerConnectionSdp_PopulateSessionDescription(
        pSession, NULL, pOutputBufferSessionDescription,
        pOutputSerializedSdpMessage, pOutputSerializedSdpMessageLength);
  }
...
PeerConnectionResult_t PeerConnection_CreateAnswer(
    PeerConnectionSession_t *pSession,
    PeerConnectionBufferSessionDescription_t *pOutputBufferSessionDescription,
    char *pOutputSerializedSdpMessage,
    size_t *pOutputSerializedSdpMessageLength) {
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ret = PeerConnectionSdp_PopulateSessionDescription(
        pSession, &pSession->remoteSessionDescription,
        pOutputBufferSessionDescription, pOutputSerializedSdpMessage,
...
PeerConnectionResult_t PeerConnection_SetOnLocalCandidateReady(
    PeerConnectionSession_t *pSession,
    OnIceCandidateReadyCallback_t onLocalCandidateReadyCallbackFunc,
    void *pOnLocalCandidateReadyCallbackCustomContext) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;

  if ((pSession == NULL) || (onLocalCandidateReadyCallbackFunc == NULL)) {
    LogError(
        ("Invalid input, pSession: %p, onLocalCandidateReadyCallbackFunc: %p",
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    pSession->onIceCandidateReadyCallbackFunc =
        onLocalCandidateReadyCallbackFunc;
    pSession->pOnLocalCandidateReadyCallbackCustomContext =
        pOnLocalCandidateReadyCallbackCustomContext;
  }

  return ret;
}

PeerConnectionResult_t PeerConnection_AddIceServerConfig(
    PeerConnectionSession_t *pSession,
    PeerConnectionSessionConfiguration_t *pSessionConfig) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  IceControllerResult_t iceControllerResult;
...
static PeerConnectionResult_t PeerConnection_OnRtcpSenderReportCallback(
  ...
  uint64_t currentTimeUs = pRequestMessage->peerConnectionSessionRequestContent
                               .rtcpContent.currentTimeUs;
  const Transceiver_t *pTransceiver =
      pRequestMessage->peerConnectionSessionRequestContent.rtcpContent
          .pTransceiver;

  if ((pSession == NULL) || (pTransceiver == NULL)) {
    LogError(("Invalid input, pSession: %p, pTransceiver: %p", pSession,
              pTransceiver));
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  }
  ...
  if (!readyToSend) {
    LogVerbose(
        ("Send Report No Frames are sent to SSRC :  %u", pTransceiver->ssrc));
  } else {
    if (pTransceiver->trackKind == TRANSCEIVER_TRACK_KIND_AUDIO) {
      rtcpSenderReport.senderInfo.rtpTime =
          pTransceiver->rtpSender.rtpTimeOffset +
          PEER_CONNECTION_SRTP_CONVERT_TIME_US_TO_RTP_TIMESTAMP(
              pSession->audioSrtpReceiver.rxJitterBuffer.clockRate,
              currentTimeUs -
                  pTransceiver->rtpSender.rtpFirstFrameWallClockTimeUs);
    } else {
      rtcpSenderReport.senderInfo.rtpTime =
          pTransceiver->rtpSender.rtpTimeOffset +
          PEER_CONNECTION_SRTP_CONVERT_TIME_US_TO_RTP_TIMESTAMP(
              pSession->videoSrtpReceiver.rxJitterBuffer.clockRate,
              currentTimeUs -
                  pTransceiver->rtpSender.rtpFirstFrameWallClockTimeUs);
    }
    rtcpSenderReport.senderSsrc = pTransceiver->ssrc;
    rtcpSenderReport.senderInfo.ntpTime =
        NetworkingUtils_GetNTPTimeFromUnixTimeUs(currentTimeUs);
    rtcpSenderReport.senderInfo.packetCount =
        pTransceiver->rtcpStats.rtpPacketsTransmitted;
    rtcpSenderReport.senderInfo.octetCount =
        pTransceiver->rtcpStats.rtpBytesTransmitted;

    /* Since the Master isn't processing the received RTP Packets. We don't have
     * any reception reports.*/
    rtcpSenderReport.pReceptionReports = NULL;
    rtcpSenderReport.numReceptionReports = 0;

    ret = PeerConnectionSrtcp_ConstructSenderReportPacket(
        (pSession), &(rtcpSenderReport), &(srtcpPacket[0]),
        &(srtcpPacketLength));
    if (ret != PEER_CONNECTION_RESULT_OK) {
    ...
    if (ret == PEER_CONNECTION_RESULT_OK) {
      iceControllerResult = IceController_SendToRemotePeer(
          &(pSession->iceControllerContext), (srtcpPacket), srtcpPacketLength);
      ...
      if (iceControllerResult != ICE_CONTROLLER_RESULT_OK) {
        ...
        LogDebug(
            ("Send RTCP Sender Report with Status : %u  to SSRC :  %u, NTP "
             "Time :  %lu, RTP Time:  %u,  PacketCount : %u, OctetCount : %u",
             ret, rtcpSenderReport.senderSsrc,
             rtcpSenderReport.senderInfo.ntpTime,
             rtcpSenderReport.senderInfo.rtpTime,
             rtcpSenderReport.senderInfo.packetCount,
             rtcpSenderReport.senderInfo.octetCount));
...
PeerConnectionResult_t PeerConnection_SetPictureLossIndicationCallback(
    PeerConnectionSession_t *pSession,
...
#if ENABLE_TWCC_SUPPORT
PeerConnectionResult_t PeerConnection_SetSenderBandwidthEstimationCallback(
    PeerConnectionSession_t *pSession,
