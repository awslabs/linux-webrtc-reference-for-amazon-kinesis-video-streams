...
#define TRANSCEIVER_CODEC_STRING_MAX_LENGTH                                    \
  (3) /* The maximum value of codec is now 127, which has length 3 in string.  \
       */
...
typedef struct TransceiverRtcpStats {
  ...
  uint64_t rtpPacketsTransmitted; // Total number of bytes sent for this SSRC.
                                  ...
                                  // section 6.4.1.
  uint64_t rtpBytesTransmitted; // The total number of payload octets (i.e., not
...
typedef struct Transceiver {
  ...
  uint32_t
      codecBitMap; // Use TransceiverRtcCodecBit_t to set corresponding bits
