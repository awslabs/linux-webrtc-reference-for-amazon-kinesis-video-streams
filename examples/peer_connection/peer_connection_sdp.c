...
#define PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_VALUE_RTCP_FB_H265                 \
  PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_VALUE_RTCP_FB_H264
#define PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_VALUE_RTCP_FB_H265_LENGTH          \
  PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_VALUE_RTCP_FB_H264_LENGTH
...
#define PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_VALUE_TWCC_EXT_URL                 \
  "http://www.ietf.org/id/draft-holmer-rmcat-transport-wide-cc-extensions-01"
#define PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_VALUE_TWCC_EXT_URL_LENGTH (73)
#define PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_VALUE_RTCP_FB_TRANSPORT_CC         \
  "transport-cc"
#define PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_VALUE_RTCP_FB_TRANSPORT_CC_LENGTH  \
  (12)
...
#define PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_VALUE_FMTP_H264_PROFILE_42E01F_LEVEL_ASYMMETRY_ALLOWED_PACKETIZATION \
  "level-asymmetry-allowed=1;packetization-mode=1;profile-level-id=42e01f"
#define PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_VALUE_FMTP_OPUS                    \
  "minptime=10;useinbandfec=1"
#define PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_VALUE_FMTP_H265                    \
...
#define PEER_CONNECTION_SDP_SET_PAYLOAD(rtxPayload, aptPayload)                \
...
FindH264FmtpAttribute(SdpControllerAttributes_t *pAttributes,
  ...
  if (pAttributes && pTargetRtpmapAttribute) {
    ...
    while (pCodecStart &&
           codecStringLength < pTargetRtpmapAttribute->attributeValueLength) {
      if ((pCodecStart[codecStringLength] >= '0') &&
          (pCodecStart[codecStringLength] <= '9')) {
        codecStringLength++;
      } else {
        break;
      }
    ...
    if (codecStringLength) {
      for (i = 0; i < attributeCount; i++) {
        if ((pAttributes[i].attributeNameLength ==
             PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_NAME_FMTP_LENGTH) &&
            ...
            (strncmp(pCodecStart, pAttributes[i].pAttributeValue,
                     codecStringLength) == 0)) {
          /* Found the fmtp. */
...
static uint32_t
CalculateH264ScoreByFmtp(SdpControllerAttributes_t *pTargetFmtpAttribute) {
  uint32_t score = 0;
  ...
  do {
    ...
    if (StringUtils_StrStr(
            pTargetFmtpAttribute->pAttributeValue,
    ...
    pProfileLevelIdStart =
        StringUtils_StrStr(pTargetFmtpAttribute->pAttributeValue,
                           pTargetFmtpAttribute->attributeValueLength,
    ...
    pProfileLevelIdStart =
        pProfileLevelIdStart + PEER_CONNECTION_SDP_H264_PROFILE_LEVEL_ID_LENGTH;
    remainLength = pTargetFmtpAttribute->pAttributeValue +
                   pTargetFmtpAttribute->attributeValueLength -
                   pProfileLevelIdStart;
    stringResult = StringUtils_ConvertStringToHex(
        pProfileLevelIdStart, remainLength, &profileLevelId);
    if (stringResult != STRING_UTILS_RESULT_OK) {
      LogWarn(("Fail to convert string(%lu): %.*s to hex.", remainLength,
               (int)remainLength, pProfileLevelIdStart));
      break;
    }

    if (((profileLevelId & PEER_CONNECTION_SDP_H264_FMTP_SUBPROFILE_MASK) ==
         (PEER_CONNECTION_SDP_H264_PROFILE_42E01F &
          PEER_CONNECTION_SDP_H264_FMTP_SUBPROFILE_MASK)) &&
        ((profileLevelId & PEER_CONNECTION_SDP_H264_FMTP_PROFILE_LEVEL_MASK) <=
         (PEER_CONNECTION_SDP_H264_PROFILE_42E01F &
          PEER_CONNECTION_SDP_H264_FMTP_PROFILE_LEVEL_MASK))) {
      score++;
    }
  } while (0);
...
static uint32_t
CollectAttributesCodec(SdpControllerAttributes_t *pAttributes,
                       uint8_t attributeCount,
  ...
  if ((pAttributes == NULL) || (codecPayloads == NULL)) {
    LogError(("Invalid input, pAttributes: %p,codecPayloads: %p", pAttributes,
              codecPayloads));
  } else {
    for (i = 0; i < attributeCount; i++) {
      if ((pAttributes[i].attributeNameLength ==
           PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_NAME_RTPMAP_LENGTH) &&
          (strncmp(PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_NAME_RTPMAP,
                   pAttributes[i].pAttributeName,
                   PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_NAME_RTPMAP_LENGTH) ==
           0)) {
        if ((highestH264Score < PEER_CONNECTION_SDP_H264_FMTP_HIGHEST_SCORE) &&
            (pAttributes[i].attributeValueLength >=
             PEER_CONNECTION_SDP_CODEC_H264_VALUE_LENGTH) &&
            (strncmp(PEER_CONNECTION_SDP_CODEC_H264_VALUE,
                     pAttributes[i].pAttributeValue +
                         pAttributes[i].attributeValueLength -
                         PEER_CONNECTION_SDP_CODEC_H264_VALUE_LENGTH,
                     PEER_CONNECTION_SDP_CODEC_H264_VALUE_LENGTH) == 0)) {
          pH264FmtpAttribute = FindH264FmtpAttribute(
              pAttributes, attributeCount, &pAttributes[i]);
          h264Score = CalculateH264ScoreByFmtp(pH264FmtpAttribute);
          if ((h264Score >= PEER_CONNECTION_SDP_H264_FMTP_MINIMUM_SCORE) &&
              (highestH264Score < h264Score)) {
            stringResult = StringUtils_ConvertStringToUl(
                pAttributes[i].pAttributeValue,
                pAttributes[i].attributeValueLength -
                    PEER_CONNECTION_SDP_CODEC_H264_VALUE_LENGTH,
                &aptPayload);
            if (stringResult != STRING_UTILS_RESULT_OK) {
              LogWarn(("StringUtils_ConvertStringToUl fail, result %d, "
                       "converting %.*s to %u",
                       stringResult,
                       (int)pAttributes[i].attributeValueLength -
                           PEER_CONNECTION_SDP_CODEC_H264_VALUE_LENGTH,
                       pAttributes[i].pAttributeValue, aptPayload));
            } else {
              ...
              codecPayloads
                  [TRANSCEIVER_RTC_CODEC_H264_PROFILE_42E01F_LEVEL_ASYMMETRY_ALLOWED_PACKETIZATION_BIT] =
                      PEER_CONNECTION_SDP_SET_PAYLOAD(0, aptPayload);
              LogDebug((
                  "Found H264 codec: %u",
                  codecPayloads
                      [TRANSCEIVER_RTC_CODEC_H264_PROFILE_42E01F_LEVEL_ASYMMETRY_ALLOWED_PACKETIZATION_BIT]));
        ...
        } else if (!TRANSCEIVER_IS_CODEC_ENABLED(
                       codecBitMap, TRANSCEIVER_RTC_CODEC_VP8_BIT) &&
                   (pAttributes[i].attributeValueLength >=
                    PEER_CONNECTION_SDP_CODEC_VP8_VALUE_LENGTH) &&
          ...
          if (stringResult != STRING_UTILS_RESULT_OK) {
            ...
            codecPayloads[TRANSCEIVER_RTC_CODEC_VP8_BIT] =
                PEER_CONNECTION_SDP_SET_PAYLOAD(0, aptPayload);
            LogDebug(("Found VP8 codec: %u",
                      codecPayloads[TRANSCEIVER_RTC_CODEC_VP8_BIT]));
          }
        } else if (!TRANSCEIVER_IS_CODEC_ENABLED(
                       codecBitMap, TRANSCEIVER_RTC_CODEC_H265_BIT) &&
                   (pAttributes[i].attributeValueLength >=
                    PEER_CONNECTION_SDP_CODEC_H265_VALUE_LENGTH) &&
          ...
          if (stringResult != STRING_UTILS_RESULT_OK) {
            ...
            codecPayloads[TRANSCEIVER_RTC_CODEC_H265_BIT] =
                PEER_CONNECTION_SDP_SET_PAYLOAD(0, aptPayload);
            LogDebug(("Found H265 codec: %u",
                      codecPayloads[TRANSCEIVER_RTC_CODEC_H265_BIT]));
          }
        } else if (!TRANSCEIVER_IS_CODEC_ENABLED(
                       codecBitMap, TRANSCEIVER_RTC_CODEC_OPUS_BIT) &&
                   (pAttributes[i].attributeValueLength >=
                    PEER_CONNECTION_SDP_CODEC_OPUS_VALUE_LENGTH) &&
          ...
          if (stringResult != STRING_UTILS_RESULT_OK) {
            ...
            codecPayloads[TRANSCEIVER_RTC_CODEC_OPUS_BIT] =
                PEER_CONNECTION_SDP_SET_PAYLOAD(0, aptPayload);
            LogDebug(("Found OPUS codec: %u",
                      codecPayloads[TRANSCEIVER_RTC_CODEC_OPUS_BIT]));
          }
        } else if (!TRANSCEIVER_IS_CODEC_ENABLED(
                       codecBitMap, TRANSCEIVER_RTC_CODEC_MULAW_BIT) &&
                   (pAttributes[i].attributeValueLength >=
                    PEER_CONNECTION_SDP_CODEC_MULAW_VALUE_LENGTH) &&
          ...
          if (stringResult != STRING_UTILS_RESULT_OK) {
            ...
            codecPayloads[TRANSCEIVER_RTC_CODEC_MULAW_BIT] =
                PEER_CONNECTION_SDP_SET_PAYLOAD(0, aptPayload);
            LogDebug(("Found MULAW codec: %u",
                      codecPayloads[TRANSCEIVER_RTC_CODEC_MULAW_BIT]));
          }
        } else if (!TRANSCEIVER_IS_CODEC_ENABLED(
                       codecBitMap, TRANSCEIVER_RTC_CODEC_ALAW_BIT) &&
                   (pAttributes[i].attributeValueLength >=
                    PEER_CONNECTION_SDP_CODEC_ALAW_VALUE_LENGTH) &&
          ...
          if (stringResult != STRING_UTILS_RESULT_OK) {
            ...
            codecPayloads[TRANSCEIVER_RTC_CODEC_ALAW_BIT] =
                PEER_CONNECTION_SDP_SET_PAYLOAD(0, aptPayload);
            LogDebug(("Found ALAW codec: %u",
                      codecPayloads[TRANSCEIVER_RTC_CODEC_ALAW_BIT]));
          }
        } else {
          /* Do nothing if it's not known string. */
        }
      } else if ((pAttributes[i].attributeNameLength ==
                  PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_NAME_FMTP_LENGTH) &&
                 (strncmp(
                      PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_NAME_FMTP,
                      pAttributes[i].pAttributeName,
                      PEER_CONNECTION_SDP_MEDIA_ATTRIBUTE_NAME_FMTP_LENGTH) ==
                  0)) {
        ...
        /* Parse RTX payload */
        stringResult = StringUtils_ConvertStringToUl(
            pAttributes[i].pAttributeValue, pAttributes[i].attributeValueLength,
            &rtxPayload);
        if (stringResult != STRING_UTILS_RESULT_OK) {
          LogWarn(("StringUtils_ConvertStringToUl RTX payload fail, result %d, "
                   "converting %.*s to %u",
                   stringResult, (int)pAttributes[i].attributeValueLength,
                   pAttributes[i].pAttributeValue, rtxPayload));
          continue;
        }

        /* Parse APT payload */
        stringLength = pAttributes[i].pAttributeValue +
                       pAttributes[i].attributeValueLength - pAtp -
                       PEER_CONNECTION_SDP_CODEC_APT_VALUE_LENGTH;
        stringResult = StringUtils_ConvertStringToUl(
            pAtp + PEER_CONNECTION_SDP_CODEC_APT_VALUE_LENGTH, stringLength,
            &aptPayload);
        if (stringResult != STRING_UTILS_RESULT_OK) {
          LogWarn(("StringUtils_ConvertStringToUl APT payload fail, result %d, "
                   "converting %.*s to %u",
                   stringResult, (int)pAttributes[i].attributeValueLength,
                   pAttributes[i].pAttributeValue, aptPayload));
          continue;
        ...
        for (j = TRANSCEIVER_RTC_CODEC_MULAW_BIT; j < TRANSCEIVER_RTC_CODEC_NUM;
             j++) {
          if (TRANSCEIVER_IS_CODEC_ENABLED(codecBitMap, j) &&
              (PEER_CONNECTION_SDP_GET_APT_CODEC_FROM_PAYLOAD(
                   codecPayloads[j]) == aptPayload)) {
            /* Found APT payload, update RTX payload. */
            codecPayloads[j] =
                PEER_CONNECTION_SDP_SET_PAYLOAD(rtxPayload, aptPayload);
          }
        }
      } else {
        /* Empty else. */
...
GetPayloadTypesFromMedia(SdpControllerMediaDescription_t *pMediaDescription,
  ...
  if ((pMediaDescription == NULL) || (codecPayloads == NULL)) {
    LogError(("Invalid input, pMediaDescription: %p, codecPayloads: %p",
              pMediaDescription, codecPayloads));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    do {
      ...
      /* Here is one media name example: "audio 9 UDP/TLS/RTP/SAVPF 111 63 103
       * 104 9 0 8 106 105 13 110 112 113 126". The beginning 3 tokens are not
       * codec part, just ignore them here. */
      if (count > 3) {
        if ((tokenLength ==
             PEER_CONNECTION_SDP_CODEC_MULAW_DEFAULT_INDEX_LENGTH) &&
            (strncmp(PEER_CONNECTION_SDP_CODEC_MULAW_DEFAULT_INDEX, pStart,
                     tokenLength) == 0)) {
          TRANSCEIVER_ENABLE_CODEC(*pCodecBitMap,
                                   TRANSCEIVER_RTC_CODEC_MULAW_BIT);
          codecPayloads[TRANSCEIVER_RTC_CODEC_MULAW_BIT] =
              TRANSCEIVER_RTC_CODEC_DEFAULT_PAYLOAD_MULAW;
          break;
        } else if ((tokenLength ==
                    PEER_CONNECTION_SDP_CODEC_ALAW_DEFAULT_INDEX_LENGTH) &&
                   (strncmp(PEER_CONNECTION_SDP_CODEC_ALAW_DEFAULT_INDEX,
                            pStart, tokenLength) == 0)) {
          TRANSCEIVER_ENABLE_CODEC(*pCodecBitMap,
                                   TRANSCEIVER_RTC_CODEC_ALAW_BIT);
          codecPayloads[TRANSCEIVER_RTC_CODEC_MULAW_BIT] =
              TRANSCEIVER_RTC_CODEC_DEFAULT_PAYLOAD_ALAW;
          break;
        } else {
      ...
      if (pEnd) {
        pStart = pEnd + 1;
        remainLength =
            remainLength - tokenLength - 1; /* minus extra 1 for space. */
      }
    } while (pEnd != NULL);

    /* Find proper codec bit map by looking for rtpmap. */
    *pCodecBitMap |= CollectAttributesCodec(
        pMediaDescription->attributes, pMediaDescription->mediaAttributesCount,
        codecPayloads);
    LogDebug(
        ("Scanned codec from remote media description, *pCodecBitMap: 0x%x",
         *pCodecBitMap));
  }
...
static PeerConnectionResult_t SetPayloadType(
  ...
  if ((pSession == NULL) || (pMediaDescription == NULL) ||
      (pCodecBitMap == NULL) || (codecPayloads == NULL) ||
      (isTransceiverCodecSet == NULL)) {
    LogError(
        ("Invalid input, pSession: %p, pMediaDescription: %p,pCodecBitMap: %p, "
         "codecPayloads: %p, isTransceiverCodecSet: %p",
         pSession, pMediaDescription, pCodecBitMap, codecPayloads,
  ...
  if ((ret == PEER_CONNECTION_RESULT_OK) &&
    ...
    for (currentTransceiverIdx = 0;
         currentTransceiverIdx < pSession->transceiverCount;
      ...
      if ((isTransceiverCodecSet[currentTransceiverIdx] != 0) ||
          (pSession->pTransceivers[currentTransceiverIdx]->trackKind !=
           trackKind) ||
          ((*pCodecBitMap &
            pSession->pTransceivers[currentTransceiverIdx]->codecBitMap) ==
           0)) {
        LogDebug(("Skip transceiver index: %d, isTransceiverCodecSet[%d]: %d, "
                  "pTransceivers[%d]->trackKind: %d, "
                  "pTransceivers[%d]->codecBitMap: %x",
                  currentTransceiverIdx, currentTransceiverIdx,
                  isTransceiverCodecSet[currentTransceiverIdx],
                  currentTransceiverIdx,
                  pSession->pTransceivers[currentTransceiverIdx]->trackKind,
                  currentTransceiverIdx,
                  pSession->pTransceivers[currentTransceiverIdx]->codecBitMap));
      ...
      if (TRANSCEIVER_IS_CODEC_ENABLED(
        ...
        *pTargetCodecPayload = PEER_CONNECTION_SDP_GET_APT_CODEC_FROM_PAYLOAD(
            codecPayloads
                [TRANSCEIVER_RTC_CODEC_H264_PROFILE_42E01F_LEVEL_ASYMMETRY_ALLOWED_PACKETIZATION_BIT]);
        *pTargetCodecRtxPayload = PEER_CONNECTION_SDP_GET_RTX_CODEC_FROM_PAYLOAD(
            codecPayloads
                [TRANSCEIVER_RTC_CODEC_H264_PROFILE_42E01F_LEVEL_ASYMMETRY_ALLOWED_PACKETIZATION_BIT]);
        if (pSession->mLinesTransceiverCount <
            PEER_CONNECTION_TRANSCEIVER_MAX_COUNT) {
          pSession->pMLinesTransceivers[pSession->mLinesTransceiverCount++] =
              pSession->pTransceivers[currentTransceiverIdx];
        } else {
        ...
        *pTargetCodecPayload = PEER_CONNECTION_SDP_GET_APT_CODEC_FROM_PAYLOAD(
            codecPayloads[TRANSCEIVER_RTC_CODEC_VP8_BIT]);
        *pTargetCodecRtxPayload =
            PEER_CONNECTION_SDP_GET_RTX_CODEC_FROM_PAYLOAD(
                codecPayloads[TRANSCEIVER_RTC_CODEC_VP8_BIT]);
        if (pSession->mLinesTransceiverCount <
            PEER_CONNECTION_TRANSCEIVER_MAX_COUNT) {
          pSession->pMLinesTransceivers[pSession->mLinesTransceiverCount++] =
    ...
    if (*pIsTargetCodecPayloadSet == 1) {
      LogDebug(("Set payload type successfully, idx: %d, payload: 0x%x, RTX "
                "payload: 0x%x",
                currentTransceiverIdx, *pTargetCodecPayload,
                *pTargetCodecRtxPayload));
    } else {
      LogWarn(("Unable to set payload type, mediaCodecBitMap: 0x%x",
               *pCodecBitMap));
    }
  }
...
static PeerConnectionResult_t PopulateMediaDescriptions(
    PeerConnectionSession_t *pSession,
  ...
  memset(&populateConfiguration, 0,
  ...
  populateConfiguration.pLocalFingerprint =
      pSession->pCtx->dtlsContext.localCertFingerprint;
  ...
  if (pRemoteBufferSessionDescription == NULL) {
    ...
    for (i = 0; i < pSession->transceiverCount; i++) {
      ...
      if (populateConfiguration.pTransceiver->trackKind ==
        ...
        populateConfiguration.rtxPayloadType =
            pSession->rtpConfig.videoCodecRtxPayload;
      } else {
        populateConfiguration.payloadType =
            pSession->rtpConfig.audioCodecPayload;
        populateConfiguration.rtxPayloadType =
            pSession->rtpConfig.audioCodecRtxPayload;
      }

      retSdpController = SdpController_PopulateSingleMedia(
          NULL, populateConfiguration,
          &pLocalBufferSessionDescription->sdpDescription.mediaDescriptions[i],
          i, ppBuffer, pBufferLength,
          populateConfiguration.pTransceiver->trackKind);
      if (retSdpController != SDP_CONTROLLER_RESULT_OK) {
        LogError(("Fail to populate single media description, result: %d",
                  retSdpController));
...
#if ENABLE_SCTP_DATA_CHANNEL
    if ((ret == PEER_CONNECTION_RESULT_OK) &&
        (pSession->ucEnableDataChannelLocal != 0)) {
      if (i < SDP_CONTROLLER_MAX_SDP_MEDIA_DESCRIPTIONS_COUNT) {
        ...
        retSdpController = SdpController_PopulateSingleMedia(
            ...
            &pLocalBufferSessionDescription->sdpDescription
                 .mediaDescriptions[i],
            i, ppBuffer, pBufferLength, TRANSCEIVER_TRACK_KIND_DATA_CHANNEL);
        if (retSdpController != SDP_CONTROLLER_RESULT_OK) {
          LogError(("Fail to populate single media data channel description, "
                    "result: %d",
                    retSdpController));
          ret =
        ...
        LogError(("Reached media description limit, failed to populate data "
                  "channel description, index: %d",
                  i));
      }
    }
#endif /* ENABLE_SCTP_DATA_CHANNEL */
    ...
    populateConfiguration.twccExtId =
        pSession->remoteSessionDescription.sdpDescription.quickAccess.twccExtId;
    ...
    if ((ret == PEER_CONNECTION_RESULT_OK) &&
        (pSession->ucEnableDataChannelRemote == 1) &&
        (i < SDP_CONTROLLER_MAX_SDP_MEDIA_DESCRIPTIONS_COUNT)) {
      populateConfiguration.pTransceiver = NULL;
      retSdpController = SdpController_PopulateSingleMedia(
          &pRemoteBufferSessionDescription->sdpDescription.mediaDescriptions[i],
          populateConfiguration,
...
static PeerConnectionResult_t PopulateSessionDescription(
    PeerConnectionSession_t *pSession,
    PeerConnectionBufferSessionDescription_t *pRemoteBufferSessionDescription,
    PeerConnectionBufferSessionDescription_t *pLocalBufferSessionDescription,
    char **ppBuffer, size_t *pBufferLength) {
  ...
  if (pRemoteBufferSessionDescription == NULL) {
    populateConfiguration.isOffer = 1U;
    retSdpController = SdpController_PopulateSessionDescription(
        NULL, populateConfiguration,
        &pLocalBufferSessionDescription->sdpDescription, ppBuffer,
        pBufferLength);
  } else {
    retSdpController = SdpController_PopulateSessionDescription(
        &pRemoteBufferSessionDescription->sdpDescription, populateConfiguration,
        &pLocalBufferSessionDescription->sdpDescription, ppBuffer,
        pBufferLength);
  }
  if (retSdpController != SDP_CONTROLLER_RESULT_OK) {
    LogWarn(
        ("Fail to populate session description, result: %d", retSdpController));
...
static void SetReceiverSsrc(
    PeerConnectionBufferSessionDescription_t *pBufferSessionDescription) {
  int i, j;
  ...
  for (i = 0; i < pBufferSessionDescription->sdpDescription.mediaCount; i++) {
    if (pBufferSessionDescription->sdpDescription.mediaDescriptions[i]
            .mediaNameLength < 5) {
      if (pBufferSessionDescription->sdpDescription.mediaDescriptions[i]
              .mediaNameLength > 0) {
        LogWarn(
            ("The media name is not known source, media name: %.*s",
             (int)pBufferSessionDescription->sdpDescription.mediaDescriptions[i]
                 .mediaNameLength,
             pBufferSessionDescription->sdpDescription.mediaDescriptions[i]
                 .pMediaName));
      } else {
    ...
    isVideoDescription =
        strncmp(pBufferSessionDescription->sdpDescription.mediaDescriptions[i]
                    .pMediaName,
                "video", 5) == 0
            ? 1U
            : 0U;
    ...
    if ((isVideoDescription == 0U) && (isAudioDescription == 0U)) {
      ...
      pMediaSsrc =
          &pBufferSessionDescription->sdpDescription.quickAccess.videoSsrc;
    } else {
      pMediaSsrc =
          &pBufferSessionDescription->sdpDescription.quickAccess.audioSsrc;
    }

    pMediaDescription =
        &pBufferSessionDescription->sdpDescription.mediaDescriptions[i];
    for (j = 0; j < pMediaDescription->mediaAttributesCount; j++) {
      if ((pMediaDescription->attributes[j].attributeNameLength ==
           strlen("ssrc")) &&
          (strncmp(pMediaDescription->attributes[j].pAttributeName, "ssrc",
                   strlen("ssrc")) == 0)) {
        ...
        stringResult = StringUtils_ConvertStringToUl(
            pMediaDescription->attributes[j].pAttributeValue,
            pMediaDescription->attributes[j].attributeValueLength, pMediaSsrc);
        if (stringResult != STRING_UTILS_RESULT_OK) {
...
PeerConnectionResult_t PeerConnectionSdp_DeserializeSdpMessage(
    PeerConnectionBufferSessionDescription_t *pBufferSessionDescription) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  SdpControllerResult_t retSdpController;

  if (pBufferSessionDescription == NULL) {
    LogError(("Invalid input, pBufferSessionDescription: %p",
...
PeerConnectionResult_t PeerConnectionSdp_SetPayloadTypes(
    PeerConnectionSession_t *pSession,
    PeerConnectionBufferSessionDescription_t *pRemoteBufferSessionDescription) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  int i;
  ...
  if ((pSession == NULL) || (pRemoteBufferSessionDescription == NULL)) {
    LogError(
        ("Invalid input, pSession: %p, pRemoteBufferSessionDescription: %p",
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    if (pRemoteBufferSessionDescription->type ==
        SDP_CONTROLLER_MESSAGE_TYPE_OFFER) {
      /* If it's SDP offer from remote peer, we should set the codec based on
       * remote codec set. */
      for (i = 0;
           i < pRemoteBufferSessionDescription->sdpDescription.mediaCount;
           i++) {
        ret = GetPayloadTypesFromMedia(
            &pRemoteBufferSessionDescription->sdpDescription
                 .mediaDescriptions[i],
            &remoteMediaCodecBitMap, remoteCodecPayloads);
        if (ret != PEER_CONNECTION_RESULT_OK) {
...
PeerConnectionResult_t PeerConnectionSdp_PopulateSessionDescription(
    PeerConnectionSession_t *pSession,
    PeerConnectionBufferSessionDescription_t *pRemoteBufferSessionDescription,
    PeerConnectionBufferSessionDescription_t *pLocalBufferSessionDescription,
  ...
  if ((pSession == NULL) || (pLocalBufferSessionDescription == NULL) ||
      (pOutputSerializedSdpMessage == NULL) ||
      (pOutputSerializedSdpMessageLength == NULL)) {
    LogError(("Invalid input, pSession: %p, pLocalBufferSessionDescription: "
              "%p, pOutputSerializedSdpMessage: %p, "
              "pOutputSerializedSdpMessageLength: %p",
              pSession, pLocalBufferSessionDescription,
    ...
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  } else if ((pRemoteBufferSessionDescription != NULL) &&
             (pRemoteBufferSessionDescription->pSdpBuffer == NULL)) {
    LogError(("Invalid input, pRemoteBufferSessionDescription->pSdpBuffer: %p",
              pRemoteBufferSessionDescription->pSdpBuffer));
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  } else {
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    /* Add media descriptions, use the temp buffer to store SDP content for
    ...
    ret = PopulateMediaDescriptions(pSession, pRemoteBufferSessionDescription,
                                    pLocalBufferSessionDescription, &pBuffer,
                                    &bufferLength);
  }

  if (ret == PEER_CONNECTION_RESULT_OK) {
    /* Add session descriptions.
     * Note that we need to session media count to populate session group
     * attribute, so this have to do after populate media sessions. */
    ret = PopulateSessionDescription(pSession, pRemoteBufferSessionDescription,
                                     pLocalBufferSessionDescription, &pBuffer,
                                     &bufferLength);
  }
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    retSdpController = SdpController_SerializeSdpMessageByDescription(
        pLocalBufferSessionDescription->type,
        &pLocalBufferSessionDescription->sdpDescription, pBuffer,
        &bufferLength);
    if (retSdpController != SDP_CONTROLLER_RESULT_OK) {
      LogWarn(("Fail to serialize session description, result: %d",
               retSdpController));
      ret = PEER_CONNECTION_RESULT_FAIL_SDP_POPULATE_SESSION_DESCRIPTION;
    } else {
