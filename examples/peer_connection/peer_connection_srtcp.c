...
#define PEER_CONNECTION_SRTCP_MID_NTP(currentTimeNTP)                          \
  (uint32_t)((currentTimeNTP >> 16U) & 0xffffffffULL)

/*-----------------------------------------------------------*/

static PeerConnectionResult_t
PeerConnectionSrtcp_MatchRemoteBySsrc(PeerConnectionSession_t *pSession,
                                      uint32_t ssrc) {
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    if ((ssrc != pSession->rtpConfig.remoteAudioSsrc) &&
        (ssrc != pSession->rtpConfig.remoteVideoSsrc)) {
...
static PeerConnectionResult_t
ResendSrtpPacket(PeerConnectionSession_t *pSession,
                 const Transceiver_t *pTransceiver, uint16_t rtpSeq,
                 uint32_t ssrc) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ret = PeerConnectionRollingBuffer_SearchRtpSequenceBuffer(
        &pSrtpSender->txRollingBuffer, rtpSeq, &pRollingBufferPacket);

    if ((ret != PEER_CONNECTION_RESULT_OK) || (pRollingBufferPacket == NULL)) {
      LogWarn(("Fail to find target buffer, seq: %u", rtpSeq));
    } else {
      LogDebug(("Found target buffer, pRollingBufferPacket: %p, "
                "packetBufferLength: %lu, pPacketBuffer: %p",
                pRollingBufferPacket, pRollingBufferPacket->packetBufferLength,
                pRollingBufferPacket->pPacketBuffer));
      LogDebug(
          ("Found target buffer, sequence in buffer: %u, target sequence: %u",
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    if (bufferAfterEncrypt == 0) {
      ...
      /* Follow RTX format to add OSN(original RTP sequence number) at the very
       * beginning of payload. Note that we reserve
       * PEER_CONNECTION_SRTP_RTX_WRITE_RESERVED_BYTES at the beginning of
       * buffer at write frame. */
      pOsn = (uint16_t *)pRollingBufferPacket->pPacketBuffer;
      *pOsn = htons(rtpSeq);
      pRollingBufferPacket->rtpPacket.payloadLength =
          pRollingBufferPacket->packetBufferLength + 2;
      pRollingBufferPacket->rtpPacket.pPayload =
          pRollingBufferPacket->pPacketBuffer;
      ...
      ret = PeerConnectionSrtp_ConstructSrtpPacket(
          pSession, &pRollingBufferPacket->rtpPacket, pSrtpPacket,
          &srtpPacketLength);
    } else {
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    resultIceController = IceController_SendToRemotePeer(
        &pSession->iceControllerContext, pSrtpPacket, srtpPacketLength);

    if (resultIceController != ICE_CONTROLLER_RESULT_OK) {
      LogWarn(("Fail to re-send RTP packet, ret: %d, seq: %u, SSRC: 0x%x",
               resultIceController, rtpSeq, ssrc));
      ret = PEER_CONNECTION_RESULT_FAIL_ICE_CONTROLLER_RESEND_RTP_PACKET;
    } else {
      LogDebug(
          ("Re-send RTP successfully, RTP seq: %u, SSRC: 0x%x", rtpSeq, ssrc));
    }
  }
...
static PeerConnectionResult_t OnRtcpFirEvent(PeerConnectionSession_t *pSession,
  ...
  if ((pSession == NULL) || (pRtcpPacket == NULL)) {
    LogError(("Invalid input, pSession: %p, pRtcpPacket: %p", pSession,
              pRtcpPacket));
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  }

  if (ret == PEER_CONNECTION_RESULT_OK) {
    memset(&firPacket, 0, sizeof(RtcpFirPacket_t));
    resultRtcp = Rtcp_ParseFirPacket(&pSession->pCtx->rtcpContext, pRtcpPacket,
                                     &firPacket);
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ret = PeerConnection_MatchTransceiverBySsrc(pSession, firPacket.senderSsrc,
                                                &pTransceiver);
  }
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    /* Initiate a callback to transmit an intra-picture to achieve
     * resynchronization. */
  } else if (ret == PEER_CONNECTION_RESULT_UNKNOWN_SSRC) {
    LogWarn(("Received FIR for non existing ssrc: %u", firPacket.senderSsrc));
  } else {
    /* Do Nothing */
  }
...
static PeerConnectionResult_t OnRtcpNackEvent(PeerConnectionSession_t *pSession,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    resultRtcp = Rtcp_ParseNackPacket(&pSession->pCtx->rtcpContext, pRtcpPacket,
                                      &nackPacket);
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ret = PeerConnection_MatchTransceiverBySsrc(
        pSession, nackPacket.mediaSourceSsrc, &pTransceiver);
  }
...
static PeerConnectionResult_t OnRtcpTwccEvent(PeerConnectionSession_t *pSession,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    memset(&twccPacket, 0, sizeof(RtcpTwccPacket_t));
    memset(&packetArrivalInfo[0], 0,
    ...
    resultRtcp = Rtcp_ParseTwccPacket(&pSession->pCtx->rtcpContext, pRtcpPacket,
                                      &twccPacket);
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    for (i = 0; i < twccPacket.arrivalInfoListLength; i++) {
      ...
      resultRtcpTwccManager = RtcpTwccManager_FindPacketInfo(
          &pSession->pCtx->rtcpTwccManager,
          twccPacket.pArrivalInfoList[i].seqNum, &pTwccPacketInfo);

      if (resultRtcpTwccManager == RTCP_TWCC_MANAGER_RESULT_OK) {
        pTwccPacketInfo->localSentTime =
            twccPacket.pArrivalInfoList[i].remoteArrivalTime;
      }
    }
  }

  if (ret == PEER_CONNECTION_RESULT_OK) {
    resultRtcpTwccManager = RtcpTwccManager_HandleTwccPacket(
        &pSession->pCtx->rtcpTwccManager, &twccPacket, &twccBandwidthInfo);

    if (resultRtcpTwccManager != RTCP_TWCC_MANAGER_RESULT_OK) {
      LogError(("Fail to handle RTCP TWCC packet, result: %d",
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    if ((twccBandwidthInfo.duration > 0) &&
        (pSession->pCtx->onBandwidthEstimationCallback != NULL)) {
      /* Call the bandwidth estimation callback */
      pSession->pCtx->onBandwidthEstimationCallback(
          pSession->pCtx->pOnBandwidthEstimationCallbackContext,
          &twccBandwidthInfo);
    }

    LogDebug(("TWCC Bandwidth Info : SentBytes - %lu, ReceivedBytes - %lu, "
              "SentPackets - %lu, ReceivedPackets - %lu, Duration - %ld",
              twccBandwidthInfo.sentBytes, twccBandwidthInfo.receivedBytes,
              twccBandwidthInfo.sentPackets, twccBandwidthInfo.receivedPackets,
...
static PeerConnectionResult_t OnRtcpPliEvent(PeerConnectionSession_t *pSession,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    memset(&pliPacket, 0, sizeof(RtcpPliPacket_t));
    resultRtcp = Rtcp_ParsePliPacket(&pSession->pCtx->rtcpContext, pRtcpPacket,
                                     &pliPacket);
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ret = PeerConnection_MatchTransceiverBySsrc(
        pSession, pliPacket.mediaSourceSsrc, &pTransceiver);
  }

  if (ret == PEER_CONNECTION_RESULT_OK) {
    /* Call the PLI callback if registered */
    if (pSession->onPictureLossIndicationCallback != NULL) {
      pSession->onPictureLossIndicationCallback(
          pSession->pPictureLossIndicationUserContext, &pliPacket);
    }
  } else if (ret == PEER_CONNECTION_RESULT_UNKNOWN_SSRC) {
    LogError(
...
static PeerConnectionResult_t OnRtcpSliEvent(PeerConnectionSession_t *pSession,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    memset(&sliPacket, 0, sizeof(RtcpSliPacket_t));
    resultRtcp = Rtcp_ParseSliPacket(&pSession->pCtx->rtcpContext, pRtcpPacket,
                                     &sliPacket);
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ret = PeerConnection_MatchTransceiverBySsrc(
        pSession, sliPacket.mediaSourceSsrc, &pTransceiver);
  }
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    LogWarn(
        ("Received SLI for non existing ssrc: %u", sliPacket.mediaSourceSsrc));
...
static PeerConnectionResult_t OnRtcpRembEvent(PeerConnectionSession_t *pSession,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    resultRtcp = Rtcp_ParseRembPacket(&pSession->pCtx->rtcpContext, pRtcpPacket,
                                      &rembPacket);
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    for (i = 0; i < rembPacket.ssrcListLength; i++) {
      ret = PeerConnection_MatchTransceiverBySsrc(
          pSession, rembPacket.pSsrcList[i], &pTransceiver);

      if (ret == PEER_CONNECTION_RESULT_OK) {
        /* Initiate a callback to update the estimated maximum bitrate to adjust
         * sending rate based on receiver's feedback. */
      } else if (ret == PEER_CONNECTION_RESULT_UNKNOWN_SSRC) {
        LogWarn(("Received REMB for non existing ssrc: %u",
...
/* TODO better sender report handling
 * https://tools.ietf.org/html/rfc3550#section-6.4.1 */
static PeerConnectionResult_t
OnRtcpSenderReportEvent(PeerConnectionSession_t *pSession,
                        RtcpPacket_t *pRtcpPacket) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  RtcpResult_t resultRtcp;
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    resultRtcp = Rtcp_ParseSenderReport(&pSession->pCtx->rtcpContext,
                                        pRtcpPacket, &senderReport);

    if (resultRtcp != RTCP_RESULT_OK) {
      LogError(
          ("Fail to parse RTCP SENDER REPORT packet, result: %d", resultRtcp));
      ret = PEER_CONNECTION_RESULT_FAIL_RTCP_PARSE_SENDER_REPORT;
    } else if (pRtcpPacket->payloadLength !=
               RTCP_SENDER_REPORT_MIN_PAYLOAD_LENGTH) {
      /* TODO: handle sender report containing receiver report blocks */
      LogWarn(("Received Sender report with containing receiver report blocks, "
               "not supported yet."));

      /* Based on the RFC 3550 document, SR (Sender Report) receives report
       * blocks when a sender is also receiving RTP data from other participants
       * in the session. Since Currently the Master isn't parsing RTP data
       * packets from multiple sources (multiple SSRCs) since the last report.
       * In case this log gets printed we can encounter that even when the
       * funcionality is not added, we are getting report blocks, so something
       * is wrong. */
    } else {
      /* Do Nothing */
    }
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    LogVerbose(("RTCP_PACKET_SENDER_REPORT, SSRC: %u, NTP Time %lu  RTP Time: "
                "%u, PacketCount: %u, OctetCount: %u",
                senderReport.senderSsrc, senderReport.senderInfo.ntpTime,
    ...
    if (ret == PEER_CONNECTION_RESULT_UNKNOWN_SSRC) {
      LogWarn(("Received sender report for non existing ssrc: %u",
               senderReport.senderSsrc));
      ret = PEER_CONNECTION_RESULT_OK;
    }
  }
...
static PeerConnectionResult_t
OnRtcpReceiverReportEvent(PeerConnectionSession_t *pSession,
                          RtcpPacket_t *pRtcpPacket) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  RtcpResult_t resultRtcp;
  RtcpReceiverReport_t receiverReport;
  RtcpReceptionReport_t receptionReport
      [PEER_CONNECTION_RTCP_RECEIVER_REPORT_RECEPTION_REPORT_NUM];
  const Transceiver_t *pTransceiver = NULL;
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    memset(&receiverReport, 0, sizeof(RtcpReceiverReport_t));
    memset(&receptionReport[0], 0,
           sizeof(RtcpReceptionReport_t) *
               PEER_CONNECTION_RTCP_RECEIVER_REPORT_RECEPTION_REPORT_NUM);

    receiverReport.pReceptionReports = receptionReport;
    receiverReport.numReceptionReports =
        PEER_CONNECTION_RTCP_RECEIVER_REPORT_RECEPTION_REPORT_NUM;

    resultRtcp = Rtcp_ParseReceiverReport(&pSession->pCtx->rtcpContext,
                                          pRtcpPacket, &receiverReport);

    if (resultRtcp != RTCP_RESULT_OK) {
      LogError(("Fail to parse RTCP RECEIVER REPORT packet, result: %d",
                resultRtcp));
    ...
    if (receiverReport.numReceptionReports > 2) {
      ...
      LogWarn(("Received receiver report with multiple reception reports, not "
               "supported yet."));

      /* Since Currently the Master isn't parsing RTP data packets from multiple
       * sources (multiple SSRCs) since the last report. Each reception report
       * block provides statistics about the data received from a particular
       * source. */
    }
  }
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    for (i = 0; i < receiverReport.numReceptionReports; i++) {
      ret = PeerConnection_MatchTransceiverBySsrc(
          pSession, receiverReport.pReceptionReports[0].sourceSsrc,
          &pTransceiver);

      LogDebug(
          ("RTCP_PACKET_TYPE_RECEIVER_REPORT, sender SSRC: %u, source SSRC: "
           "%u, fraction loss: %u, cumulative loss: %u, highest seq: %u, jit: "
           "%u, lsr: %u, dlsr: %u",
      ...
      if (ret == PEER_CONNECTION_RESULT_UNKNOWN_SSRC) {
        LogWarn(("Received receiver report for non existing ssrc: %u",
                 receiverReport.pReceptionReports[0].sourceSsrc));
        ret = PEER_CONNECTION_RESULT_OK;
        continue;
      }
      ...
      if ((ret == PEER_CONNECTION_RESULT_OK) &&
        ...
        /*      SSRC_r by recording the time A when this reception report block
         * is */
        /*      received.  It calculates the total round-trip time A-LSR using
         * the */
        /*      last SR timestamp (LSR) field, and then subtracting this field
         * to */
        /*      leave the round-trip propagation delay as (A - LSR - DLSR). */
        currentTimeNTP = NetworkingUtils_GetNTPTimeFromUnixTimeUs(
            NetworkingUtils_GetCurrentTimeUs(NULL));
        currentTimeNTP = PEER_CONNECTION_SRTCP_MID_NTP(currentTimeNTP);
        roundTripPropagationDelay =
            currentTimeNTP - receiverReport.pReceptionReports[0].lastSR -
            receiverReport.pReceptionReports[0].delaySinceLastSR;
        roundTripPropagationDelay =
            (roundTripPropagationDelay * 1000) /
            PEER_CONNECTION_SRTCP_DLSR_TIMESCALE; /* The Round Trip Propogation
                                                     Delay is in ms unit. */

        if (pTransceiver->trackKind == TRANSCEIVER_TRACK_KIND_AUDIO) {
          LogVerbose(("RTCP_PACKET_TYPE_RECEIVER_REPORT Round Trip Propagation "
                      "Delay for Audio : %u ms",
                      roundTripPropagationDelay));
        } else if (pTransceiver->trackKind == TRANSCEIVER_TRACK_KIND_VIDEO) {
          LogVerbose(("RTCP_PACKET_TYPE_RECEIVER_REPORT Round Trip Propagation "
                      "Delay for Video : %u ms",
                      roundTripPropagationDelay));
        }
      }
    }
  ...
  /* Update stats if like reportsReceived counter, roundTripPropagationDelay,
   * fraction Lost , roundTripTimeMeasurements counter  */

  return ret;
}

PeerConnectionResult_t PeerConnectionSrtcp_ConstructSenderReportPacket(
    PeerConnectionSession_t *pSession, RtcpSenderReport_t *pSenderReport,
    uint8_t *pOutputSrtcpPacket, size_t *pOutputSrtcpPacketLength) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  RtcpResult_t resultRtcp;
  ...
  if ((pSession == NULL) || (pSenderReport == NULL) ||
      (pOutputSrtcpPacket == NULL) || (pOutputSrtcpPacketLength == NULL)) {
    LogError(("Invalid input, pSession: %p, pSenderReport: %p, "
              "pOutputSrtcpPacket: %p, pOutputSrtcpPacketLength: %p",
              pSession, pSenderReport, pOutputSrtcpPacket,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    resultRtcp =
        Rtcp_SerializeSenderReport(&pSession->pCtx->rtcpContext, pSenderReport,
                                   pOutputSrtcpPacket, &rtcpBufferLength);

    if (resultRtcp != RTCP_RESULT_OK) {
      LogError(
          ("Fail to serialize RTCP Sender Report, result: %d", resultRtcp));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    if (pSession->srtpTransmitSession != NULL) {
      errorStatus = srtp_protect_rtcp(
          pSession->srtpTransmitSession, pOutputSrtcpPacket, rtcpBufferLength,
          pOutputSrtcpPacket, pOutputSrtcpPacketLength, 0);
      if (errorStatus != srtp_err_status_ok) {
        LogError(
            ("Fail to encrypt Tx SRTCP packet, errorStatus: %d", errorStatus));
        ret = PEER_CONNECTION_RESULT_FAIL_ENCRYPT_SRTP_RTCP_PACKET;
      }
    } else {
      LogError(("SRTP session has been freed before encrypting."));
      ret = PEER_CONNECTION_RESULT_FAIL_ENCRYPT_SRTP_RTCP_PACKET;
    }
  }
...
PeerConnectionSrtp_HandleSrtcpPacket(PeerConnectionSession_t *pSession,
                                     uint8_t *pBuffer, size_t bufferLength) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  srtp_err_status_t errorStatus;
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    if (pthread_mutex_lock(&(pSession->srtpSessionMutex)) == 0) {
      ...
      LogError(("Fail to take SRTP session mutex to decrypt SRTCP packet."));
      ret = PEER_CONNECTION_RESULT_FAIL_TAKE_SRTP_MUTEX;
    }
  }
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    if (pSession->srtpReceiveSession != NULL) {
      errorStatus =
          srtp_unprotect_rtcp(pSession->srtpReceiveSession, pBuffer,
                              bufferLength, rtcpBuffer, &rtcpBufferLength);
      if (errorStatus != srtp_err_status_ok) {
        LogError(
            ("Fail to decrypt Rx SRTCP packet, errorStatus: %d", errorStatus));
        ret = PEER_CONNECTION_RESULT_FAIL_DECRYPT_SRTP_RTP_PACKET;
      } else {
        remainingLength = rtcpBufferLength;
        LogVerbose(("Decrypt SRTCP packet successfully, decrypted length: %lu",
                    rtcpBufferLength));
      }
    } else {
      LogError(("SRTP session has been freed before decrypting."));
      ret = PEER_CONNECTION_RESULT_FAIL_DECRYPT_SRTP_RTP_PACKET;
    }
  }
  ...
  while ((remainingLength >= RTCP_HEADER_LENGTH) &&
         (ret == PEER_CONNECTION_RESULT_OK)) {
    resultRtcp =
        Rtcp_DeserializePacket(&pSession->pCtx->rtcpContext, currentPacket,
                               remainingLength, &rtcpPacket);
    ...
    if (ret == PEER_CONNECTION_RESULT_OK) {
      ...
      case RTCP_PACKET_FIR:
        ret = OnRtcpFirEvent(pSession, &rtcpPacket);
        break;
...
#if ENABLE_TWCC_SUPPORT
        ret = OnRtcpTwccEvent(pSession, &rtcpPacket);
#endif
        break;

      case RTCP_PACKET_PAYLOAD_FEEDBACK_PLI:
        ret = OnRtcpPliEvent(pSession, &rtcpPacket);
        break;

      case RTCP_PACKET_PAYLOAD_FEEDBACK_SLI:
        ret = OnRtcpSliEvent(pSession, &rtcpPacket);
        break;

      case RTCP_PACKET_PAYLOAD_FEEDBACK_REMB:
        ret = OnRtcpRembEvent(pSession, &rtcpPacket);
        break;

      case RTCP_PACKET_SENDER_REPORT:
        ret = OnRtcpSenderReportEvent(pSession, &rtcpPacket);
        break;

      case RTCP_PACKET_RECEIVER_REPORT:
        ret = OnRtcpReceiverReportEvent(pSession, &rtcpPacket);
        break;
