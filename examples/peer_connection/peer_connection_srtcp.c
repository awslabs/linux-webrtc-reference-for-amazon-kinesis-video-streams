                                              ...
                                              RtcpPacket_t * pRtcpPacket )
{
    ...
    {
        LogError( ( "Received PLI for non existing ssrc: %u", pliPacket.mediaSourceSsrc ) );
    }
    else
    {
        /* Do Nothing */
                                              ...
                                              RtcpPacket_t * pRtcpPacket )
{
    ...
    {
        LogWarn( ( "Received SLI for non existing ssrc: %u", sliPacket.mediaSourceSsrc ) );
                                               ...
                                               RtcpPacket_t * pRtcpPacket )
{
    ...
    if( ret == PEER_CONNECTION_RESULT_OK )
    {
        for( i = 0; i < rembPacket.ssrcListLength; i++ )
        {
            ...
            {
                LogWarn( ( "Received REMB for non existing ssrc: %u", rembPacket.pSsrcList[ i ] ) );
            }
            else
            {
                /* Do Nothing */
                                                       ...
                                                       RtcpPacket_t * pRtcpPacket )
{
    ...
    if( ret == PEER_CONNECTION_RESULT_OK )
    {
        ...
        {
            LogWarn( ( "Received sender report for non existing ssrc: %u", senderReport.senderSsrc ) );
            ret = PEER_CONNECTION_RESULT_OK;
        }
    }
                                                         ...
                                                         RtcpPacket_t * pRtcpPacket )
{
    ...
    if( ret == PEER_CONNECTION_RESULT_OK )
    {
        for( i = 0; i < receiverReport.numReceptionReports; i++ )
        {
            ...
            if( ret == PEER_CONNECTION_RESULT_UNKNOWN_SSRC )
            {
                LogWarn( ( "Received receiver report for non existing ssrc: %u", receiverReport.pReceptionReports[ 0 ].sourceSsrc ) );
                ret = PEER_CONNECTION_RESULT_OK;
                continue;
            }
