...
#define PEER_CONNECTION_SRTP_CONVERT_TIME_US_TO_RTP_TIMESTAMP( \
    clockRate, presentationUs )                                \
    ( uint32_t )( ( ( ( presentationUs ) * ( clockRate ) ) /   \
                      PEER_CONNECTION_SRTP_US_IN_A_SECOND ) &  \
                  0xFFFFFFFF )
#define PEER_CONNECTION_SRTP_CONVERT_RTP_TIMESTAMP_TO_TIME_US(             \
    clockRate, rtpTimestamp )                                              \
    ( ( uint64_t ) ( rtpTimestamp ) *PEER_CONNECTION_SRTP_US_IN_A_SECOND / \
        ( clockRate ) )
...
#define PEER_CONNECTION_SRTP_GET_TWCC_PAYLOAD( extId, sequenceNum ) \
    ( ( ( ( extId ) &0xfu ) << 28u ) | ( 1u << 24u ) |              \
        ( ( uint32_t ) ( sequenceNum ) << 8u ) )
