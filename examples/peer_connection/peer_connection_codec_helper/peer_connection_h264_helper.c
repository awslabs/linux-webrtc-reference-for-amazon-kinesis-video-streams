...
GetH264PacketProperty(PeerConnectionJitterBufferPacket_t *pPacket,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    resultH264 = H264Depacketizer_GetPacketProperties(
...
PeerConnectionResult_t
FillFrameH264(PeerConnectionJitterBuffer_t *pJitterBuffer, uint16_t rtpSeqStart,
  ...
  H264Packet_t
      h264Packets[PEER_CONNECTION_JITTER_BUFFER_MAX_PACKETS_NUM_IN_A_FRAME];
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    resultH264 = H264Depacketizer_Init(
        &h264DepacketizerContext, h264Packets,
        PEER_CONNECTION_JITTER_BUFFER_MAX_PACKETS_NUM_IN_A_FRAME);
    if (resultH264 != H264_RESULT_OK) {
      LogError(
          ("Fail to initialize H264 depacketizer, result: %d", resultH264));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    for (i = rtpSeqStart; i != rtpSeqEnd + 1; i++) {
      ...
      LogDebug(("Adding packet seq: %u, length: %lu, timestamp: %u", i,
                h264Packet.packetDataLength, rtpTimestamp));

      resultH264 =
          H264Depacketizer_AddPacket(&h264DepacketizerContext, &h264Packet);
      if (resultH264 != H264_RESULT_OK) {
        LogError(
            ("Fail to add H264 depacketizer packet, result: %d", resultH264));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    resultH264 = H264Depacketizer_GetFrame(&h264DepacketizerContext, &frame);
    if (resultH264 != H264_RESULT_OK) {
...
PeerConnectionResult_t
PeerConnectionSrtp_WriteH264Frame(PeerConnectionSession_t *pSession,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    resultH264 =
        H264Packetizer_Init(&h264PacketizerContext, nalusArray,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    resultH264 = H264Packetizer_AddFrame(&h264PacketizerContext, &h264Frame);
    if (resultH264 != H264_RESULT_OK) {
  ...
  while (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    resultH264 = H264Packetizer_GetPacket(&h264PacketizerContext, &packetH264);
