...
GetOpusPacketProperty(PeerConnectionJitterBufferPacket_t *pPacket,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    resultOpus = OpusDepacketizer_GetPacketProperties(
...
PeerConnectionResult_t
FillFrameOpus(PeerConnectionJitterBuffer_t *pJitterBuffer, uint16_t rtpSeqStart,
  ...
  OpusPacket_t
      opusPackets[PEER_CONNECTION_JITTER_BUFFER_MAX_PACKETS_NUM_IN_A_FRAME];
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    resultOpus = OpusDepacketizer_Init(
        &opusDepacketizerContext, opusPackets,
        PEER_CONNECTION_JITTER_BUFFER_MAX_PACKETS_NUM_IN_A_FRAME);
    if (resultOpus != OPUS_RESULT_OK) {
      LogError(
          ("Fail to initialize OPUS depacketizer, result: %d", resultOpus));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    for (i = rtpSeqStart; i != rtpSeqEnd + 1; i++) {
      ...
      LogDebug(("Adding packet seq: %u, length: %lu, timestamp: %u", i,
                opusPacket.packetDataLength, rtpTimestamp));

      resultOpus =
          OpusDepacketizer_AddPacket(&opusDepacketizerContext, &opusPacket);
      if (resultOpus != OPUS_RESULT_OK) {
        LogError(
            ("Fail to add OPUS depacketizer packet, result: %d", resultOpus));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    resultOpus = OpusDepacketizer_GetFrame(&opusDepacketizerContext, &frame);
    if (resultOpus != OPUS_RESULT_OK) {
...
PeerConnectionResult_t
PeerConnectionSrtp_WriteOpusFrame(PeerConnectionSession_t *pSession,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    resultOpus = OpusPacketizer_Init(&opusPacketizerContext, &opusFrame);
    if (resultOpus != OPUS_RESULT_OK) {
  ...
  while (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    if (bufferAfterEncrypt == 0) {
      packetOpus.pPacketData = pRollingBufferPacket->pPacketBuffer +
                               PEER_CONNECTION_SRTP_RTX_WRITE_RESERVED_BYTES;
      packetOpus.packetDataLength =
    ...
    resultOpus = OpusPacketizer_GetPacket(&opusPacketizerContext, &packetOpus);
    if (resultOpus == OPUS_RESULT_NO_MORE_PACKETS) {
      ...
      /* For Opus, typically each packet is complete, so we set the marker bit
       * for each packet */
      ...
      pRollingBufferPacket->rtpPacket.header.timestamp =
          PEER_CONNECTION_SRTP_CONVERT_TIME_US_TO_RTP_TIMESTAMP(
              PEER_CONNECTION_SRTP_OPUS_CLOCKRATE, pFrame->presentationUs);
