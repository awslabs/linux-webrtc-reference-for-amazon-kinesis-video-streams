...
GetG711PacketProperty(PeerConnectionJitterBufferPacket_t *pPacket,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    resultG711 = G711Depacketizer_GetPacketProperties(
...
PeerConnectionResult_t
FillFrameG711(PeerConnectionJitterBuffer_t *pJitterBuffer, uint16_t rtpSeqStart,
  ...
  G711Packet_t
      g711Packets[PEER_CONNECTION_JITTER_BUFFER_MAX_PACKETS_NUM_IN_A_FRAME];
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    resultG711 = G711Depacketizer_Init(
        &g711DepacketizerContext, g711Packets,
        PEER_CONNECTION_JITTER_BUFFER_MAX_PACKETS_NUM_IN_A_FRAME);
    if (resultG711 != G711_RESULT_OK) {
      LogError(
          ("Fail to initialize G711 depacketizer, result: %d", resultG711));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    for (i = rtpSeqStart; i != rtpSeqEnd + 1; i++) {
      ...
      LogDebug(("Adding packet seq: %u, length: %lu, timestamp: %u", i,
                g711Packet.packetDataLength, rtpTimestamp));

      resultG711 =
          G711Depacketizer_AddPacket(&g711DepacketizerContext, &g711Packet);
      if (resultG711 != G711_RESULT_OK) {
        LogError(
            ("Fail to add G711 depacketizer packet, result: %d", resultG711));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    resultG711 = G711Depacketizer_GetFrame(&g711DepacketizerContext, &frame);
    if (resultG711 != G711_RESULT_OK) {
...
PeerConnectionResult_t
PeerConnectionSrtp_WriteG711Frame(PeerConnectionSession_t *pSession,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    resultG711 = G711Packetizer_Init(&g711PacketizerContext, &g711Frame);
    if (resultG711 != G711_RESULT_OK) {
  ...
  while (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    if (bufferAfterEncrypt == 0) {
      packetG711.pPacketData = pRollingBufferPacket->pPacketBuffer +
                               PEER_CONNECTION_SRTP_RTX_WRITE_RESERVED_BYTES;
      packetG711.packetDataLength =
    ...
    resultG711 = G711Packetizer_GetPacket(&g711PacketizerContext, &packetG711);
    if (resultG711 == G711_RESULT_NO_MORE_PACKETS) {
      ...
      /* For G711, typically each packet is complete, so we set the marker bit
       * for each packet */
      ...
      pRollingBufferPacket->rtpPacket.header.timestamp =
          PEER_CONNECTION_SRTP_CONVERT_TIME_US_TO_RTP_TIMESTAMP(
              PEER_CONNECTION_SRTP_PCM_CLOCKRATE, pFrame->presentationUs);
