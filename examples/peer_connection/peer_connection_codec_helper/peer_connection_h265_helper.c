...
GetH265PacketProperty(PeerConnectionJitterBufferPacket_t *pPacket,
                      uint8_t *pIsStartPacket) {
  PeerConnectionResult_t ret = PEER_CONNECTION_RESULT_OK;
  H265Result_t resultH265;
  uint32_t properties = 0;

  if ((pPacket == NULL) || (pIsStartPacket == NULL)) {
    LogError(("Invalid input, pPacket: %p, pIsStartPacket: %p", pPacket,
              pIsStartPacket));
    ret = PEER_CONNECTION_RESULT_BAD_PARAMETER;
  }

  if (ret == PEER_CONNECTION_RESULT_OK) {
    resultH265 = H265Depacketizer_GetPacketProperties(
        pPacket->pPacketBuffer, pPacket->packetBufferLength, &properties);
...
PeerConnectionResult_t
FillFrameH265(PeerConnectionJitterBuffer_t *pJitterBuffer, uint16_t rtpSeqStart,
  ...
  H265Packet_t
      h265Packets[PEER_CONNECTION_JITTER_BUFFER_MAX_PACKETS_NUM_IN_A_FRAME];
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    resultH265 = H265Depacketizer_Init(
        &h265DepacketizerContext, h265Packets,
        PEER_CONNECTION_JITTER_BUFFER_MAX_PACKETS_NUM_IN_A_FRAME);
    if (resultH265 != H265_RESULT_OK) {
      LogError(
          ("Fail to initialize H265 depacketizer, result: %d", resultH265));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    for (i = rtpSeqStart; i != rtpSeqEnd + 1; i++) {
      ...
      LogDebug(("Adding packet seq: %u, length: %lu, timestamp: %u", i,
                h265Packet.packetDataLength, rtpTimestamp));

      resultH265 =
          H265Depacketizer_AddPacket(&h265DepacketizerContext, &h265Packet);
      if (resultH265 != H265_RESULT_OK) {
        LogError(
            ("Fail to add h265 depacketizer packet, result: %d", resultH265));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    resultH265 = H265Depacketizer_GetFrame(&h265DepacketizerContext, &frame);
    if (resultH265 != H265_RESULT_OK) {
...
PeerConnectionResult_t
PeerConnectionSrtp_WriteH265Frame(PeerConnectionSession_t *pSession,
  ...
  uint32_t randomRtpTimeoffset =
      0; // TODO : Spec required random rtp time offset ( current implementation
         // of KVS SDK )
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    resulth265 =
        H265Packetizer_Init(&h265PacketizerContext, nalusArray,
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    resulth265 = H265Packetizer_AddFrame(&h265PacketizerContext, &h265Frame);
    if (resulth265 != H265_RESULT_OK) {
      LogError(
          ("Fail to add frame in  h265 packetizer, result: %d", resulth265));
  ...
  while (ret == PEER_CONNECTION_RESULT_OK) {
     ...
     * PeerConnectionRollingBuffer_GetRtpSequenceBuffer() returns the buffer
     * with its size. If the bufferAfterEncrypt = 0, we store only RTP payload
     * to the buffer. If the bufferAfterEncrypt = 1, we store the encrypted SRTP
     * packet to the buffer. */
    pRollingBufferPacket = NULL;
    ret = PeerConnectionRollingBuffer_GetRtpSequenceBuffer(
        &pSrtpSender->txRollingBuffer, *pRtpSeq, &pRollingBufferPacket);
    if (ret != PEER_CONNECTION_RESULT_OK) {
    ...
    if (bufferAfterEncrypt == 0) {
      packeth265.pPacketData = pRollingBufferPacket->pPacketBuffer +
                               PEER_CONNECTION_SRTP_RTX_WRITE_RESERVED_BYTES;
      packeth265.packetDataLength =
    ...
    resulth265 = H265Packetizer_GetPacket(&h265PacketizerContext, &packeth265);

    if (resulth265 == H265_RESULT_NO_MORE_NALUS) {
      PeerConnectionRollingBuffer_DiscardRtpSequenceBuffer(
          &pSrtpSender->txRollingBuffer, pRollingBufferPacket);
      ...
      pRollingBufferPacket->rtpPacket.header.timestamp =
          PEER_CONNECTION_SRTP_CONVERT_TIME_US_TO_RTP_TIMESTAMP(
              PEER_CONNECTION_SRTP_VIDEO_CLOCKRATE, pFrame->presentationUs);

      if (pSession->rtpConfig.twccId > 0) {
        pRollingBufferPacket->rtpPacket.header.flags |=
            RTP_HEADER_FLAG_EXTENSION;
        pRollingBufferPacket->rtpPacket.header.extension.extensionProfile =
            PEER_CONNECTION_SRTP_TWCC_EXT_PROFILE;
        pRollingBufferPacket->rtpPacket.header.extension
            .extensionPayloadLength = 1;
        pRollingBufferPacket->twccExtensionPayload =
            PEER_CONNECTION_SRTP_GET_TWCC_PAYLOAD(
                pSession->rtpConfig.twccId, pSession->rtpConfig.twccSequence);
        pRollingBufferPacket->rtpPacket.header.extension.pExtensionPayload =
            &pRollingBufferPacket->twccExtensionPayload;
      ...
      /* PeerConnectionSrtp_ConstructSrtpPacket() serializes RTP packet and
       * encrypt it. */
    ...
    if (ret == PEER_CONNECTION_RESULT_OK) {
      ...
      ret = PeerConnectionRollingBuffer_SetPacket(
          &pSrtpSender->txRollingBuffer, (*pRtpSeq)++, pRollingBufferPacket);
    }
