...
static int32_t OnNewVideoSample(GstElement *sink, gpointer user_data) {
  int32_t ret = 0;
  ...
  if (ret == 0) {
    ...
    if (gst_buffer_map(pBuffer, &map, GST_MAP_READ)) {
      ...
      if (pVideoContext->pSourcesContext->onMediaSinkHookFunc) {
        LogVerbose(("Sending video frame: size=%u, ts=%lu", frame.size,
                    frame.timestampUs));
...
static void *VideoTx_Task(void *pParameter) {
  ...
  GstMediaSourceContext_t *pVideoContext =
      (GstMediaSourceContext_t *)pParameter;
  ...
  if (ret == 0) {
    ...
    g_signal_connect(pVideoContext->pAppsink, "new-sample",
                     G_CALLBACK(OnNewVideoSample), pVideoContext);
...
static int32_t OnNewAudioSample(GstElement *sink, gpointer user_data) {
  int32_t ret = 0;
  ...
  if (ret == 0) {
    ...
    if (gst_buffer_map(pBuffer, &map, GST_MAP_READ)) {
      ...
      if (pAudioContext->pSourcesContext->onMediaSinkHookFunc) {
        LogVerbose(("Sending audio frame: size=%u, ts=%lu", frame.size,
                    frame.timestampUs));
...
static void *AudioTx_Task(void *pParameter) {
  ...
  GstMediaSourceContext_t *pAudioContext =
      (GstMediaSourceContext_t *)pParameter;
  ...
  if (ret == 0) {
    // Connect to new-sample signal
    g_signal_connect(pAudioContext->pAppsink, "new-sample",
                     G_CALLBACK(OnNewAudioSample), pAudioContext);
...
static int32_t HandlePcEventCallback(void *pCustomContext,
  ...
  GstMediaSourceContext_t *pMediaSource =
      (GstMediaSourceContext_t *)pCustomContext;
  int32_t ret = 0;
  ...
  if (ret == 0) {
    ...
    case TRANSCEIVER_CB_EVENT_REMOTE_PEER_READY:
      ...
      change_state_ret = gst_element_set_state(
          pMediaSource->pSourcesContext->videoContext.pPipeline,
          GST_STATE_PLAYING);
...
static int32_t InitPipeline(GstMediaSourcesContext_t *pCtx) {
  ...
  if (ret == 0) {
    gchar *pPipeline_desc = g_strdup_printf(
        ...
        "video/"
        "x-h264,profile=constrained-baseline,stream-format=byte-stream,"
        "alignment=au ! "
        "h264parse config-interval=1 ! "
        "queue max-size-buffers=2 ! "
        "appsink name=vsink sync=true emit-signals=true max-buffers=1 "
        "drop=true "
        "autoaudiosrc ! "
        "queue leaky=2 max-size-buffers=400 ! audioconvert ! audioresample ! "
        "opusenc name=audioEncoder ! "
        "audio/x-opus,rate=48000,channels=2 ! appsink sync=TRUE "
        "emit-signals=TRUE max-buffers=1 drop=true name=asink ");

    GError *pError = NULL;
    pCtx->videoContext.pPipeline = gst_parse_launch(pPipeline_desc, &pError);
  ...
  if (ret == 0) {
    // Get video sink
    pCtx->videoContext.pAppsink =
        gst_bin_get_by_name(GST_BIN(pCtx->videoContext.pPipeline), "vsink");
  ...
  if (ret == 0) {
    // Get audio sink
    pCtx->audioContext.pAppsink =
        gst_bin_get_by_name(GST_BIN(pCtx->videoContext.pPipeline), "asink");
  ...
  if (ret == 0) {
    ...
    pCtx->audioContext.pEncoder = gst_bin_get_by_name(
        GST_BIN(pCtx->audioContext.pPipeline), "audioEncoder");
  ...
  if (ret == 0) {
    // Get encoder elements for bitrate control
    pCtx->videoContext.pEncoder = gst_bin_get_by_name(
        GST_BIN(pCtx->videoContext.pPipeline), "videoEncoder");
...
int32_t GstMediaSource_Cleanup(GstMediaSourcesContext_t *pCtx) {
  ...
  if (ret == 0) {
    ...
    if (pCtx->videoContext.pPipeline) {
      gst_element_set_state(pCtx->videoContext.pPipeline, GST_STATE_NULL);
...
int32_t GstMediaSource_Init(GstMediaSourcesContext_t *pCtx,
  ...
  if (ret == 0) {
    if (pthread_create(&videoTid, NULL, VideoTx_Task, &pCtx->videoContext) !=
        0) {
  ...
  if (ret == 0) {
    if (pthread_create(&audioTid, NULL, AudioTx_Task, &pCtx->audioContext) !=
        0) {
...
int32_t GstMediaSource_InitVideoTransceiver(GstMediaSourcesContext_t *pCtx,
  ...
  if (ret == 0) {
    ...
    TRANSCEIVER_ENABLE_CODEC(
        pVideoTransceiver->codecBitMap,
        TRANSCEIVER_RTC_CODEC_H264_PROFILE_42E01F_LEVEL_ASYMMETRY_ALLOWED_PACKETIZATION_BIT);

    pVideoTransceiver->rollingbufferDurationSec =
        DEFAULT_TRANSCEIVER_ROLLING_BUFFER_DURATION_SECOND;

    g_object_get(G_OBJECT(pCtx->videoContext.pEncoder), "bitrate", &bitrate,
                 NULL);
...
int32_t GstMediaSource_InitAudioTransceiver(GstMediaSourcesContext_t *pCtx,
  ...
  if ((pCtx == NULL) || (pAudioTransceiver == NULL)) {
    LogError(("Invalid input, pCtx: %p, pAudioTransceiver: %p", pCtx,
              pAudioTransceiver));
    ret = -1;
  }
  ...
  if (ret == 0) {
    ...
    pAudioTransceiver->rollingbufferDurationSec =
        DEFAULT_TRANSCEIVER_ROLLING_BUFFER_DURATION_SECOND;

    g_object_get(G_OBJECT(pCtx->audioContext.pEncoder), "bitrate", &bitrate,
                 NULL);
