...
/* Considering 1.4 Mbps for 720p (which is what our samples use). This is for
 * H.264. For H.265, we're using a lower bit rate of about 462 Kbps. The value
 * could be different for other codecs. */
...
static void *VideoTx_Task(void *pParameter) {
  AppMediaSourceContext_t *pVideoContext =
      (AppMediaSourceContext_t *)pParameter;
  WebrtcFrame_t frame;
  ...
  if (pVideoContext == NULL) {
    ...
    while (1) {
      ...
      if (pVideoContext->numReadyPeer != 0) {
        ...
        {
          pVideoContext->fileIndex =
              pVideoContext->fileIndex % NUMBER_OF_H265_FRAME_SAMPLE_FILES + 1;
          snprintf(filePath, MAX_PATH_LEN,
                   "./examples/app_media_source/samples/h265SampleFrames/"
                   "frame-%04d.h265",
                   pVideoContext->fileIndex);
        }
#else
        {
          pVideoContext->fileIndex =
              pVideoContext->fileIndex % NUMBER_OF_H264_FRAME_SAMPLE_FILES + 1;
          snprintf(filePath, MAX_PATH_LEN,
                   "./examples/app_media_source/samples/h264SampleFrames/"
                   "frame-%04d.h264",
                   pVideoContext->fileIndex);
        }
#endif
...
static void *AudioTx_Task(void *pParameter) {
  AppMediaSourceContext_t *pAudioContext =
      (AppMediaSourceContext_t *)pParameter;
  WebrtcFrame_t frame;
#ifndef ENABLE_STREAMING_LOOPBACK
  char filePath[MAX_PATH_LEN + 1];
  ...
  if (pAudioContext == NULL) {
    ...
    while (1) {
      ...
      if (pAudioContext->numReadyPeer != 0) {
        pAudioContext->fileIndex =
            pAudioContext->fileIndex % NUMBER_OF_OPUS_FRAME_SAMPLE_FILES + 1;
        snprintf(filePath, MAX_PATH_LEN,
                 "./examples/app_media_source/samples/opusSampleFrames/"
                 "sample-%03d.opus",
                 pAudioContext->fileIndex);
...
static int32_t OnPcEventRemotePeerReady(AppMediaSourceContext_t *pMediaSource) {
  ...
  if (ret == 0) {
    if (pthread_mutex_lock(&(pMediaSource->pSourcesContext->mediaMutex)) == 0) {
      ...
      LogInfo(("Starting track kind(%d) media, value=%u",
               pMediaSource->trackKind, pMediaSource->numReadyPeer));
    } else {
      LogError(("Failed to lock media mutex, track kind=%d.",
                pMediaSource->trackKind));
      ret = -1;
    }
  }
...
static int32_t
OnPcEventRemotePeerClosed(AppMediaSourceContext_t *pMediaSource) {
  int32_t ret = 0;
  ...
  if (ret == 0) {
    if (pthread_mutex_lock(&(pMediaSource->pSourcesContext->mediaMutex)) == 0) {
      ...
      /* We have finished accessing the shared resource.  Release the mutex. */
      pthread_mutex_unlock(&(pMediaSource->pSourcesContext->mediaMutex));
      LogInfo(("Stopping track kind(%d) media, value=%u",
               pMediaSource->trackKind, pMediaSource->numReadyPeer));
    } else {
      LogError(("Failed to lock media mutex, track kind=%d.",
                pMediaSource->trackKind));
...
static int32_t HandlePcEventCallback(void *pCustomContext,
  ...
  int32_t ret = 0;
  AppMediaSourceContext_t *pMediaSource =
      (AppMediaSourceContext_t *)pCustomContext;
...
static int32_t InitializeVideoSource(AppMediaSourceContext_t *pVideoSource) {
  ...
  if (ret == 0) {
    retMessageQueue = MessageQueue_Create(
        &pVideoSource->dataQueue, DEMO_TRANSCEIVER_VIDEO_DATA_QUEUE_NAME,
        sizeof(WebrtcFrame_t), DEMO_TRANSCEIVER_MAX_QUEUE_MSG_NUM);
  ...
  if (ret == 0) {
    ...
    pthread_create(&tid, NULL, VideoTx_Task, pVideoSource);
    // if( xTaskCreate( VideoTx_Task, ( ( const char * )"VideoTask" ), 2048,
    // pVideoSource, tskIDLE_PRIORITY + 1, NULL ) != pdPASS )
    // {
...
static int32_t InitializeAudioSource(AppMediaSourceContext_t *pAudioSource) {
  ...
  if (ret == 0) {
    retMessageQueue = MessageQueue_Create(
        &pAudioSource->dataQueue, DEMO_TRANSCEIVER_AUDIO_DATA_QUEUE_NAME,
        sizeof(WebrtcFrame_t), DEMO_TRANSCEIVER_MAX_QUEUE_MSG_NUM);
    if (retMessageQueue != MESSAGE_QUEUE_RESULT_OK) {
  ...
  if (ret == 0) {
    ...
    pthread_create(&tid, NULL, AudioTx_Task, pAudioSource);

    // if( xTaskCreate( AudioTx_Task, ( ( const char * )"AudioTask" ), 2048,
    // pAudioSource, tskIDLE_PRIORITY + 1, NULL ) != pdPASS )
    // {
...
int32_t AppMediaSource_InitVideoTransceiver(AppMediaSourcesContext_t *pCtx,
  ...
  if ((pCtx == NULL) || (pVideoTranceiver == NULL)) {
    LogError(("Invalid input, pCtx: %p, pVideoTranceiver: %p", pCtx,
              pVideoTranceiver));
    ret = -1;
  }
  ...
  if (ret == 0) {
...
#else
    {
      TRANSCEIVER_ENABLE_CODEC(
          pVideoTranceiver->codecBitMap,
          TRANSCEIVER_RTC_CODEC_H264_PROFILE_42E01F_LEVEL_ASYMMETRY_ALLOWED_PACKETIZATION_BIT);
      pVideoTranceiver->rollingbufferBitRate =
          DEFAULT_TRANSCEIVER_VIDEO_BIT_RATE;
    }
#endif
    pVideoTranceiver->rollingbufferDurationSec =
        DEFAULT_TRANSCEIVER_ROLLING_BUFFER_DURACTION_SECOND;
    strncpy(pVideoTranceiver->streamId, DEFAULT_TRANSCEIVER_MEDIA_STREAM_ID,
            sizeof(pVideoTranceiver->streamId));
    pVideoTranceiver->streamIdLength =
        strlen(DEFAULT_TRANSCEIVER_MEDIA_STREAM_ID);
    strncpy(pVideoTranceiver->trackId, DEFAULT_TRANSCEIVER_VIDEO_TRACK_ID,
            sizeof(pVideoTranceiver->trackId));
    pVideoTranceiver->trackIdLength =
        strlen(DEFAULT_TRANSCEIVER_VIDEO_TRACK_ID);
...
int32_t AppMediaSource_InitAudioTransceiver(AppMediaSourcesContext_t *pCtx,
  ...
  if ((pCtx == NULL) || (pAudioTranceiver == NULL)) {
    LogError(("Invalid input, pCtx: %p, pAudioTranceiver: %p", pCtx,
              pAudioTranceiver));
    ret = -1;
  }
...
#endif
    pAudioTranceiver->rollingbufferDurationSec =
        DEFAULT_TRANSCEIVER_ROLLING_BUFFER_DURACTION_SECOND;
    pAudioTranceiver->rollingbufferBitRate = DEFAULT_TRANSCEIVER_AUDIO_BIT_RATE;
    strncpy(pAudioTranceiver->streamId, DEFAULT_TRANSCEIVER_MEDIA_STREAM_ID,
            sizeof(pAudioTranceiver->streamId));
    pAudioTranceiver->streamIdLength =
        strlen(DEFAULT_TRANSCEIVER_MEDIA_STREAM_ID);
    strncpy(pAudioTranceiver->trackId, DEFAULT_TRANSCEIVER_AUDIO_TRACK_ID,
            sizeof(pAudioTranceiver->trackId));
    pAudioTranceiver->trackIdLength =
        strlen(DEFAULT_TRANSCEIVER_AUDIO_TRACK_ID);
