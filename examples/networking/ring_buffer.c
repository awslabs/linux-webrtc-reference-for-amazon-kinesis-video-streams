...
RingBufferResult_t RingBuffer_Insert(RingBuffer_t *pRingBuffer, char *pBuffer,
  ...
  if ((pRingBuffer == NULL) || (pBuffer == NULL) || (bufferLength == 0)) {
    ret = RING_BUFFER_RESULT_BAD_PARAM;
  }

  if (ret == RING_BUFFER_RESULT_OK) {
    pElement = (RingBufferElementInternal_t *)malloc(
        sizeof(RingBufferElementInternal_t));
...
RingBufferResult_t RingBuffer_GetHeadEntry(RingBuffer_t *pRingBuffer,
  ...
  if ((pRingBuffer == NULL) || (ppElement == NULL)) {
    ret = RING_BUFFER_RESULT_BAD_PARAM;
  }
...
RingBufferResult_t RingBuffer_RemoveHeadEntry(RingBuffer_t *pRingBuffer,
  ...
  if ((pRingBuffer == NULL) || (pElement == NULL)) {
    ret = RING_BUFFER_RESULT_BAD_PARAM;
  }
