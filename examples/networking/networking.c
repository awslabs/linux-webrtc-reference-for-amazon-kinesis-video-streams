...
static int LwsHttpCallback(struct lws *pWsi, enum lws_callback_reasons reason,
                           void *pUser, void *pData, size_t dataLength);

static int LwsWebsocketCallback(struct lws *pWsi,
                                enum lws_callback_reasons reason, void *pUser,
...
static int32_t Sha256Update(void *pHashContext, const uint8_t *pInput,
                            size_t inputLen) {
  int32_t ret = 0;
...
static int32_t Sha256Final(void *pHashContext, uint8_t *pOutput,
                           size_t outputLen) {
  int32_t ret = 0;
...
static int GetHostFromUrl(const char *pUrl, size_t urlLength,
                          const char **ppHost, size_t *pHostLength) {
  int ret = 0;
...
static int GetPathFromUrl(const char *pUrl, size_t urlLength,
                          const char **ppPath, size_t *pPathLength) {
  int ret = 0;
...
static int GetCurrentTimeInIso8601Format(char *pBuf, size_t *pBufLength) {
  int ret = 0;
  time_t now;
  size_t timeLength = 0;
...
static int UriEncode(const char *pSrc, size_t srcLength, char *pDst,
                     size_t *pDstLength) {
  int ret = 0;
  char ch;
...
static int SignHttpRequest(NetworkingHttpContext_t *pHttpCtx,
  ...
  snprintfRetVal = snprintf(
      &(pHttpCtx->sigV4Metadata[0]), SIGV4_METADATA_BUFFER_LENGTH,
      "host: %.*s\r\n%s: %.*s\r\n%s: %.*s\r\n", (int)pHttpCtx->uriHostLength,
      &(pHttpCtx->uriHost[0]),
      pHttpCtx->requiredHeaders[REQUIRED_HEADER_USER_AGENT_IDX].pName,
      (int)pHttpCtx->requiredHeaders[REQUIRED_HEADER_USER_AGENT_IDX]
          .valueLength,
      ...
      (int)pHttpCtx->requiredHeaders[REQUIRED_HEADER_ISO8601_TIME_IDX]
          .valueLength,
      pHttpCtx->requiredHeaders[REQUIRED_HEADER_ISO8601_TIME_IDX].pValue);

  if ((snprintfRetVal < 0) ||
      (snprintfRetVal == SIGV4_METADATA_BUFFER_LENGTH)) {
    ret = -1;
  }

  if (ret == 0) {
    sigv4HttpParams.pHttpMethod =
        (pRequest->verb == HTTP_POST) ? "POST" : "GET";
    ...
    pHttpCtx->sigv4AuthorizationHeaderLength =
        SIGV4_AUTHORIZATION_HEADER_BUFFER_LENGTH;

    if (SigV4_GenerateHTTPAuthorization(
            &(sigv4Params), &(pHttpCtx->sigv4AuthorizationHeader[0]),
            &(pHttpCtx->sigv4AuthorizationHeaderLength), &(pSignature),
  ...
  if (ret == 0) {
    pHttpCtx->requiredHeaders[REQUIRED_HEADER_AUTHORIZATION_IDX].pName =
        "Authorization";
    pHttpCtx->requiredHeaders[REQUIRED_HEADER_AUTHORIZATION_IDX].pValue =
        &(pHttpCtx->sigv4AuthorizationHeader[0]);
    pHttpCtx->requiredHeaders[REQUIRED_HEADER_AUTHORIZATION_IDX].valueLength =
        pHttpCtx->sigv4AuthorizationHeaderLength;
  }
...
static int SignWebsocketRequest(NetworkingWebsocketContext_t *pWebsocketCtx,
  ...
  char *pSignature;
  size_t pathLength, queryLength, remainingLength, writtenLength,
      signatureLength;
  size_t channelArnValueLength, encodedLength, canonicalQueryStringLength,
      canonicalHeadersLength;
  SigV4HttpParameters_t sigv4HttpParams;
  SigV4Parameters_t sigv4Params;
  SigV4Credentials_t sigv4Credentials;
  SigV4CryptoInterface_t sigv4CryptoInterface;
  ...
  snprintfRetVal =
      snprintf(&(pWebsocketCtx->sigV4Metadata[writtenLength]), remainingLength,
  ...
  if (ret == 0) {
    ret = GetPathFromUrl(pConnectInfo->pUrl, pConnectInfo->urlLength, &(pPath),
                         &(pathLength));
    ...
    if (ret == 0) {
      if ((queryLength < strlen("X-Amz-ChannelARN")) ||
          (strncmp(pQueryStart, "X-Amz-ChannelARN",
    ...
    if (ret == 0) {
      snprintfRetVal = snprintf(&(pWebsocketCtx->sigV4Metadata[writtenLength]),
                                remainingLength, "&X-Amz-ChannelARN=");
      if ((snprintfRetVal < 0) || (snprintfRetVal == remainingLength)) {
    ...
    if (ret == 0) {
      ...
      ret = UriEncode(pChannelArnValue, channelArnValueLength,
  ...
  if (ret == 0) {
    snprintfRetVal = snprintf(&(pWebsocketCtx->sigV4Metadata[writtenLength]),
                              remainingLength, "&X-Amz-Credential=");
    if ((snprintfRetVal < 0) || (snprintfRetVal == remainingLength)) {
    ...
    if (ret == 0) {
      encodedLength = remainingLength;
      ret = UriEncode(
          pAwsCredentials->pAccessKeyId, pAwsCredentials->accessKeyIdLen,
          &(pWebsocketCtx->sigV4Metadata[writtenLength]), &(encodedLength));
      if (ret == 0) {
        writtenLength += encodedLength;
        remainingLength -= encodedLength;
      } else {
    ...
    if (ret == 0) {
      encodedLength = remainingLength;
      ret = UriEncode("/", 1, &(pWebsocketCtx->sigV4Metadata[writtenLength]),
                      &(encodedLength));
      if (ret == 0) {
        writtenLength += encodedLength;
        remainingLength -= encodedLength;
    ...
    if (ret == 0) {
      encodedLength = remainingLength;
      ret = UriEncode(
          &(pWebsocketCtx->iso8601Time[0]), 8, /* Only the date part. */
          &(pWebsocketCtx->sigV4Metadata[writtenLength]), &(encodedLength));
      if (ret == 0) {
        writtenLength += encodedLength;
        remainingLength -= encodedLength;
    ...
    if (ret == 0) {
      encodedLength = remainingLength;
      ret = UriEncode(pAwsConfig->pRegion, pAwsConfig->regionLen,
                      &(pWebsocketCtx->sigV4Metadata[writtenLength]),
                      &(encodedLength));
      if (ret == 0) {
        writtenLength += encodedLength;
    ...
    if (ret == 0) {
      encodedLength = remainingLength;
      ret = UriEncode(pAwsConfig->pService, pAwsConfig->serviceLen,
                      &(pWebsocketCtx->sigV4Metadata[writtenLength]),
                      &(encodedLength));
      if (ret == 0) {
        writtenLength += encodedLength;
    ...
    if (ret == 0) {
      encodedLength = remainingLength;
      ret = UriEncode("/aws4_request", strlen("/aws4_request"),
                      &(pWebsocketCtx->sigV4Metadata[writtenLength]),
                      &(encodedLength));
      if (ret == 0) {
        writtenLength += encodedLength;
  ...
  if (ret == 0) {
    ...
    snprintfRetVal =
        snprintf(&(pWebsocketCtx->sigV4Metadata[writtenLength]),
                 remainingLength, "&X-Amz-Expires=%lu", expirationSeconds);
    if ((snprintfRetVal < 0) || (snprintfRetVal == remainingLength)) {
  ...
  if ((ret == 0) && (pAwsCredentials->pSessionToken != NULL) &&
      (pAwsCredentials->sessionTokenLength > 0)) {
    snprintfRetVal = snprintf(&(pWebsocketCtx->sigV4Metadata[writtenLength]),
                              remainingLength, "&X-Amz-Security-Token=");
    if ((snprintfRetVal < 0) || (snprintfRetVal == remainingLength)) {
    ...
    if (ret == 0) {
      encodedLength = remainingLength;
      ret = UriEncode(
          pAwsCredentials->pSessionToken, pAwsCredentials->sessionTokenLength,
          &(pWebsocketCtx->sigV4Metadata[writtenLength]), &(encodedLength));
      if (ret == 0) {
        writtenLength += encodedLength;
        remainingLength -= encodedLength;
  ...
  if (ret == 0) {
    snprintfRetVal = snprintf(&(pWebsocketCtx->sigV4Metadata[writtenLength]),
                              remainingLength, "&X-Amz-SignedHeaders=host");
    if ((snprintfRetVal < 0) || (snprintfRetVal == remainingLength)) {
  ...
  if (ret == 0) {
    ...
    sigv4HttpParams.pHeaders =
        &(pWebsocketCtx->sigV4Metadata[canonicalQueryStringLength]);
    ...
    if (SigV4_GenerateHTTPAuthorization(
            &(sigv4Params), &(pWebsocketCtx->sigv4AuthorizationHeader[0]),
            &(pWebsocketCtx->sigv4AuthorizationHeaderLength), &(pSignature),
            &(signatureLength)) != SigV4Success) {
  ...
  if (ret == 0) {
    ...
    snprintfRetVal = snprintf(&(pWebsocketCtx->sigV4Metadata[writtenLength]),
                              remainingLength, "&X-Amz-Signature=%.*s",
                              (int)signatureLength, pSignature);
  ...
  if (ret == 0) {
    if ((writtenLength + 2) <= WEBSOCKET_URI_PATH_BUFFER_LENGTH) {
      ...
      memcpy(&(pWebsocketCtx->uriPath[2]), &(pWebsocketCtx->sigV4Metadata[0]),
...
static int LwsHttpCallback(struct lws *pWsi, enum lws_callback_reasons reason,
  ...
  case LWS_CALLBACK_ESTABLISHED_CLIENT_HTTP: {
    ...
    LogDebug(("Connected with HTTP server. Response: %d.",
  ...
  case LWS_CALLBACK_RECEIVE_CLIENT_HTTP_READ: {
    ...
    LogVerbose(
        ("Received HTTP data: %.*s", (int)dataLength, (const char *)pData));
  ...
  case LWS_CALLBACK_RECEIVE_CLIENT_HTTP: {
    ...
    if (lws_http_client_read(pWsi, (char **)pHttpContext->rxBuffer,
                             &(bufferLength)) < 0) {
  ...
  case LWS_CALLBACK_CLIENT_APPEND_HANDSHAKE_HEADER: {
    ...
    for (i = 0; i < NUM_REQUIRED_HEADERS; i++) {
      if (pHttpContext->requiredHeaders[i].valueLength > 0) {
        lwsStatus = lws_add_http_header_by_name(
            pWsi, (const unsigned char *)pHttpContext->requiredHeaders[i].pName,
            (const unsigned char *)pHttpContext->requiredHeaders[i].pValue,
            pHttpContext->requiredHeaders[i].valueLength, ppStart, pEnd);
      }
    }

    lwsStatus =
        lws_add_http_header_by_name(pWsi, (const unsigned char *)"content-type",
                                    (const unsigned char *)"application/json",
                                    strlen("application/json"), ppStart, pEnd);

    contentLengthStrLength = snprintf(&(contentLengthStr[0]), 11, "%lu",
                                      pHttpContext->pRequest->bodyLength);
    lwsStatus = lws_add_http_header_by_name(
        pWsi, (const unsigned char *)"content-length",
        (const unsigned char *)&(contentLengthStr[0]), contentLengthStrLength,
        ppStart, pEnd);
    ...
    for (i = 0; i < pHttpContext->pRequest->numHeaders; i++) {
      lwsStatus = lws_add_http_header_by_name(
          ...
          pHttpContext->pRequest->pHeaders[i].valueLength, ppStart, pEnd);
    }
  ...
  case LWS_CALLBACK_CLIENT_HTTP_WRITEABLE: {
    ...
    writtenBodyLength =
        lws_write(pWsi, (unsigned char *)pHttpContext->pRequest->pBody,
                  pHttpContext->pRequest->bodyLength, LWS_WRITE_TEXT);
...
static int LwsWebsocketCallback(struct lws *pWsi,
  ...
  case LWS_CALLBACK_CLIENT_RECEIVE: {
    ...
    if ((lws_frame_is_binary(pWsi) == 0) && (dataLength > 0)) {
      if (lws_is_first_fragment(pWsi)) {
        pWebsocketContext->dataLengthInRxBuffer = 0;
      }

      if ((pWebsocketContext->dataLengthInRxBuffer + dataLength) <=
          WEBSOCKET_RX_BUFFER_LENGTH) {
        memcpy(&(pWebsocketContext
                     ->rxBuffer[pWebsocketContext->dataLengthInRxBuffer]),
               pData, dataLength);
        pWebsocketContext->dataLengthInRxBuffer += dataLength;

        LogDebug(("Received %lu bytes of WSS message. Total %lu bytes received "
                  "so far.",
                  dataLength, pWebsocketContext->dataLengthInRxBuffer));

        if (lws_is_final_fragment(pWsi)) {
          if (pWebsocketContext->rxCallback != NULL) {
            ret = pWebsocketContext->rxCallback(
                &(pWebsocketContext->rxBuffer[0]),
        ...
        LogWarn(("WSS RX buffer is not large enough for received message. "
                 "Message size: %lu.",
                 pWebsocketContext->dataLengthInRxBuffer + dataLength));
        ret = 1;
      }
  ...
  case LWS_CALLBACK_CLIENT_WRITEABLE: {
    ...
    ringBufferResult =
        RingBuffer_GetHeadEntry(&(pWebsocketContext->ringBuffer), &(pElement));
    ...
    if (ringBufferResult == RING_BUFFER_RESULT_OK) {
      ...
      writtenLength =
          lws_write(pWsi,
                    (unsigned char *)&(
                        pElement->pBuffer[LWS_PRE + pElement->currentIndex]),
                    remainingLength, LWS_WRITE_TEXT);

      if (writtenLength < 0) {
        LogError(("lws_write failed in LWS_CALLBACK_CLIENT_WRITEABLE callback, "
                  "result: %d!",
                  writtenLength));
        ret = 1;
  ...
  case LWS_CALLBACK_EVENT_WAIT_CANCELLED: {
    ...
    if (pWebsocketContext->connectionCloseRequested != 0U) {
      LogDebug(("Received request to close websocket connection. Initiating "
                "graceful shutdown."));

      lws_set_timeout(pWebsocketContext->pWsi, PENDING_TIMEOUT_USER_OK,
                      LWS_TO_KILL_ASYNC);
...
NetworkingResult_t Networking_HttpInit(NetworkingHttpContext_t *pHttpCtx,
  ...
  if (ret == NETWORKING_RESULT_OK) {
    ...
    creationInfo.client_ssl_cipher_list =
...
NetworkingResult_t
Networking_WebsocketInit(NetworkingWebsocketContext_t *pWebsocketCtx,
                         const SSLCredentials_t *pCreds) {
  NetworkingResult_t ret = NETWORKING_RESULT_OK;
  ...
  if (ret == NETWORKING_RESULT_OK) {
    ...
    if (RingBuffer_Init(&(pWebsocketCtx->ringBuffer)) !=
...
NetworkingResult_t Networking_HttpSend(NetworkingHttpContext_t *pHttpCtx,
  ...
  if ((pHttpCtx == NULL) || (pRequest == NULL) || (pResponse == NULL)) {
    ret = NETWORKING_RESULT_BAD_PARAM;
  }
  ...
  if (ret == NETWORKING_RESULT_OK) {
    ...
    pHttpCtx->requiredHeaders[REQUIRED_HEADER_USER_AGENT_IDX].pValue =
        pRequest->pUserAgent;
    pHttpCtx->requiredHeaders[REQUIRED_HEADER_USER_AGENT_IDX].valueLength =
        pRequest->userAgentLength;

    if (GetHostFromUrl(pRequest->pUrl, pRequest->urlLength, &(pHost),
                       &(hostLength)) == 0) {
      if (hostLength <= HTTP_URI_HOST_BUFFER_LENGTH) {
        memcpy(&(pHttpCtx->uriHost[0]), pHost, hostLength);
        pHttpCtx->uriHost[hostLength] = '\0';
        pHttpCtx->uriHostLength = hostLength;
      } else {
        LogError(("uriHost buffer is not large enough to fit the host, "
                  "hostLength = %lu!",
                  hostLength));
        ret = NETWORKING_RESULT_FAIL;
      }
  ...
  if (ret == NETWORKING_RESULT_OK) {
    ...
    if (GetCurrentTimeInIso8601Format(&(pHttpCtx->iso8601Time[0]),
      ...
      pHttpCtx->requiredHeaders[REQUIRED_HEADER_ISO8601_TIME_IDX].pName =
          "x-amz-date";
      pHttpCtx->requiredHeaders[REQUIRED_HEADER_ISO8601_TIME_IDX].pValue =
          &(pHttpCtx->iso8601Time[0]);
      pHttpCtx->requiredHeaders[REQUIRED_HEADER_ISO8601_TIME_IDX].valueLength =
          pHttpCtx->iso8601TimeLength;
    } else {
  ...
  if (ret == NETWORKING_RESULT_OK) {
    if ((pAwsCredentials != NULL) && (pAwsCredentials->pSessionToken != NULL) &&
        (pAwsCredentials->sessionTokenLength > 0)) {
      pHttpCtx->requiredHeaders[REQUIRED_HEADER_SESSION_TOKEN_IDX].pName =
          "x-amz-security-token";
      pHttpCtx->requiredHeaders[REQUIRED_HEADER_SESSION_TOKEN_IDX].pValue =
          pAwsCredentials->pSessionToken;
      pHttpCtx->requiredHeaders[REQUIRED_HEADER_SESSION_TOKEN_IDX].valueLength =
  ...
  if (ret == NETWORKING_RESULT_OK) {
    if (GetPathFromUrl(pRequest->pUrl, pRequest->urlLength, &(pPath),
                       &(pathLength)) == 0) {
      if (pathLength <= HTTP_URI_PATH_BUFFER_LENGTH) {
        memcpy(&(pHttpCtx->uriPath[0]), pPath, pathLength);
        pHttpCtx->uriPath[0] = '/';
        pHttpCtx->uriPath[pathLength] = '\0';
        pHttpCtx->uriPathLength = pathLength;
      } else {
        LogError(("uriPath buffer is not large enough to fit the path, "
                  "pathLength = %lu!",
                  pathLength));
        ret = NETWORKING_RESULT_FAIL;
      }
  ...
  if ((ret == NETWORKING_RESULT_OK) && (pAwsCredentials != NULL)) {
    if (SignHttpRequest(pHttpCtx, pRequest, pAwsCredentials, pAwsConfig) != 0) {
...
NetworkingResult_t
Networking_WebsocketConnect(NetworkingWebsocketContext_t *pWebsocketCtx,
                            const WebsocketConnectInfo_t *pConnectInfo,
                            const AwsCredentials_t *pAwsCredentials,
                            const AwsConfig_t *pAwsConfig) {
  NetworkingResult_t ret = NETWORKING_RESULT_OK;
  ...
  if ((pWebsocketCtx == NULL) || (pConnectInfo == NULL) ||
  ...
  if (ret == NETWORKING_RESULT_OK) {
    do {
      /* Connection check before starting. Will skip following process if
       * connection is already there. This might happen when fail happening at
       * join storage session request for any reason. */
      if (pWebsocketCtx->connectionEstablished != 0) {
        LogDebug(("Websocket connection is already there, skip the connect "
                  "process."));
        break;
      }
      ...
      if ((pWebsocketCtx->sslCreds.pDeviceCertPath != NULL) &&
          (pWebsocketCtx->sslCreds.pDeviceKeyPath != NULL)) {
        creationInfo.client_ssl_cert_filepath =
            pWebsocketCtx->sslCreds.pDeviceCertPath;
        creationInfo.client_ssl_private_key_filepath =
            pWebsocketCtx->sslCreds.pDeviceKeyPath;
      }
      ...
      if (GetHostFromUrl(pConnectInfo->pUrl, pConnectInfo->urlLength, &(pHost),
                         &(hostLength)) == 0) {
        if (hostLength <= WEBSOCKET_URI_HOST_BUFFER_LENGTH) {
          memcpy(&(pWebsocketCtx->uriHost[0]), pHost, hostLength);
      ...
      if (GetCurrentTimeInIso8601Format(&(pWebsocketCtx->iso8601Time[0]),
                                        &(pWebsocketCtx->iso8601TimeLength)) !=
          0) {
        LogError(("Failed to get ISO8601 time!"));
        ret = NETWORKING_RESULT_FAIL;
        break;
      ...
      if (SignWebsocketRequest(pWebsocketCtx, pConnectInfo, pAwsCredentials,
                               pAwsConfig) != 0) {
...
NetworkingResult_t
Networking_WebsocketSend(NetworkingWebsocketContext_t *pWebsocketCtx,
                         const char *pMessage, size_t messageLength) {
  NetworkingResult_t ret = NETWORKING_RESULT_OK;
  char *pWebsocketMessage;

  if ((pWebsocketCtx == NULL) || (pMessage == NULL) || (messageLength == 0)) {
    ret = NETWORKING_RESULT_BAD_PARAM;
  }
  ...
  if (ret == NETWORKING_RESULT_OK) {
    memcpy(&(pWebsocketMessage[LWS_PRE]), pMessage, messageLength);

    if (RingBuffer_Insert(&(pWebsocketCtx->ringBuffer), pWebsocketMessage,
...
NetworkingResult_t
Networking_WebsocketDisconnect(NetworkingWebsocketContext_t *pWebsocketCtx) {
...
NetworkingResult_t
Networking_WebsocketSignal(NetworkingWebsocketContext_t *pWebsocketCtx) {
  NetworkingResult_t ret = NETWORKING_RESULT_OK;
