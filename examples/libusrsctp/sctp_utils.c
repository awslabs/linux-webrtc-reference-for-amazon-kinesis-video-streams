...
static int OnSctpOutboundPacket(void *pAddr, void *pData, size_t length,
                                uint8_t tos, uint8_t setDf);

static SctpUtilsResult_t HandleDcepMessage(SctpSession_t *pSctpSession,
                                           uint16_t channelId, uint8_t *pData,
                                           size_t length);

static int OnSctpInboundPacket(struct socket *pSocket,
                               union sctp_sockstore addr, void *pData,
                               size_t length, struct sctp_rcvinfo rcv,
                               int flags, void *pUlpInfo);
...
static SctpUtilsResult_t ConfigureSctpSocket(struct socket *pSocket) {
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    ...
    if (usrsctp_setsockopt(pSocket, SOL_SOCKET, SO_LINGER, &(lingerOpts),
                           sizeof(lingerOpts)) != 0) {
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    ...
    if (usrsctp_setsockopt(pSocket, IPPROTO_SCTP, SCTP_NODELAY, &(valueOn),
                           sizeof(valueOn)) != 0) {
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    ...
    event.se_on = 1;
    for (i = 0; i < (uint32_t)(sizeof(eventTypes) / sizeof(eventTypes[0]));
         i++) {
      event.se_type = eventTypes[i];
      if (usrsctp_setsockopt(pSocket, IPPROTO_SCTP, SCTP_EVENT, &(event),
                             sizeof(event)) != 0) {
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    ...
    if (usrsctp_setsockopt(pSocket, IPPROTO_SCTP, SCTP_INITMSG, &(initmsg),
...
static int OnSctpOutboundPacket(void *pAddr, void *pData, size_t length,
   ...
   * connection. */
  pSctpSession->sctpSessionCallbacks.outboundPacketCallback(
      pSctpSession->sctpSessionCallbacks.pUserData, pData, length);
...
static SctpUtilsResult_t HandleDcepMessage(SctpSession_t *pSctpSession,
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    dcepResult =
        Dcep_GetMessageType(&(dcepCtx), pData, length, &(dcepMessageType));
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    ...
    case DCEP_MESSAGE_DATA_CHANNEL_ACK: {
      if (pSctpSession->sctpSessionCallbacks.dataChannelOpenAckCallback(
              pSctpSession->sctpSessionCallbacks.pUserData, channelId) ==
          SCTP_UTILS_RESULT_OK) {
        LogInfo(("Successfully opened data channel ID: %u",
                 (unsigned int)channelId));
      } else {
        LogWarn((" Failed to open data channel for which "
                 "DCEP_MESSAGE_DATA_CHANNEL_ACK was received "));
      }
    } break;

    case DCEP_MESSAGE_DATA_CHANNEL_OPEN: {
      dcepResult = Dcep_DeserializeChannelOpenMessage(&(dcepCtx), pData, length,
                                                      &(channelOpenMessage));

      if (dcepResult == DCEP_RESULT_OK) {
        pSctpSession->sctpSessionCallbacks.dataChannelOpenCallback(
            pSctpSession->sctpSessionCallbacks.pUserData, channelId,
            channelOpenMessage.pChannelName,
            channelOpenMessage.channelNameLength);

        /* Send DATA_CHANNEL_ACK Message. */
        if (SendOpenDataChannelAck(pSctpSession, channelId) !=
            SCTP_UTILS_RESULT_OK) {
...
static int OnSctpInboundPacket(struct socket *pSocket,
  ...
  case SCTP_PPID_DCEP: {
    if (HandleDcepMessage(pSctpSession, rcv.rcv_sid, pData, length) !=
        SCTP_UTILS_RESULT_OK) {
      retStatus = 1;
    }
  } break;
  ...
  case SCTP_PPID_STRING_EMPTY: {
    ...
    pSctpSession->sctpSessionCallbacks.dataChannelMessageCallback(
        pSctpSession->sctpSessionCallbacks.pUserData, rcv.rcv_sid, isBinary,
        pData, length);
  } break;

  default: {
    LogWarn(("Unhandled PPID on incoming SCTP message %ld",
             (unsigned long)rcv.rcv_ppid));
  } break;
  }
...
static SctpUtilsResult_t SendOpenDataChannelAck(SctpSession_t *pSctpSession,
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    ...
    dcepResult = Dcep_SerializeChannelAckMessage(
        &(dcepCtx), &(pSctpSession->packet[0]), &(pSctpSession->packetSize));
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    ...
    if (usrsctp_sendv(pSctpSession->socket, &(pSctpSession->packet[0]),
                      pSctpSession->packetSize, NULL, 0, &(pSctpSession->spa),
                      sizeof(pSctpSession->spa), SCTP_SENDV_SPA, 0) <= 0) {
...
SctpUtilsResult_t Sctp_CreateSession(SctpSession_t *pSctpSession,
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    ...
    pSctpSession->socket =
        usrsctp_socket(AF_CONN, SOCK_STREAM, IPPROTO_SCTP, &OnSctpInboundPacket,
                       NULL, 0, pSctpSession);
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    if (usrsctp_bind(pSctpSession->socket, (struct sockaddr *)&(localConn),
                     sizeof(localConn)) != 0) {
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    connectStatus =
        usrsctp_connect(pSctpSession->socket, (struct sockaddr *)&(remoteConn),
                        sizeof(remoteConn));
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    ...
    if (usrsctp_setsockopt(pSctpSession->socket, IPPROTO_SCTP,
                           SCTP_PEER_ADDR_PARAMS, &(params),
                           sizeof(params)) != 0) {
      LogError(
          ("usrsctp_setsockopt failed: IPPROTO_SCTP, SCTP_PEER_ADDR_PARAMS!"));
      retStatus = SCTP_UTILS_RESULT_FAIL;
    }
  }
...
SctpUtilsResult_t Sctp_FreeSession(SctpSession_t *pSctpSession) {
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    if (pSctpSession->shutdownStatus == SCTP_SESSION_ACTIVE) {
      ...
      /* handle issue mentioned here:
       * https://github.com/sctplab/usrsctp/issues/147 the change in
       * shutdownStatus will trigger OnSctpOutboundPacket to return -1. */
      pSctpSession->shutdownStatus = SCTP_SESSION_SHUTDOWN_INITIATED;
...
SctpUtilsResult_t Sctp_ProcessMessage(SctpSession_t *pSctpSession,
  ...
  if ((pSctpSession == NULL) || (pBuf == NULL)) {
    retStatus = SCTP_UTILS_RESULT_BAD_PARAM;
  }
...
SctpUtilsResult_t
Sctp_OpenDataChannel(SctpSession_t *pSctpSession,
  ...
  if ((pSctpSession == NULL) || (pDataChannelInitInfo == NULL) ||
      (pDataChannel == NULL)) {
    retStatus = SCTP_UTILS_RESULT_BAD_PARAM;
  }
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    ...
    dcepChannelOpenMessage.numRetransmissions =
        pDataChannelInitInfo->numRetransmissions;
    dcepChannelOpenMessage.maxLifetimeInMilliseconds =
        pDataChannelInitInfo->maxLifetimeInMilliseconds;

    dcepChannelOpenMessage.pChannelName =
        (const uint8_t *)pDataChannelInitInfo->pChannelName;
    dcepChannelOpenMessage.channelNameLength =
        pDataChannelInitInfo->channelNameLen;
    ...
    dcepResult = Dcep_SerializeChannelOpenMessage(
        &(dcepCtx), &(dcepChannelOpenMessage), &(pSctpSession->packet[0]),
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    ...
    pDataChannel->maxLifetimeInMilliseconds =
        pDataChannelInitInfo->maxLifetimeInMilliseconds;
...
SctpUtilsResult_t Sctp_SendMessage(SctpSession_t *pSctpSession,
  ...
  if ((pSctpSession == NULL) || (pDataChannel == NULL) || (pMessage == NULL)) {
    retStatus = SCTP_UTILS_RESULT_BAD_PARAM;
  }
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    ...
    pSctpSession->spa.sendv_sndinfo.snd_ppid =
        isBinary ? ntohl(SCTP_PPID_BINARY) : ntohl(SCTP_PPID_STRING);

    if (usrsctp_sendv(pSctpSession->socket, pMessage, messageLen, NULL, 0,
                      &(pSctpSession->spa), sizeof(pSctpSession->spa),
                      SCTP_SENDV_SPA, 0) <= 0) {
      LogError(("usrsctp_sendv failed!"));
      retStatus = SCTP_UTILS_RESULT_FAIL;
    }
...
SctpUtilsResult_t Sctp_CloseDataChannel(SctpSession_t *pSctpSession,
  ...
  if ((pSctpSession == NULL) || (pDataChannel == NULL)) {
    retStatus = SCTP_UTILS_RESULT_BAD_PARAM;
  }
  ...
  if (retStatus == SCTP_UTILS_RESULT_OK) {
    ...
    if (usrsctp_setsockopt(pSctpSession->socket, IPPROTO_SCTP,
                           SCTP_RESET_STREAMS, pSrs, (socklen_t)len) < 0) {
      LogDebug(("Failed to reset SCTP streams - this is expected if viewer was "
                "already closed."));
      retStatus = SCTP_UTILS_RESULT_FAIL;
    }
  }
