...
#define SCTP_MAX_PACKET_LENGTH                         \
    ( DCEP_HEADER_LENGTH + MAX_DATA_CHANNEL_NAME_LEN + \
 ...
 */
typedef void ( *SctpSessionOutboundPacket_t )(
    void * pUserData, uint8_t * pPacket, uint32_t packetLength );
...
typedef void ( *SctpSessionDataChannelOpen_t )( void * pUserData,
    uint16_t channelId, const uint8_t * pChannelName,
 ...
 */
typedef SctpUtilsResult_t ( *SctpSessionDataChannelAck_t )(
    void * pUserData, uint16_t channelId );
...
typedef void ( *SctpSessionDataChannelMessage_t )( void * pUserData,
    uint16_t channelId, uint8_t isBinary, uint8_t * pData,
    uint32_t dataLength );
...
SctpUtilsResult_t Sctp_CreateSession(
    SctpSession_t * pSctpSession, uint8_t isServer );

SctpUtilsResult_t Sctp_FreeSession( SctpSession_t * pSctpSession );

SctpUtilsResult_t Sctp_ProcessMessage(
    SctpSession_t * pSctpSession, uint8_t * pBuf, uint32_t bufLen );
...
SctpUtilsResult_t Sctp_SendMessage( SctpSession_t * pSctpSession,
    const SctpDataChannel_t * pDataChannel, uint8_t isBinary,
    uint8_t * pMessage, uint32_t messageLen );

SctpUtilsResult_t Sctp_CloseDataChannel(
    SctpSession_t * pSctpSession, const SctpDataChannel_t * pDataChannel );
