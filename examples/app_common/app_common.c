...
#define SET_REMOTE_INFO_USERNAME_FOUND(isFoundBit)                             \
  (isFoundBit |= IS_USERNAME_FOUND_BIT)
#define SET_REMOTE_INFO_PASSWORD_FOUND(isFoundBit)                             \
  (isFoundBit |= IS_PASSWORD_FOUND_BIT)
#define IS_REMOTE_INFO_ALL_FOUND(isFoundBit)                                   \
  (isFoundBit & IS_USERNAME_FOUND_BIT && isFoundBit & IS_PASSWORD_FOUND_BIT)
...
#define DEMO_ICE_CANDIDATE_JSON_TEMPLATE                                       \
  "{\"candidate\":\"%.*s\",\"sdpMid\":\"0\",\"sdpMLineIndex\":0}"
#define DEMO_ICE_CANDIDATE_JSON_MAX_LENGTH (1024)
#define DEMO_ICE_CANDIDATE_JSON_IPV4_TEMPLATE                                  \
  "candidate:%lu 1 udp %u %d.%d.%d.%d %d typ %s raddr 0.0.0.0 rport 0 "        \
  "generation 0 network-cost 999"
#define DEMO_ICE_CANDIDATE_JSON_IPV6_TEMPLATE                                  \
  "candidate:%lu 1 udp %u "                                                    \
...
/**
 * EMA (Exponential Moving Average) alpha value and 1-alpha value - over appx 20
 * samples
 */
 ...
 */
#define EMA_ACCUMULATOR_GET_NEXT(a, v)                                         \
  (double)(EMA_ALPHA_VALUE * (v) + ONE_MINUS_EMA_ALPHA_VALUE * (a))
...
/* Sample callback for TWCC. The average packet loss is tracked using an
   exponential moving average (EMA).
   - If packet loss stays at or below 5%, the bitrate increases by 5%.
   - If packet loss exceeds 5%, the bitrate decreases by the same percentage as
   the loss. The bitrate is adjusted once per second, ensuring it stays within
   predefined limits. */
static void
SampleSenderBandwidthEstimationHandler(void *pCustomContext,
...
static void HandleRemoteCandidate(AppContext_t *pAppContext,
                                  const SignalingMessage_t *pSignalingMessage);
static void
HandleIceServerReconnect(AppContext_t *pAppContext,
                         const SignalingMessage_t *pSignalingMessage);
static const char *GetCandidateTypeString(IceCandidateType_t candidateType);
static void HandleLocalCandidateReady(
    void *pCustomContext,
...
static void OnDataChannelMessage(PeerConnectionDataChannel_t *pDataChannel,
                                 uint8_t isBinary, uint8_t *pMessage,
...
static int32_t StartPeerConnectionSession(AppContext_t *pAppContext,
  ...
  if (remoteClientIdLength > REMOTE_ID_MAX_LENGTH) {
    LogWarn(("The remote client ID length(%lu) is too long to store.",
  ...
  if (ret == 0) {
    ...
    pcConfig.natTraversalConfigBitmap =
        ICE_CANDIDATE_NAT_TRAVERSAL_CONFIG_ALLOW_ALL;

    ret = GetIceServerList(pAppContext, pcConfig.iceServers,
                           &pcConfig.iceServersCount);
  }

  if (ret == 0) {
    peerConnectionResult = PeerConnection_AddIceServerConfig(
        &pAppSession->peerConnectionSession, &pcConfig);
    if (peerConnectionResult != PEER_CONNECTION_RESULT_OK) {
      LogWarn(("PeerConnection_AddIceServerConfig fail, result: %d",
  ...
  if (ret == 0) {
    peerConnectionResult = PeerConnection_SetOnLocalCandidateReady(
        &pAppSession->peerConnectionSession, HandleLocalCandidateReady,
        pAppSession);
    if (peerConnectionResult != PEER_CONNECTION_RESULT_OK) {
      LogWarn(("PeerConnection_SetOnLocalCandidateReady fail, result: %d",
               peerConnectionResult));
      ret = -1;
    }
  }
  ...
  if (ret == 0) {
    pTransceiver =
        &pAppSession->transceivers[DEMO_TRANSCEIVER_MEDIA_INDEX_VIDEO];
    ...
    if (ret != 0) {
      ...
      peerConnectionResult = PeerConnection_AddTransceiver(
          &pAppSession->peerConnectionSession, pTransceiver);
      if (peerConnectionResult != PEER_CONNECTION_RESULT_OK) {
        LogError(("Fail to add video transceiver, result = %d.",
                  peerConnectionResult));
        ret = -1;
      }
    }
  ...
  if (ret == 0) {
    pTransceiver =
        &pAppSession->transceivers[DEMO_TRANSCEIVER_MEDIA_INDEX_AUDIO];
    ret = pAppContext->initTransceiverFunc(pAppContext->pAppMediaSourcesContext,
    ...
    if (ret != 0) {
      ...
      peerConnectionResult = PeerConnection_AddTransceiver(
          &pAppSession->peerConnectionSession, pTransceiver);
      if (peerConnectionResult != PEER_CONNECTION_RESULT_OK) {
        LogError(("Fail to add audio transceiver, result = %d.",
                  peerConnectionResult));
        ret = -1;
      }
    }
  ...
  if (ret == 0) {
    ...
    peerConnectionResult =
        PeerConnection_Start(&pAppSession->peerConnectionSession);
    if (peerConnectionResult != PEER_CONNECTION_RESULT_OK) {
      LogError(("Fail to start peer connection, result = %d.",
                peerConnectionResult));
      ret = -1;
    }
  }
...
static int32_t ParseIceServerUri(IceControllerIceServer_t *pIceServer,
   ...
   *  1.
   * turn:35-94-7-249.t-490d1050.kinesisvideo.us-west-2.amazonaws.com:443?transport=udp
   *  2. stun:stun.kinesisvideo.us-west-2.amazonaws.com:443 */
  if ((uriLength > ICE_SERVER_TYPE_STUN_LENGTH) &&
      (strncmp(ICE_SERVER_TYPE_STUN, pUri, ICE_SERVER_TYPE_STUN_LENGTH) == 0)) {
    pIceServer->serverType = ICE_CONTROLLER_ICE_SERVER_TYPE_STUN;
    pTail = pUri + uriLength;
    pCurr = pUri + ICE_SERVER_TYPE_STUN_LENGTH;
  } else if (((uriLength > ICE_SERVER_TYPE_TURNS_LENGTH) &&
              (strncmp(ICE_SERVER_TYPE_TURNS, pUri,
  ...
  } else if ((uriLength > ICE_SERVER_TYPE_TURN_LENGTH) &&
             (strncmp(ICE_SERVER_TYPE_TURN, pUri,
    ...
    LogWarn(
        ("Unable to parse Ice URI, drop it, URI: %.*s", (int)uriLength, pUri));
    ret = -1;
  }
  ...
  if (ret == 0) {
    ...
    if (pNext == NULL) {
      LogWarn(("Unable to find second ':', drop it, URI: %.*s", (int)uriLength,
               pUri));
      ret = -1;
    } else {
      if (pNext - pCurr >= ICE_CONTROLLER_ICE_SERVER_URL_MAX_LENGTH) {
        LogWarn((
            "URL buffer is not enough to store Ice URL, length: %ld, URI: %.*s",
            pNext - pCurr, (int)uriLength, pUri));
        ret = -1;
      } else {
  ...
  if ((ret == 0) && (pCurr <= pTail)) {
    ...
    if ((retString != STRING_UTILS_RESULT_OK) || (port > UINT16_MAX)) {
      LogWarn(("No valid port number, parsed string: %.*s",
               (int)portStringLength, pCurr));
      ret = -1;
    } else {
  ...
  if (ret == 0) {
    if (pCurr >= pTail) {
      ...
      if (strncmp(pCurr, "?transport=udp", pTail - pCurr) == 0) {
        ...
        LogWarn(("Unknown transport string found, protocol: %.*s",
                 (int)(pTail - pCurr), pCurr));
        ret = -1;
      }
    } else {
...
static int32_t GetIceServerList(AppContext_t *pAppContext,
  ...
  if ((pAppContext == NULL) || (pOutputIceServers == NULL) ||
      (pOutputIceServersCount == NULL)) {
    LogError(("Invalid input, pAppContext: %p, pOutputIceServers: %p, "
              "pOutputIceServersCount: %p",
              pAppContext, pOutputIceServers, pOutputIceServersCount));
    skipProcess = 1;
  ...
  if (skipProcess == 0) {
    signalingControllerReturn = SignalingController_QueryIceServerConfigs(
        &pAppContext->signalingControllerContext, &pIceServerConfigs,
        &iceServerConfigsCount);
    if (signalingControllerReturn != SIGNALING_CONTROLLER_RESULT_OK) {
      LogError(("Fail to get Ice server configs, result: %d",
  ...
  if (skipProcess == 0) {
    ...
    written =
        snprintf(pOutputIceServers[currentIceServerIndex].url,
                 ICE_CONTROLLER_ICE_SERVER_URL_MAX_LENGTH,
                 AWS_DEFAULT_STUN_SERVER_URL, AWS_REGION, pStunUrlPostfix);
    ...
    if (written < 0) {
      ...
      /* STUN server is written correctly. Set UDP as protocol since we always
       * use UDP to query server reflexive address. */
      pOutputIceServers[currentIceServerIndex].protocol =
          ICE_SOCKET_PROTOCOL_UDP;
      pOutputIceServers[currentIceServerIndex].serverType =
      ...
      pOutputIceServers[currentIceServerIndex]
          .iceEndpoint.transportAddress.port = 443;
      pOutputIceServers[currentIceServerIndex].url[written] =
          '\0'; /* It must be NULL terminated for DNS query. */
  ...
  if (skipProcess == 0) {
    ...
    for (i = 0; i < iceServerConfigsCount; i++) {
      if (pIceServerConfigs[i].userNameLength >
          ICE_CONTROLLER_ICE_SERVER_USERNAME_MAX_LENGTH) {
        LogError(("The length of Ice server's username is too long to store, "
                  "length: %lu",
                  pIceServerConfigs[i].userNameLength));
        continue;
      } else if (pIceServerConfigs[i].passwordLength >
                 ICE_CONTROLLER_ICE_SERVER_PASSWORD_MAX_LENGTH) {
        LogError(("The length of Ice server's password is too long to store, "
                  "length: %lu",
                  pIceServerConfigs[i].passwordLength));
        continue;
      } else if (currentIceServerIndex >= *pOutputIceServersCount) {
        LogWarn(("The size of Ice server buffer has no space for more server "
                 "info, current index: %lu, buffer size: %lu, skipped server "
                 "config idx: %u",
                 currentIceServerIndex, *pOutputIceServersCount, i));
        break;
      } else {
        /* Do nothing, coverity happy. */
      }
      ...
      for (j = 0; j < pIceServerConfigs[i].iceServerUriCount; j++) {
        if (currentIceServerIndex >= *pOutputIceServersCount) {
          LogWarn(("The size of Ice server buffer has no space for more server "
                   "info, current index: %lu, buffer size: %lu, skipped server "
                   "URL: %.*s",
                   currentIceServerIndex, *pOutputIceServersCount,
        ...
        /* Parse each URI */
        parseResult =
            ParseIceServerUri(&pOutputIceServers[currentIceServerIndex],
                              pIceServerConfigs[i].iceServerUris[j].uri,
        ...
        memcpy(pOutputIceServers[currentIceServerIndex].userName,
               ...
               pIceServerConfigs[i].userNameLength);
        pOutputIceServers[currentIceServerIndex].userNameLength =
            pIceServerConfigs[i].userNameLength;
        memcpy(pOutputIceServers[currentIceServerIndex].password,
               pIceServerConfigs[i].password,
               pIceServerConfigs[i].passwordLength);
        pOutputIceServers[currentIceServerIndex].passwordLength =
            pIceServerConfigs[i].passwordLength;
        currentIceServerIndex++;
      }
    }
...
/* Sample callback for TWCC. The average packet loss is tracked using an
   ...
   the loss. The bitrate is adjusted once per second, ensuring it stays within
   predefined limits. */
static void SampleSenderBandwidthEstimationHandler(
    void *pCustomContext, TwccBandwidthInfo_t *pTwccBandwidthInfo) {
  ...
  if ((pCustomContext == NULL) || (pTwccBandwidthInfo == NULL)) {
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    lostPacketCount =
        pTwccBandwidthInfo->sentPackets - pTwccBandwidthInfo->receivedPackets;
    percentLost = (double)((pTwccBandwidthInfo->sentPackets > 0)
                               ? ((double)(lostPacketCount * 100) /
                                  (double)pTwccBandwidthInfo->sentPackets)
                               : 0.0);
    timeDifference = currentTimeUs - pTwccMetaData->lastAdjustmentTimeUs;

    pTwccMetaData->averagePacketLoss = EMA_ACCUMULATOR_GET_NEXT(
        pTwccMetaData->averagePacketLoss, ((double)percentLost));
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    if (pTwccMetaData->averagePacketLoss <= 5) {
      ...
      // Decrease encoder bitrate by average packet loss percent, with a cap at
      // MIN_BITRATE
      videoBitrate = (uint64_t)MAX(
          videoBitrate * (1.0 - (pTwccMetaData->averagePacketLoss / 100.0)),
          PEER_CONNECTION_MIN_VIDEO_BITRATE_KBPS);
      audioBitrate = (uint64_t)MAX(
          audioBitrate * (1.0 - (pTwccMetaData->averagePacketLoss / 100.0)),
  ...
  if (ret == PEER_CONNECTION_RESULT_OK) {
    ...
    LogInfo((
        "Adjusted made : average packet loss = %.2f%%, timeDifference = %lu us",
        pTwccMetaData->averagePacketLoss, timeDifference));
    LogInfo(("Suggested video bitrate: %lu kbps, suggested audio bitrate: %lu "
             "bps, sent: %lu bytes, %lu packets,   received: %lu bytes, %lu "
             "packets, in %llu msec ",
             videoBitrate, audioBitrate, pTwccBandwidthInfo->sentBytes,
             pTwccBandwidthInfo->sentPackets, pTwccBandwidthInfo->receivedBytes,
             pTwccBandwidthInfo->receivedPackets,
             pTwccBandwidthInfo->duration / 10000ULL));
  }
}
#endif
...
static int32_t InitializeAppSession(AppContext_t *pAppContext,
  ...
  peerConnectionResult =
      PeerConnection_Init(&pAppSession->peerConnectionSession, &pcConfig);
  if (peerConnectionResult != PEER_CONNECTION_RESULT_OK) {
...
#if ENABLE_TWCC_SUPPORT
  if (ret == 0) {
    /* In case you want to set a different callback based on your business
     * logic, you could replace SampleSenderBandwidthEstimationHandler() with
     * your Handler. */
    peerConnectionResult = PeerConnection_SetSenderBandwidthEstimationCallback(
        &pAppSession->peerConnectionSession,
        SampleSenderBandwidthEstimationHandler,
        &pAppSession->peerConnectionSession.twccMetaData);
    if (peerConnectionResult != PEER_CONNECTION_RESULT_OK) {
      LogError(("Fail to set Sender Bandwidth Estimation Callback, result: %d",
                peerConnectionResult));
      ret = -1;
    }
  }
  ...
  if (ret == 0) {
    pAppSession->pSignalingControllerContext =
        &(pAppContext->signalingControllerContext);
  }
...
static AppSession_t *GetCreatePeerConnectionSession(AppContext_t *pAppContext,
  ...
  for (i = 0; i < AWS_MAX_VIEWER_NUM; i++) {
    if ((pAppContext->appSessions[i].remoteClientIdLength ==
         remoteClientIdLength) &&
        (strncmp(pAppContext->appSessions[i].remoteClientId, pRemoteClientId,
                 remoteClientIdLength) == 0)) {
      /* Found existing session. */
      pAppSession = &pAppContext->appSessions[i];
      break;
    } else if ((allowCreate != 0) && (pAppSession == NULL) &&
               (pAppContext->appSessions[i].peerConnectionSession.state ==
  ...
  if ((pAppSession != NULL) && (pAppSession->peerConnectionSession.state ==
                                PEER_CONNECTION_SESSION_STATE_INITED)) {
    /* Initialize Peer Connection. */
    LogDebug(("Start peer connection on idx: %d for client ID(%lu): %.*s", i,
              remoteClientIdLength, (int)remoteClientIdLength,
              pRemoteClientId));
    initResult = StartPeerConnectionSession(
        pAppContext, pAppSession, pRemoteClientId, remoteClientIdLength);
    if (initResult != 0) {
      pAppSession = NULL;
    }
  }
...
HandleRxVideoFrame(void *pCustomContext, PeerConnectionFrame_t *pFrame) {
#ifdef ENABLE_STREAMING_LOOPBACK
  WebrtcFrame_t frame;
  ...
  if (pFrame != NULL) {
    ...
    if (pAppContext->pAppMediaSourcesContext->onMediaSinkHookFunc) {
      (void)pAppContext->pAppMediaSourcesContext->onMediaSinkHookFunc(
          pAppContext->pAppMediaSourcesContext->pOnMediaSinkHookCustom, &frame);
    }
  }
...
static PeerConnectionResult_t
HandleRxAudioFrame(void *pCustomContext, PeerConnectionFrame_t *pFrame) {
#ifdef ENABLE_STREAMING_LOOPBACK
  WebrtcFrame_t frame;
  AppContext_t *pAppContext = (AppContext_t *)pCustomContext;
...
static void HandleSdpOffer(AppContext_t *pAppContext,
  ...
  if ((pAppContext == NULL) || (pSignalingMessage == NULL)) {
    LogError(("Invalid input, pAppContext: %p, pEvent: %p", pAppContext,
              pSignalingMessage));
    skipProcess = 1;
  }
  ...
  if (skipProcess == 0) {
    ...
    signalingControllerReturn =
        SignalingController_ExtractSdpOfferFromSignalingMessage(
            pSignalingMessage->pMessage, pSignalingMessage->messageLength,
            &pSdpOfferMessage, &sdpOfferMessageLength);
    if (signalingControllerReturn != SIGNALING_CONTROLLER_RESULT_OK) {
      LogError(("Fail to parse SDP offer content, result: %d, event "
                "message(%lu): %.*s.",
                signalingControllerReturn, pSignalingMessage->messageLength,
  ...
  if (skipProcess == 0) {
    /* Translate the newline into SDP formal format. The end pattern from
     * signaling event message is "\\n" or "\\r\\n", so we replace that with
     * "\n" by calling this function. Note that this doesn't support inplace
     * replacement. */
    formalSdpMessageLength = PEER_CONNECTION_SDP_DESCRIPTION_BUFFER_MAX_LENGTH;
    signalingControllerReturn =
        SignalingController_DeserializeSdpContentNewline(
            pSdpOfferMessage, sdpOfferMessageLength, pAppContext->sdpBuffer,
            &formalSdpMessageLength);
    if (signalingControllerReturn != SIGNALING_CONTROLLER_RESULT_OK) {
      LogError(("Fail to deserialize SDP offer newline, result: %d, event "
                "message(%lu): %.*s.",
                signalingControllerReturn, pSignalingMessage->messageLength,
                (int)pSignalingMessage->messageLength,
                pSignalingMessage->pMessage));
  ...
  if (skipProcess == 0) {
    pAppSession = GetCreatePeerConnectionSession(
        pAppContext, pSignalingMessage->pRemoteClientId,
        pSignalingMessage->remoteClientIdLength, 1U);
    if (pAppSession == NULL) {
      LogWarn(("No available peer connection session for remote client "
               "ID(%lu): %.*s",
  ...
  if (skipProcess == 0) {
    ...
    peerConnectionResult = PeerConnection_SetRemoteDescription(
        &pAppSession->peerConnectionSession, &bufferSessionDescription);
    if (peerConnectionResult != PEER_CONNECTION_RESULT_OK) {
      LogWarn(("PeerConnection_SetRemoteDescription fail, result: %d, dropping "
               "ICE candidate.",
               peerConnectionResult));
    }
  }
  ...
  if (skipProcess == 0) {
    peerConnectionResult = PeerConnection_SetVideoOnFrame(
        &pAppSession->peerConnectionSession, HandleRxVideoFrame, pAppContext);
    if (peerConnectionResult != PEER_CONNECTION_RESULT_OK) {
      LogWarn(("PeerConnection_SetVideoOnFrame fail, result: %d.",
               peerConnectionResult));
      skipProcess = 1;
    }
  }
  ...
  if (skipProcess == 0) {
    peerConnectionResult = PeerConnection_SetAudioOnFrame(
        &pAppSession->peerConnectionSession, HandleRxAudioFrame, pAppContext);
    if (peerConnectionResult != PEER_CONNECTION_RESULT_OK) {
      LogWarn(("PeerConnection_SetAudioOnFrame fail, result: %d.",
               peerConnectionResult));
      skipProcess = 1;
    }
  }
  ...
  if (skipProcess == 0) {
    memset(&bufferSessionDescription, 0,
           sizeof(PeerConnectionBufferSessionDescription_t));
    bufferSessionDescription.pSdpBuffer = pAppContext->sdpBuffer;
    bufferSessionDescription.sdpBufferLength =
        PEER_CONNECTION_SDP_DESCRIPTION_BUFFER_MAX_LENGTH;
    peerConnectionResult = PeerConnection_SetLocalDescription(
        &pAppSession->peerConnectionSession, &bufferSessionDescription);
    if (peerConnectionResult != PEER_CONNECTION_RESULT_OK) {
      LogWarn(("PeerConnection_SetLocalDescription fail, result: %d.",
               peerConnectionResult));
      skipProcess = 1;
    }
  }
  ...
  if (skipProcess == 0) {
    ...
    peerConnectionResult = PeerConnection_CreateAnswer(
        ...
        &pAppContext->sdpConstructedBufferLength);
    if (peerConnectionResult != PEER_CONNECTION_RESULT_OK) {
      LogWarn(("PeerConnection_CreateAnswer fail, result: %d.",
               peerConnectionResult));
      skipProcess = 1;
    }
  }
  ...
  if (skipProcess == 0) {
    /* Translate from SDP formal format into signaling event message by
    ...
    signalingControllerReturn = SignalingController_SerializeSdpContentNewline(
        pAppContext->sdpConstructedBuffer,
        pAppContext->sdpConstructedBufferLength, pAppContext->sdpBuffer,
        &sdpAnswerMessageLength);
    if (signalingControllerReturn != SIGNALING_CONTROLLER_RESULT_OK) {
      LogError(("Fail to deserialize SDP offer newline, result: %d, "
                "constructed buffer(%lu): %.*s",
                signalingControllerReturn,
                pAppContext->sdpConstructedBufferLength,
  ...
  if (skipProcess == 0) {
    ...
    signalingMessageSdpAnswer.pRemoteClientId =
        pSignalingMessage->pRemoteClientId;
    signalingMessageSdpAnswer.remoteClientIdLength =
        pSignalingMessage->remoteClientIdLength;

    signalingControllerReturn = SignalingController_SendMessage(
        &(pAppContext->signalingControllerContext), &signalingMessageSdpAnswer);
    if (signalingControllerReturn != SIGNALING_CONTROLLER_RESULT_OK) {
      skipProcess = 1;
      LogError(("Send signaling message fail, result: %d",
                signalingControllerReturn));
    }
  }
}
...
static void HandleRemoteCandidate(AppContext_t *pAppContext,
  ...
  if (pAppSession == NULL) {
    LogWarn(
        ("No available peer connection session for remote client ID(%lu): %.*s",
         pSignalingMessage->remoteClientIdLength,
         (int)pSignalingMessage->remoteClientIdLength,
         pSignalingMessage->pRemoteClientId));
    skipProcess = 1;
  ...
  if (skipProcess == 0) {
    peerConnectionResult = PeerConnection_AddRemoteCandidate(
        &pAppSession->peerConnectionSession, pSignalingMessage->pMessage,
        pSignalingMessage->messageLength);
    if (peerConnectionResult != PEER_CONNECTION_RESULT_OK) {
      LogWarn(("PeerConnection_AddRemoteCandidate fail, result: %d, dropping "
               "ICE candidate.",
               peerConnectionResult));
    }
  }
...
HandleIceServerReconnect(AppContext_t *pAppContext,
  ...
  while (currTimeSec < initTimeSec + SIGNALING_CONNECT_STATE_TIMEOUT_SEC) {
    ret = SignalingController_RefreshIceServerConfigs(
        &(pAppContext->signalingControllerContext));
...
static void HandleLocalCandidateReady(
  ...
  if ((pAppSession == NULL) || (pIceLocalCandidate == NULL)) {
    /* Log for debugging. */
    LogWarn(("Invalid local candidate ready event, pAppSession: %p, "
             "pIceLocalCandidate: %p",
             pAppSession, pIceLocalCandidate));

    skipProcess = 1;
  }

  if (skipProcess == 0) {
    if (pIceLocalCandidate->pLocalCandidate->endpoint.transportAddress.family ==
        STUN_ADDRESS_IPv4) {
      written = snprintf(
          candidateStringBuffer, DEMO_JSON_CANDIDATE_MAX_LENGTH,
          DEMO_ICE_CANDIDATE_JSON_IPV4_TEMPLATE,
          pIceLocalCandidate->localCandidateIndex,
          pIceLocalCandidate->pLocalCandidate->priority,
          pIceLocalCandidate->pLocalCandidate->endpoint.transportAddress
              .address[0],
          pIceLocalCandidate->pLocalCandidate->endpoint.transportAddress
              .address[1],
          pIceLocalCandidate->pLocalCandidate->endpoint.transportAddress
          ...
          GetCandidateTypeString(
              pIceLocalCandidate->pLocalCandidate->candidateType));
    } else {
      written = snprintf(
          candidateStringBuffer, DEMO_JSON_CANDIDATE_MAX_LENGTH,
  ...
  if (skipProcess == 0) {
    ...
    written = snprintf(&(buffer[0]), DEMO_JSON_CANDIDATE_MAX_LENGTH,
                       DEMO_ICE_CANDIDATE_JSON_TEMPLATE, written,
  ...
  if (skipProcess == 0) {
    ...
    signalingControllerReturn = SignalingController_SendMessage(
        pAppSession->pSignalingControllerContext, &signalingMessage);
    if (signalingControllerReturn != SIGNALING_CONTROLLER_RESULT_OK) {
      LogError(("Send signaling message fail, result: %d",
                signalingControllerReturn));
      skipProcess = 1;
    } else {
      LogDebug(("Sent local candidate to remote peer, msg(%d): %.*s", written,
                written, &(buffer[0])));
    }
  }
}
...
static int OnSignalingMessageReceived(SignalingMessage_t *pSignalingMessage,
  ...
  LogDebug(("Sender ID: %.*s", (int)pSignalingMessage->remoteClientIdLength,
            pSignalingMessage->pRemoteClientId));
  LogDebug(("Correlation ID: %.*s", (int)pSignalingMessage->correlationIdLength,
            pSignalingMessage->pCorrelationId));
  LogDebug(("Message Length: %lu, Message:", pSignalingMessage->messageLength));
  LogDebug(("%.*s", (int)pSignalingMessage->messageLength,
            pSignalingMessage->pMessage));
...
#endif
    HandleSdpOffer(pAppContext, pSignalingMessage);
    break;
  case SIGNALING_TYPE_MESSAGE_SDP_ANSWER:
    break;
  case SIGNALING_TYPE_MESSAGE_ICE_CANDIDATE:
    HandleRemoteCandidate(pAppContext, pSignalingMessage);
    break;
  case SIGNALING_TYPE_MESSAGE_RECONNECT_ICE_SERVER:
    HandleIceServerReconnect(pAppContext, pSignalingMessage);
    break;
...
static void OnDataChannelMessage(PeerConnectionDataChannel_t *pDataChannel,
  ...
  if (isBinary) {
    LogWarn(
        ("[VIEWER] [Peer: %s Channel Name: %s] >>> DataChannel Binary Message",
         pDataChannel->pPeerConnection->combinedName,
         pDataChannel->ucDataChannelName));
  } else {
    LogWarn(("[VIEWER] [Peer: %s Channel Name: %s] >>> DataChannel String "
             "Message: %.*s\n",
             pDataChannel->pPeerConnection->combinedName,
             pDataChannel->ucDataChannelName, (int)pMessageLen, pMessage));

    sprintf(
        ucSendMessage, "Received %ld bytes, ECHO: %.*s", (long int)pMessageLen,
        (int)(pMessageLen > (DEFAULT_DATA_CHANNEL_ON_MESSAGE_BUFFER_SIZE - 128)
                  ? (DEFAULT_DATA_CHANNEL_ON_MESSAGE_BUFFER_SIZE - 128)
                  : pMessageLen),
        pMessage);
    retStatus = PeerConnectionSCTP_DataChannelSend(
        pDataChannel, 0U, (uint8_t *)ucSendMessage, strlen(ucSendMessage));
  }

  if (retStatus != PEER_CONNECTION_RESULT_OK) {
    LogWarn(("[KVS Master] OnDataChannelMessage(): operation returned status "
             "code: 0x%08x \n",
             (unsigned int)retStatus));
  }
}

OnDataChannelMessageReceived_t
PeerConnectionSCTP_SetChannelOnMessageCallbackHook(
    PeerConnectionSession_t *pPeerConnectionSession, uint32_t ulChannelId,
    const uint8_t *pucName, uint32_t ulNameLen) {
...
int AppCommon_Init(AppContext_t *pAppContext,
  ...
  if ((pAppContext == NULL) || (pMediaContext == NULL) ||
      (initTransceiverFunc == NULL)) {
    LogError(("Invalid parameter, pAppContext: %p, pMediaContext: %p, "
              "initTransceiverFunc: %p",
              pAppContext, pMediaContext, initTransceiverFunc));
    ret = -1;
  }
  ...
  if (ret == 0) {
    ...
    signalingControllerReturn = SignalingController_Init(
        &pAppContext->signalingControllerContext, &sslCreds);
    if (signalingControllerReturn != SIGNALING_CONTROLLER_RESULT_OK) {
  ...
  if (ret == 0) {
    for (i = 0; i < AWS_MAX_VIEWER_NUM; i++) {
      ret = InitializeAppSession(pAppContext, &(pAppContext->appSessions[i]));
      if (ret != 0) {
...
int AppCommon_Start(AppContext_t *pAppContext) {
  ...
  if (ret == 0) {
    ...
    connectInfo.awsIotCreds.iotCredentialsEndpointLength =
        strlen(AWS_CREDENTIALS_ENDPOINT);
    ...
    signalingControllerReturn = SignalingController_StartListening(
        &(pAppContext->signalingControllerContext), &connectInfo);
    if (signalingControllerReturn != SIGNALING_CONTROLLER_RESULT_OK) {
