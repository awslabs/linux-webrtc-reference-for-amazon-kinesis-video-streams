...
#define SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_TWCC_EXT_URL                      \
...
#define SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_FINGERPRINT_PREFIX_LENGTH         \
  (8) // the length of "sha-256 "

#define SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_FMTP_H264_PROFILE_42E01F_LEVEL_ASYMMETRY_ALLOWED_PACKETIZATION \
  "level-asymmetry-allowed=1;packetization-mode=1;profile-level-id=42e01f"
#define SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_FMTP_OPUS                         \
  "minptime=10;useinbandfec=1"
#define SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_FMTP_H265                         \
  "profile-space=0;profile-id=0;tier-flag=0;level-id=0;interop-constraints="   \
  "000000000000;sprop-vps=QAEMAf//"                                            \
  "AIAAAAMAAAMAAAMAAAMAALUCQA==;sprop-sps=QgEBAIAAAAMAAAMAAAMAAAMAAKACgIAtH+"  \
  "W1kkbQzkkktySqSfKSyA==;sprop-pps=RAHBpVgeSA=="
...
#define SDP_CONTROLLER_SET_PAYLOAD(rtxPayload, aptPayload)                     \
  (rtxPayload << 16 | aptPayload)

#define SDP_CONTROLLER_DATA_CHANNEL_ATTRIBUTE_NAME_MEDIA_NAME                  \
...
static SdpControllerResult_t
ParseExtraAttributes(SdpControllerSdpDescription_t *pOffer,
                     SdpAttribute_t *pAttribute);
static SdpControllerResult_t
parseMediaAttributes(SdpControllerSdpDescription_t *pOffer,
                     const char *pAttributeBuffer,
                     size_t attributeBufferLength);
static SdpControllerResult_t
parseSessionAttributes(SdpControllerSdpDescription_t *pOffer,
                       const char *pAttributeBuffer,
                       size_t attributeBufferLength);
...
static SdpControllerResult_t
serializeAttributes(SdpSerializerContext_t *pCtx,
                    SdpControllerAttributes_t *pAttributes,
                    uint16_t attributeCount);
static SdpControllerResult_t
serializeConnectionInfo(SdpSerializerContext_t *pCtx,
                        SdpControllerConnectionInformation_t *pConnectionInfo);
static SdpControllerResult_t
serializeMedias(SdpSerializerContext_t *pCtx,
                SdpControllerMediaDescription_t *pMediaDescriptions,
                uint16_t mediaCount);
static SdpControllerResult_t
serializeSdpMessage(SdpControllerSdpDescription_t *pSdpDescription,
                    char *pOutputBuffer, size_t *pOutputBufferSize);
static SdpControllerResult_t
PopulateTransceiverSsrc(char **ppBuffer, size_t *pBufferLength,
                        SdpControllerMediaDescription_t *pLocalMediaDescription,
                        const Transceiver_t *pTransceiver, const char *pCname,
                        size_t cnameLength, uint32_t containRtx);
static SdpControllerResult_t
PopulateRtcpFb(uint32_t payload, uint16_t twccExtId, char **ppBuffer,
               size_t *pBufferLength,
               SdpControllerMediaDescription_t *pLocalMediaDescription);
static SdpControllerResult_t
PopulateCodecAttributesH264Profile42E01FLevelAsymmetryAllowedPacketization(
    SdpControllerMediaDescription_t *pRemoteMediaDescription,
    const Transceiver_t *pTransceiver, uint32_t payload, uint32_t rtxPayload,
    char **ppBuffer, size_t *pBufferLength,
    SdpControllerMediaDescription_t *pLocalMediaDescription);
static SdpControllerResult_t PopulateCodecAttributesOpus(
    SdpControllerMediaDescription_t *pRemoteMediaDescription,
    const Transceiver_t *pTransceiver, uint32_t payload, uint32_t rtxPayload,
    char **ppBuffer, size_t *pBufferLength,
    SdpControllerMediaDescription_t *pLocalMediaDescription);
static SdpControllerResult_t PopulateCodecAttributesVp8(
    SdpControllerMediaDescription_t *pRemoteMediaDescription,
    const Transceiver_t *pTransceiver, uint32_t payload, uint32_t rtxPayload,
    char **ppBuffer, size_t *pBufferLength,
    SdpControllerMediaDescription_t *pLocalMediaDescription);
static SdpControllerResult_t PopulateCodecAttributesMulaw(
    SdpControllerMediaDescription_t *pRemoteMediaDescription,
    const Transceiver_t *pTransceiver, uint32_t payload, uint32_t rtxPayload,
    char **ppBuffer, size_t *pBufferLength,
    SdpControllerMediaDescription_t *pLocalMediaDescription);
static SdpControllerResult_t PopulateCodecAttributesAlaw(
    SdpControllerMediaDescription_t *pRemoteMediaDescription,
    const Transceiver_t *pTransceiver, uint32_t payload, uint32_t rtxPayload,
    char **ppBuffer, size_t *pBufferLength,
    SdpControllerMediaDescription_t *pLocalMediaDescription);
static SdpControllerResult_t PopulateCodecAttributesH265(
    SdpControllerMediaDescription_t *pRemoteMediaDescription,
    const Transceiver_t *pTransceiver, uint32_t payload, uint32_t rtxPayload,
    char **ppBuffer, size_t *pBufferLength,
    SdpControllerMediaDescription_t *pLocalMediaDescription);
static SdpControllerResult_t PopulateCodecAttributes(
    SdpControllerMediaDescription_t *pRemoteMediaDescription,
    SdpControllerPopulateMediaConfiguration_t populateConfiguration,
    char **ppBuffer, size_t *pBufferLength,
    SdpControllerMediaDescription_t *pLocalMediaDescription);
static const SdpControllerAttributes_t *
FindAttributeName(const SdpControllerAttributes_t *pAttributes,
                  size_t attributeCount, char *pPattern, size_t patternLength);
static const SdpControllerAttributes_t *
FindFmtpBasedOnCodec(const SdpControllerAttributes_t *pAttributes,
                     size_t attributeCount, uint32_t codec);
...
ParseExtraAttributes(SdpControllerSdpDescription_t *pOffer,
  ...
  if ((pOffer == NULL) || (pAttribute == NULL)) {
    LogError(("Fail to parse extra attributes, pOffer: %p, pAttribute: %p",
              pOffer, pAttribute));
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ...
    if ((pAttribute->attributeNameLength ==
         SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_FINGERPRINT_LENGTH) &&
        (strncmp(SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_FINGERPRINT,
                 pAttribute->pAttributeName,
                 SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_FINGERPRINT_LENGTH) ==
         0) &&
        (pAttribute->attributeValueLength >
         SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_FINGERPRINT_PREFIX_LENGTH)) {
      /* Found fingerprint, store it as extra info. */
      pOffer->quickAccess.pFingerprint =
          pAttribute->pAttributeValue +
          SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_FINGERPRINT_PREFIX_LENGTH;
      pOffer->quickAccess.fingerprintLength =
          pAttribute->attributeValueLength -
          SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_FINGERPRINT_PREFIX_LENGTH;
    } else if ((pAttribute->attributeNameLength ==
                SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_SETUP_LENGTH) &&
               (strncmp(SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_SETUP,
                        pAttribute->pAttributeName,
                        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_SETUP_LENGTH) ==
                0)) {
      /* Found setup, store it as extra info. */
      if ((pAttribute->attributeValueLength ==
           SDP_CONTROLLER_MEDIA_DTLS_ROLE_ACTIVE_LENGTH) &&
          (strncmp(SDP_CONTROLLER_MEDIA_DTLS_ROLE_ACTIVE,
                   pAttribute->pAttributeName,
                   SDP_CONTROLLER_MEDIA_DTLS_ROLE_ACTIVE_LENGTH) == 0)) {
        pOffer->quickAccess.dtlsRole = SDP_CONTROLLER_DTLS_ROLE_ACTIVE;
      } else {
        pOffer->quickAccess.dtlsRole = SDP_CONTROLLER_DTLS_ROLE_ACTPASS;
      }
    } else if ((pAttribute->attributeNameLength ==
                SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_ICE_OPTION_LENGTH) &&
               (strncmp(
                    SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_ICE_OPTION,
                    pAttribute->pAttributeName,
                    SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_ICE_OPTION_LENGTH) ==
                0) &&
               (pAttribute->attributeValueLength ==
                SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_ICE_OPTION_LENGTH) &&
               (strncmp(
                    SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_ICE_OPTION,
                    pAttribute->pAttributeValue,
                    SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_ICE_OPTION_LENGTH) ==
                0)) {
      pOffer->quickAccess.isIceTrickle = 1U;
    } else if ((pAttribute->attributeNameLength ==
                SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_ICE_UFRAG_LENGTH) &&
               (strncmp(SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_ICE_UFRAG,
                        pAttribute->pAttributeName,
                        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_ICE_UFRAG_LENGTH) ==
                0)) {
      ...
      pOffer->quickAccess.iceUfragLength = pAttribute->attributeValueLength;
                ...
                SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_ICE_PWD_LENGTH) &&
               (strncmp(SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_ICE_PWD,
                        pAttribute->pAttributeName,
                        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_ICE_PWD_LENGTH) ==
                0)) {
      ...
      pOffer->quickAccess.icePwdLength = pAttribute->attributeValueLength;
                ...
                SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_EXTMAP_LENGTH) &&
               (strncmp(SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_EXTMAP,
                        pAttribute->pAttributeName,
                        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_EXTMAP_LENGTH) ==
                0) &&
               (pAttribute->attributeValueLength >
                SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_TWCC_EXT_URL_LENGTH)) {
      /* The attribute value length is confirmed larger than URL. */
      size_t length = pAttribute->attributeValueLength -
                      SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_TWCC_EXT_URL_LENGTH;
      const char *pFindStart = pAttribute->pAttributeValue + length;
      if (strncmp(SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_TWCC_EXT_URL, pFindStart,
                  SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_TWCC_EXT_URL_LENGTH) ==
          0) {
        /* Found TWCC ext URL. */
        stringResult =
            StringUtils_ConvertStringToUl(pAttribute->pAttributeValue, length,
                                          &pOffer->quickAccess.twccExtId);
        if (stringResult != STRING_UTILS_RESULT_OK) {
          LogError(("StringUtils_ConvertStringToUl fail, result %d, converting "
                    "%.*s to %u",
                    stringResult, (int)length, pAttribute->pAttributeValue,
                    pOffer->quickAccess.twccExtId));
          ...
          LogDebug(("Found TWCC, ID: %u", pOffer->quickAccess.twccExtId));
                ...
                SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_CANDIDATE_LENGTH) &&
               (strncmp(SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_CANDIDATE,
                        pAttribute->pAttributeName,
                        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_CANDIDATE_LENGTH) ==
                0)) {
      /* Ensure we do not exceed the maximum candidate limit. */
      if (pOffer->quickAccess.remoteCandidateCount <
          SDP_CONTROLLER_MAX_SDP_ATTRIBUTES_COUNT) {
        pOffer->quickAccess
            .pRemoteCandidates[pOffer->quickAccess.remoteCandidateCount] =
            pAttribute->pAttributeValue;
        pOffer->quickAccess
            .remoteCandidateLengths[pOffer->quickAccess.remoteCandidateCount] =
            pAttribute->attributeValueLength;
        pOffer->quickAccess.remoteCandidateCount++;
      } else {
        LogWarn(("Max candidate limit reached (%d), ignoring extra candidates.",
                 SDP_CONTROLLER_MAX_SDP_ATTRIBUTES_COUNT));
      }
    }
  }
...
parseMediaAttributes(SdpControllerSdpDescription_t *pOffer,
  ...
  uint8_t *pAttributeCount =
      &pOffer->mediaDescriptions[mediaIndex].mediaAttributesCount;

  if (pOffer->mediaDescriptions[mediaIndex].mediaAttributesCount >=
      SDP_CONTROLLER_MAX_SDP_ATTRIBUTES_COUNT) {
    ret = SDP_CONTROLLER_RESULT_SDP_MEDIA_ATTRIBUTE_MAX_EXCEDDED;
  }

  if (ret == SDP_CONTROLLER_RESULT_OK) {
    sdpResult = SdpDeserializer_ParseAttribute(
        pAttributeBuffer, attributeBufferLength, &attribute);
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    pOffer->mediaDescriptions[mediaIndex]
        .attributes[*pAttributeCount]
        .pAttributeName = attribute.pAttributeName;
    pOffer->mediaDescriptions[mediaIndex]
        .attributes[*pAttributeCount]
        .attributeNameLength = attribute.attributeNameLength;
    pOffer->mediaDescriptions[mediaIndex]
        .attributes[*pAttributeCount]
        .pAttributeValue = attribute.pAttributeValue;
    pOffer->mediaDescriptions[mediaIndex]
        .attributes[*pAttributeCount]
...
parseSessionAttributes(SdpControllerSdpDescription_t *pOffer,
  ...
  if (pOffer->sessionAttributesCount >=
      SDP_CONTROLLER_MAX_SDP_ATTRIBUTES_COUNT) {
    ret = SDP_CONTROLLER_RESULT_SDP_SESSION_ATTRIBUTE_MAX_EXCEDDED;
  }
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    pOffer->attributes[pOffer->sessionAttributesCount].pAttributeName =
        attribute.pAttributeName;
    pOffer->attributes[pOffer->sessionAttributesCount].attributeNameLength =
        attribute.attributeNameLength;
    pOffer->attributes[pOffer->sessionAttributesCount].pAttributeValue =
        attribute.pAttributeValue;
    pOffer->attributes[pOffer->sessionAttributesCount].attributeValueLength =
...
static SdpControllerResult_t serializeOrigin(SdpSerializerContext_t *pCtx,
  ...
  origin.connectionInfo.pAddress =
      pOrigin->sdpConnectionInformation.pConnectionAddress;
  origin.connectionInfo.addressLength =
      pOrigin->sdpConnectionInformation.connectionAddressLength;
...
serializeAttributes(SdpSerializerContext_t *pCtx,
  ...
  for (i = 0; i < attributeCount; i++) {
    ...
    attribute.attributeNameLength =
        (pCurrentAttrubute + i)->attributeNameLength;
    attribute.pAttributeValue = (pCurrentAttrubute + i)->pAttributeValue;
    attribute.attributeValueLength =
        (pCurrentAttrubute + i)->attributeValueLength;

    sdpResult =
        SdpSerializer_AddAttribute(pCtx, SDP_TYPE_ATTRIBUTE, &attribute);
    if (sdpResult != SDP_RESULT_OK) {
      LogError(("Serialize SDP attribute failure, result: %x, attribute name: "
                "%.*s, value: %.*s",
                sdpResult, (int)attribute.attributeNameLength,
                attribute.pAttributeName, (int)attribute.attributeValueLength,
                attribute.pAttributeValue));
      ret = SDP_CONTROLLER_RESULT_SDP_FAIL_SERIALIZER_ADD;
    }
  }
...
serializeConnectionInfo(SdpSerializerContext_t *pCtx,
  ...
  sdpResult =
      SdpSerializer_AddConnectionInfo(pCtx, SDP_TYPE_CONNINFO, &connInfo);
  if (sdpResult != SDP_RESULT_OK) {
    LogError(("Serialize SDP connection information failure, result: %d",
              sdpResult));
    ret = SDP_CONTROLLER_RESULT_SDP_FAIL_SERIALIZER_ADD;
  }
...
serializeMedias(SdpSerializerContext_t *pCtx,
  ...
  for (i = 0; i < mediaCount; i++) {
    ...
    /* Media name */
    sdpResult =
        SdpSerializer_AddBuffer(pCtx, SDP_TYPE_MEDIA, pCurrentMedia->pMediaName,
                                pCurrentMedia->mediaNameLength);
    if (sdpResult != SDP_RESULT_OK) {
    ...
    if (pCurrentMedia->pMediaTitle) {
      sdpResult = SdpSerializer_AddBuffer(pCtx, SDP_TYPE_MEDIA_TITLE,
                                          pCurrentMedia->pMediaTitle,
                                          pCurrentMedia->mediaTitleLength);
      if (sdpResult != SDP_RESULT_OK) {
    ...
    ret = serializeAttributes(pCtx, &pCurrentMedia->attributes[0],
                              pCurrentMedia->mediaAttributesCount);
    if (ret != SDP_CONTROLLER_RESULT_OK) {
      break;
    }
  }
...
serializeSdpMessage(SdpControllerSdpDescription_t *pSdpDescription,
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    /* Append version. */
    sdpResult =
        SdpSerializer_AddU64(&ctx, SDP_TYPE_VERSION, pSdpDescription->version);
    if (sdpResult != SDP_RESULT_OK) {
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    /* Append session name. */
    sdpResult = SdpSerializer_AddBuffer(&ctx, SDP_TYPE_SESSION_NAME,
                                        pSdpDescription->pSessionName,
                                        pSdpDescription->sessionNameLength);
    if (sdpResult != SDP_RESULT_OK) {
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ...
    ret = serializeAttributes(&ctx, &pSdpDescription->attributes[0],
                              pSdpDescription->sessionAttributesCount);
  }

  if (ret == SDP_CONTROLLER_RESULT_OK) {
    /* Append media information. */
    ret = serializeMedias(&ctx, &pSdpDescription->mediaDescriptions[0],
...
PopulateTransceiverSsrc(char **ppBuffer, size_t *pBufferLength,
  ...
  if ((ppBuffer == NULL) || (pBufferLength == NULL) ||
      (pLocalMediaDescription == NULL) || (pTransceiver == NULL) ||
      (pCname == NULL)) {
    LogError(("Invalid input, ppBuffer: %p, pBufferLength: %p, "
              "pLocalMediaDescription: %p, pTransceiver: %p, pCname: %p",
              ppBuffer, pBufferLength, pLocalMediaDescription, pTransceiver,
              pCname));
    ret = SDP_CONTROLLER_RESULT_BAD_PARAMETER;
  } else {
    /* Initialize current buffer pointer/size and the pointer to the attribute
     * count. */
    pCurBuffer = *ppBuffer;
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    pTargetAttribute =
        &pLocalMediaDescription->attributes[*pTargetAttributeCount];
    pTargetAttribute->pAttributeName = SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_SSRC;
    pTargetAttribute->attributeNameLength =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_SSRC_LENGTH;

    written = snprintf(pCurBuffer, remainSize, "%u cname:%.*s",
                       pTransceiver->ssrc, (int)cnameLength, pCname);
    if (written < 0) {
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ...
    written = snprintf(pCurBuffer, remainSize, "%u msid:%.*s %.*s",
                       pTransceiver->ssrc, (int)pTransceiver->streamIdLength,
                       pTransceiver->streamId, (int)pTransceiver->trackIdLength,
                       pTransceiver->trackId);
    if (written < 0) {
      ret = SDP_CONTROLLER_RESULT_SDP_FAIL_SNPRINTF;
      LogError(("snprintf return unexpected value %d", written));
    } else if (written == remainSize) {
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ...
    written =
        snprintf(pCurBuffer, remainSize, "%u mslabel:%.*s", pTransceiver->ssrc,
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ...
    written =
        snprintf(pCurBuffer, remainSize, "%u label:%.*s", pTransceiver->ssrc,
  ...
  if ((ret == SDP_CONTROLLER_RESULT_OK) && (containRtx != 0)) {
    ...
    written = snprintf(pCurBuffer, remainSize, "%u cname:%.*s",
                       pTransceiver->rtxSsrc, (int)cnameLength, pCname);
    if (written < 0) {
      ret = SDP_CONTROLLER_RESULT_SDP_FAIL_SNPRINTF;
      LogError(("snprintf return unexpected value %d", written));
    } else if (written == remainSize) {
  ...
  if ((ret == SDP_CONTROLLER_RESULT_OK) && (containRtx != 0)) {
    ...
    written = snprintf(pCurBuffer, remainSize, "%u msid:%.*s %.*sRTX",
                       pTransceiver->rtxSsrc, (int)pTransceiver->streamIdLength,
                       pTransceiver->streamId, (int)pTransceiver->trackIdLength,
                       pTransceiver->trackId);
    if (written < 0) {
      ret = SDP_CONTROLLER_RESULT_SDP_FAIL_SNPRINTF;
  ...
  if ((ret == SDP_CONTROLLER_RESULT_OK) && (containRtx != 0)) {
    ...
    written = snprintf(pCurBuffer, remainSize, "%u mslabel:%.*sRTX",
                       pTransceiver->rtxSsrc, (int)pTransceiver->streamIdLength,
                       pTransceiver->streamId);
    if (written < 0) {
      ret = SDP_CONTROLLER_RESULT_SDP_FAIL_SNPRINTF;
      LogError(("snprintf return unexpected value %d", written));
    } else if (written == remainSize) {
  ...
  if ((ret == SDP_CONTROLLER_RESULT_OK) && (containRtx != 0)) {
    ...
    written = snprintf(pCurBuffer, remainSize, "%u label:%.*sRTX",
                       pTransceiver->rtxSsrc, (int)pTransceiver->trackIdLength,
                       pTransceiver->trackId);
    if (written < 0) {
      ret = SDP_CONTROLLER_RESULT_SDP_FAIL_SNPRINTF;
      LogError(("snprintf return unexpected value %d", written));
...
PopulateRtcpFb(uint32_t payload, uint16_t twccExtId, char **ppBuffer,
  ...
  if ((ppBuffer == NULL) || (pBufferLength == NULL) ||
    ...
    LogError(("Invalid input, ppBuffer: %p, pBufferLength: %p, "
              "pLocalMediaDescription: %p",
              ppBuffer, pBufferLength, pLocalMediaDescription));
    ret = SDP_CONTROLLER_RESULT_BAD_PARAMETER;
  }
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ...
    pTargetAttribute =
        &pLocalMediaDescription->attributes[*pTargetAttributeCount];
    pTargetAttribute->pAttributeName =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_RTCP_FB;
    pTargetAttribute->attributeNameLength =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_RTCP_FB_LENGTH;

    written = snprintf(pCurBuffer, remainSize, "%u %s", payload,
  ...
  if ((ret == SDP_CONTROLLER_RESULT_OK) && (twccExtId > 0)) {
    ...
    written =
        snprintf(pCurBuffer, remainSize, "%u %s", payload,
...
PopulateCodecAttributesH264Profile42E01FLevelAsymmetryAllowedPacketization(
  ...
  pTargetAttribute->attributeNameLength =
      SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_RTPMAP_LENGTH;
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    if (!isOffer) {
      ...
      pSourceAttribute = FindFmtpBasedOnCodec(
          pRemoteMediaDescription->attributes,
          pRemoteMediaDescription->mediaAttributesCount, payload);
    }

    /* Set fmtp only if:
     *   1. It's offer, or
     *   2. It's not offer but we found the corresponding fmtp from remote
     * description. */
    if (isOffer || pSourceAttribute) {
      pTargetAttribute =
          &pLocalMediaDescription->attributes[*pTargetAttributeCount];
      pTargetAttribute->pAttributeName =
          SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_FMTP;
      pTargetAttribute->attributeNameLength =
          SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_FMTP_LENGTH;

      if (isOffer) {
        written = snprintf(
            pCurBuffer, remainSize, "%u %s", payload,
        ...
        written = snprintf(pCurBuffer, remainSize, "%.*s",
                           (int)pSourceAttribute->attributeValueLength,
                           pSourceAttribute->pAttributeValue);
      }
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    if (rtxPayload != 0) {
      pTargetAttribute =
          &pLocalMediaDescription->attributes[*pTargetAttributeCount];
      pTargetAttribute->pAttributeName =
          SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_RTPMAP;
      pTargetAttribute->attributeNameLength =
          SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_RTPMAP_LENGTH;

      written = snprintf(pCurBuffer, remainSize, "%u %s", rtxPayload,
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    if (rtxPayload != 0) {
      ...
      written =
          snprintf(pCurBuffer, remainSize, "%u apt=%u", rtxPayload, payload);
...
static SdpControllerResult_t PopulateCodecAttributesOpus(
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    if (rtxPayload != 0) {
      LogWarn(("Ignore RTX for OPUS, RTX payload: %u.", rtxPayload));
    }
  }
...
static SdpControllerResult_t PopulateCodecAttributesVp8(
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    if (rtxPayload != 0) {
      LogWarn(("Ignore RTX for VP8, RTX payload: %u.", rtxPayload));
    }
  }
...
static SdpControllerResult_t PopulateCodecAttributesMulaw(
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    if (rtxPayload != 0) {
      LogWarn(("Ignore RTX for MULAW, RTX payload: %u.", rtxPayload));
    }
  }
...
static SdpControllerResult_t PopulateCodecAttributesAlaw(
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    if (rtxPayload != 0) {
      LogWarn(("Ignore RTX for ALAW, RTX payload: %u.", rtxPayload));
    }
  }
...
static SdpControllerResult_t PopulateCodecAttributes(
  ...
  if ((ppBuffer == NULL) || (pBufferLength == NULL) ||
    ...
    ret = SDP_CONTROLLER_RESULT_BAD_PARAMETER;
    LogError(("Invalid input, ppBuffer: %p, pBufferLength: %p, "
              "pLocalMediaDescription: %p, pTransceiver: %p",
              ppBuffer, pBufferLength, pLocalMediaDescription, pTransceiver));
  }

  if (ret == SDP_CONTROLLER_RESULT_OK) {
    if (TRANSCEIVER_IS_CODEC_ENABLED(
            pTransceiver->codecBitMap,
            TRANSCEIVER_RTC_CODEC_H264_PROFILE_42E01F_LEVEL_ASYMMETRY_ALLOWED_PACKETIZATION_BIT)) {
      ret =
          PopulateCodecAttributesH264Profile42E01FLevelAsymmetryAllowedPacketization(
              pRemoteMediaDescription, pTransceiver, payload, rtxPayload,
              ppBuffer, pBufferLength, pLocalMediaDescription);
    } else if (TRANSCEIVER_IS_CODEC_ENABLED(pTransceiver->codecBitMap,
                                            TRANSCEIVER_RTC_CODEC_OPUS_BIT)) {
      ret = PopulateCodecAttributesOpus(pRemoteMediaDescription, pTransceiver,
                                        payload, rtxPayload, ppBuffer,
                                        pBufferLength, pLocalMediaDescription);
    } else if (TRANSCEIVER_IS_CODEC_ENABLED(pTransceiver->codecBitMap,
                                            TRANSCEIVER_RTC_CODEC_VP8_BIT)) {
      ret = PopulateCodecAttributesVp8(pRemoteMediaDescription, pTransceiver,
                                       payload, rtxPayload, ppBuffer,
                                       pBufferLength, pLocalMediaDescription);
    } else if (TRANSCEIVER_IS_CODEC_ENABLED(pTransceiver->codecBitMap,
                                            TRANSCEIVER_RTC_CODEC_MULAW_BIT)) {
      ret = PopulateCodecAttributesMulaw(pRemoteMediaDescription, pTransceiver,
                                         payload, rtxPayload, ppBuffer,
                                         pBufferLength, pLocalMediaDescription);
    } else if (TRANSCEIVER_IS_CODEC_ENABLED(pTransceiver->codecBitMap,
                                            TRANSCEIVER_RTC_CODEC_ALAW_BIT)) {
      ret = PopulateCodecAttributesAlaw(pRemoteMediaDescription, pTransceiver,
                                        payload, rtxPayload, ppBuffer,
                                        pBufferLength, pLocalMediaDescription);
    } else if (TRANSCEIVER_IS_CODEC_ENABLED(pTransceiver->codecBitMap,
                                            TRANSCEIVER_RTC_CODEC_H265_BIT)) {
      ret = PopulateCodecAttributesH265(pRemoteMediaDescription, pTransceiver,
                                        payload, rtxPayload, ppBuffer,
                                        pBufferLength, pLocalMediaDescription);
    } else {
      /* TODO: Unknown, no matching codec. */
      LogError(("Codec is not supported, codec bit map: %x",
                (int)pTransceiver->codecBitMap));
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ret = PopulateRtcpFb(payload, populateConfiguration.twccExtId, ppBuffer,
                         pBufferLength, pLocalMediaDescription);
  }
...
FindAttributeName(const SdpControllerAttributes_t *pAttributes,
  ...
  if ((pAttributes == NULL) || (pPattern == NULL)) {
    LogError(("Invalid input, pAttributes: %p, pPattern: %p", pAttributes,
              pPattern));
  } else {
    for (i = 0; i < attributeCount; i++) {
      if ((pAttributes[i].attributeNameLength == patternLength) &&
          (strncmp(pAttributes[i].pAttributeName, pPattern, patternLength) ==
           0)) {
        pRet = &pAttributes[i];
        break;
      }
...
FindFmtpBasedOnCodec(const SdpControllerAttributes_t *pAttributes,
  ...
  if (pAttributes == NULL) {
    ...
    written = snprintf(codecString, TRANSCEIVER_CODEC_STRING_MAX_LENGTH + 1,
                       "%u", codec);
    if (written < 0) {
      LogError(("snprintf return unexpected value %d", written));
      ...
      for (i = 0; i < attributeCount; i++) {
        if ((pAttributes[i].attributeNameLength ==
             SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_FMTP_LENGTH) &&
            (strncmp(pAttributes[i].pAttributeName,
                     SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_FMTP,
                     SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_FMTP_LENGTH) == 0) &&
            (pAttributes[i].attributeValueLength >= written) &&
            (strncmp(pAttributes[i].pAttributeValue, codecString, written) ==
             0)) {
          pRet = &pAttributes[i];
          break;
        }
...
static SdpControllerAttributes_t *
MatchAttributesValuePrefix(SdpControllerAttributes_t *pAttributes,
                           size_t attributeNum, const char *pPattern,
  ...
  if ((pAttributes == NULL) || (pPattern == NULL)) {
    ...
    for (i = 0; i < attributeNum; i++) {
      if (((pAttributes + i)->attributeValueLength >= patternLength) &&
          (strncmp((pAttributes + i)->pAttributeValue, pPattern,
                   patternLength) == 0)) {
...
static int
AddSessionAttributeGroup(char *pBuffer, size_t remainSize,
  ...
  if ((pBuffer == NULL) || (pLocalSdpDescription == NULL)) {
    totalWritten = -1;
    LogError(("Invalid input"));
  }
  ...
  if (totalWritten >= 0) {
    ...
    if (written < 0) {
      ...
      pLocalSdpDescription
          ->attributes[pLocalSdpDescription->sessionAttributesCount]
          .pAttributeName = pCurBuffer;
      pLocalSdpDescription
          ->attributes[pLocalSdpDescription->sessionAttributesCount]
          .attributeNameLength = strlen(pCurBuffer);
  ...
  if (totalWritten >= 0) {
    ...
    if (pRemoteSdpDescription != NULL) {
      pRemoteAttribute = MatchAttributesValuePrefix(
          pRemoteSdpDescription->attributes,
          SDP_CONTROLLER_MAX_SDP_ATTRIBUTES_COUNT, "BUNDLE", strlen("BUNDLE"));
    }
    ...
    if (pRemoteAttribute != NULL) {
      ...
      if (written < 0) {
        ...
        pLocalSdpDescription
            ->attributes[pLocalSdpDescription->sessionAttributesCount]
            .pAttributeValue = pCurBuffer;
        pLocalSdpDescription
            ->attributes[pLocalSdpDescription->sessionAttributesCount]
            .attributeValueLength = strlen(pCurBuffer);
  ...
  if ((totalWritten >= 0) && (pRemoteAttribute == NULL)) {
    ...
    if (written < 0) {
      ...
      for (i = 0; i < pLocalSdpDescription->mediaCount; i++) {
        written =
            snprintf(pAppendNumber + offset, remainSize - offset, " %d", i);
        if (written < 0) {
          totalWritten = -1;
          LogError(("snprintf return unexpected value %d", written));
          break;
...
static int AddSessionAttributeIceOptions(
    char *pBuffer, size_t remainSize,
    SdpControllerSdpDescription_t *pLocalSdpDescription,
    SdpControllerSdpDescription_t *pRemoteSdpDescription) {
  int totalWritten = 0;
  int written = 0;
...
static int AddSessionAttributeMsidSemantic(
    char *pBuffer, size_t remainSize,
    SdpControllerSdpDescription_t *pLocalSdpDescription,
    SdpControllerSdpDescription_t *pRemoteSdpDescription) {
  int totalWritten = 0;
...
static SdpControllerResult_t PopulateSessionAttributes(
    SdpControllerSdpDescription_t *pRemoteSdpDescription,
    SdpControllerPopulateSessionConfiguration_t populateConfiguration,
    SdpControllerSdpDescription_t *pLocalSdpDescription, char **ppBuffer,
    size_t *pBufferLength) {
  SdpControllerResult_t ret = SDP_CONTROLLER_RESULT_OK;
  int written;
  ...
  if ((ppBuffer == NULL) || (*ppBuffer == NULL) || (pBufferLength == NULL) ||
      (pLocalSdpDescription == NULL)) {
    LogError(("Invalid input, ppBuffer: %p, pBufferLength: %p, "
              "pLocalSdpDescription: %p",
              ppBuffer, pBufferLength, pLocalSdpDescription));
    ret = SDP_CONTROLLER_RESULT_BAD_PARAMETER;
  }
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ...
    written = AddSessionAttributeGroup(
        pCurBuffer, remainSize, pLocalSdpDescription, pRemoteSdpDescription);
    if (written < 0) {
      ret = SDP_CONTROLLER_RESULT_SDP_FAIL_ADD_SESSION_ATTRIBUTE_GROUP;
      LogError(
          ("Fail to add group to session attribute with return %d", written));
    } else {
      pCurBuffer += written;
      remainSize -= written;
    }
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ...
    if (populateConfiguration.canTrickleIce != 0U) {
      written = AddSessionAttributeIceOptions(
          pCurBuffer, remainSize, pLocalSdpDescription, pRemoteSdpDescription);
      if (written < 0) {
        ret = SDP_CONTROLLER_RESULT_SDP_FAIL_ADD_SESSION_ATTRIBUTE_ICE_OPTIONS;
        LogError(("Fail to add ice-options to session attribute with return %d",
                  written));
      } else {
        pCurBuffer += written;
        remainSize -= written;
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ...
    written = AddSessionAttributeMsidSemantic(
        pCurBuffer, remainSize, pLocalSdpDescription, pRemoteSdpDescription);
    if (written < 0) {
      ret = SDP_CONTROLLER_RESULT_SDP_FAIL_ADD_SESSION_ATTRIBUTE_MSID_SEMANTIC;
      LogError(("Fail to add msid-semantic to session attribute with return %d",
                written));
    } else {
      pCurBuffer += written;
      remainSize -= written;
...
SdpController_DeserializeSdpOffer(const char *pSdpOfferContent,
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ...
    sdpResult =
        SdpDeserializer_Init(&ctx, pSdpOfferContent, sdpOfferContentLength);
    if (sdpResult != SDP_RESULT_OK) {
  ...
  while (sdpResult == SDP_RESULT_OK) {
    ...
    if (sdpResult != SDP_RESULT_OK) {
      ...
      pOffer->mediaDescriptions[pOffer->mediaCount].mediaNameLength =
          valueLength;
      pOffer->mediaCount++;
      ...
      if (type == SDP_TYPE_ATTRIBUTE) {
        ...
        pOffer->mediaDescriptions[pOffer->mediaCount - 1].mediaTitleLength =
            valueLength;
      } else {
        /* Do nothing. */
      }
      ...
      if (type == SDP_TYPE_SESSION_NAME) {
        ...
        // Version
        stringResult = StringUtils_ConvertStringToUl(pValue, valueLength,
                                                     &pOffer->version);
        if (stringResult != STRING_UTILS_RESULT_OK) {
          LogError(("StringUtils_ConvertStringToUl fail, result %d, converting "
                    "%.*s to %u",
                    stringResult, (int)valueLength, pValue, pOffer->version));
...
SdpControllerResult_t SdpController_SerializeSdpMessageByDescription(
    SdpControllerMessageType_t messageType,
  ...
  if ((pSdpDescription == NULL) || (pOutputSerializedSdpMessage == NULL) ||
      (pOutputSerializedSdpMessageLength == NULL)) {
    LogError(
        ("Invalid input, pSdpDescription: %p, pOutputSerializedSdpMessage: %p, "
         "pOutputSerializedSdpMessageLength: %p",
         pSdpDescription, pOutputSerializedSdpMessage,
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ...
    written = snprintf(
        pCurrentOutput, remainSize, SDP_CONTROLLER_MESSAGE_TEMPLATE_HEAD,
        messageType == SDP_CONTROLLER_MESSAGE_TYPE_OFFER ? "offer" : "answer");
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    remainSize = *pOutputSerializedSdpMessageLength - outputBufferWrittenSize;
    written = snprintf(pCurrentOutput, remainSize,
                       SDP_CONTROLLER_MESSAGE_TEMPLATE_TAIL);
...
static SdpControllerResult_t
PopulateSessionOrigin(char **ppBuffer, size_t *pBufferLength,
                      SdpControllerOrigin_t *pOrigin) {
  SdpControllerResult_t ret = SDP_CONTROLLER_RESULT_OK;
  ...
  pOrigin->sdpConnectionInformation.pNetworkType =
      SDP_CONTROLLER_ORIGIN_DEFAULT_NET_TYPE;
  pOrigin->sdpConnectionInformation.networkTypeLength =
      strlen(SDP_CONTROLLER_ORIGIN_DEFAULT_NET_TYPE);
  pOrigin->sdpConnectionInformation.pAddressType =
      SDP_CONTROLLER_ORIGIN_IPV4_TYPE;
  pOrigin->sdpConnectionInformation.addressTypeLength =
      strlen(SDP_CONTROLLER_ORIGIN_IPV4_TYPE);
  pOrigin->sdpConnectionInformation.pConnectionAddress =
      SDP_CONTROLLER_ORIGIN_DEFAULT_IP_ADDRESS;
  pOrigin->sdpConnectionInformation.connectionAddressLength =
      strlen(SDP_CONTROLLER_ORIGIN_DEFAULT_IP_ADDRESS);

  return ret;
}

SdpControllerResult_t SdpController_PopulateSingleMedia(
    SdpControllerMediaDescription_t *pRemoteMediaDescription,
    SdpControllerPopulateMediaConfiguration_t populateConfiguration,
    SdpControllerMediaDescription_t *pLocalMediaDescription,
    uint32_t currentMediaIdx, char **ppBuffer, size_t *pBufferLength,
  ...
  if ((pLocalMediaDescription == NULL) || (ppBuffer == NULL) ||
      (pBufferLength == NULL)) {
    LogError(("Invalid input, pLocalMediaDescription: %p, ppBuffer: %p, "
              "pBufferLength: %p",
              pLocalMediaDescription, ppBuffer, pBufferLength));
    ret = SDP_CONTROLLER_RESULT_BAD_PARAMETER;
    ...
    LogError(
        ("Invalid input, pCname: %p, pLocalFingerprint: %p, pPassword: %p, "
         "pTransceiver: %p, pUserName: %p, trackKind: %d",
         populateConfiguration.pCname, populateConfiguration.pLocalFingerprint,
         populateConfiguration.pPassword, populateConfiguration.pTransceiver,
         populateConfiguration.pUserName, trackKind));
    ret = SDP_CONTROLLER_RESULT_BAD_PARAMETER;
  } else {
    /* Empty else marker. */
  }
  ...
  /* Set media name. */
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    /* We support only one payload type, so only one payload type printed in
     * media name. */
    switch (trackKind) {
    case TRANSCEIVER_TRACK_KIND_VIDEO: {
      if (populateConfiguration.rtxPayloadType == 0) {
        written =
            snprintf(pCurBuffer, remainSize, "video 9 UDP/TLS/RTP/SAVPF %u",
                     populateConfiguration.payloadType);
      } else {
        written =
            snprintf(pCurBuffer, remainSize, "video 9 UDP/TLS/RTP/SAVPF %u %u",
                     populateConfiguration.payloadType,
                     populateConfiguration.rtxPayloadType);
      }
      break;
    }
    case TRANSCEIVER_TRACK_KIND_AUDIO: {
      if (populateConfiguration.rtxPayloadType == 0) {
        written =
            snprintf(pCurBuffer, remainSize, "audio 9 UDP/TLS/RTP/SAVPF %u",
                     populateConfiguration.payloadType);
      } else {
        written =
            snprintf(pCurBuffer, remainSize, "audio 9 UDP/TLS/RTP/SAVPF %u %u",
                     populateConfiguration.payloadType,
                     populateConfiguration.rtxPayloadType);
      }
      break;
    ...
    case TRANSCEIVER_TRACK_KIND_DATA_CHANNEL: {
      written = snprintf(pCurBuffer, remainSize, "%s",
                         SDP_CONTROLLER_DATA_CHANNEL_ATTRIBUTE_NAME_MEDIA_NAME);
      break;
    }
    /* Since the range of trackKind is checked above the
     * TRANSCEIVER_TRACK_KIND_UNKNOWN and default case is not required to be
     * handled. The labels are kept for keeping the compiler happy wrt
     * -Werror=switch */
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ...
    pLocalMediaDescription->connectionInformation.pNetworkType =
        SDP_CONTROLLER_ORIGIN_DEFAULT_NET_TYPE;
    pLocalMediaDescription->connectionInformation.networkTypeLength =
        strlen(SDP_CONTROLLER_ORIGIN_DEFAULT_NET_TYPE);
    pLocalMediaDescription->connectionInformation.pAddressType =
        SDP_CONTROLLER_ORIGIN_IPV4_TYPE;
    pLocalMediaDescription->connectionInformation.addressTypeLength =
        strlen(SDP_CONTROLLER_ORIGIN_IPV4_TYPE);
    pLocalMediaDescription->connectionInformation.pConnectionAddress =
        SDP_CONTROLLER_ORIGIN_DEFAULT_IP_ADDRESS;
    pLocalMediaDescription->connectionInformation.connectionAddressLength =
        strlen(SDP_CONTROLLER_ORIGIN_DEFAULT_IP_ADDRESS);
  }
  ...
  if ((ret == SDP_CONTROLLER_RESULT_OK) &&
    ...
    pTargetAttribute->attributeNameLength =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_MSID_LENGTH;

    if (populateConfiguration.rtxPayloadType == 0) {
      written =
          snprintf(pCurBuffer, remainSize, "%.*s %.*s",
                   (int)populateConfiguration.pTransceiver->streamIdLength,
                   populateConfiguration.pTransceiver->streamId,
                   (int)populateConfiguration.pTransceiver->trackIdLength,
                   populateConfiguration.pTransceiver->trackId);
    } else {
      written =
          snprintf(pCurBuffer, remainSize, "%.*s %.*sRTX",
                   (int)populateConfiguration.pTransceiver->streamIdLength,
                   populateConfiguration.pTransceiver->streamId,
                   (int)populateConfiguration.pTransceiver->trackIdLength,
                   populateConfiguration.pTransceiver->trackId);
  ...
  if ((ret == SDP_CONTROLLER_RESULT_OK) &&
      (trackKind != TRANSCEIVER_TRACK_KIND_DATA_CHANNEL) &&
      (populateConfiguration.rtxPayloadType != 0)) {
    /* msid */
    pTargetAttribute =
        &pLocalMediaDescription->attributes[*pTargetAttributeCount];
    pTargetAttribute->pAttributeName =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_SSRC_GROUP;
    pTargetAttribute->attributeNameLength =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_SSRC_GROUP_LENGTH;

    written = snprintf(pCurBuffer, remainSize, "FID %u %u",
  ...
  if ((ret == SDP_CONTROLLER_RESULT_OK) &&
      (trackKind != TRANSCEIVER_TRACK_KIND_DATA_CHANNEL)) {
    ret = PopulateTransceiverSsrc(
        &pCurBuffer, &remainSize, pLocalMediaDescription,
        populateConfiguration.pTransceiver, populateConfiguration.pCname,
        populateConfiguration.cnameLength,
        populateConfiguration.rtxPayloadType == 0 ? 0 : 1);
  }
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ...
    pTargetAttribute->attributeNameLength =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_RTCP_LENGTH;
    pTargetAttribute->pAttributeValue =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_RTCP;
    pTargetAttribute->attributeValueLength =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_VALUE_RTCP_LENGTH;
    *pTargetAttributeCount += 1;

    pTargetAttribute =
        &pLocalMediaDescription->attributes[*pTargetAttributeCount];
    pTargetAttribute->pAttributeName =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_ICE_UFRAG;
    pTargetAttribute->attributeNameLength =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_ICE_UFRAG_LENGTH;
    pTargetAttribute->pAttributeValue = populateConfiguration.pUserName;
    pTargetAttribute->attributeValueLength =
        populateConfiguration.userNameLength;
    *pTargetAttributeCount += 1;

    pTargetAttribute =
        &pLocalMediaDescription->attributes[*pTargetAttributeCount];
    pTargetAttribute->pAttributeName =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_ICE_PWD;
    pTargetAttribute->attributeNameLength =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_ICE_PWD_LENGTH;
    pTargetAttribute->pAttributeValue = populateConfiguration.pPassword;
    pTargetAttribute->attributeValueLength =
        populateConfiguration.passwordLength;
    *pTargetAttributeCount += 1;
  }
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    pTargetAttribute =
        &pLocalMediaDescription->attributes[*pTargetAttributeCount];
    pTargetAttribute->pAttributeName =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_FINGERPRINT;
    pTargetAttribute->attributeNameLength =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_FINGERPRINT_LENGTH;

    written = snprintf(pCurBuffer, remainSize, "sha-256 %.*s",
                       (int)populateConfiguration.localFingerprintLength,
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    pTargetAttribute =
        &pLocalMediaDescription->attributes[*pTargetAttributeCount];
    pTargetAttribute->pAttributeName =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_SETUP;
    pTargetAttribute->attributeNameLength =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_SETUP_LENGTH;

    if (populateConfiguration.isOffer) {
      pTargetAttribute->pAttributeValue =
          SDP_CONTROLLER_MEDIA_DTLS_ROLE_ACTPASS;
      pTargetAttribute->attributeValueLength =
          SDP_CONTROLLER_MEDIA_DTLS_ROLE_ACTPASS_LENGTH;
    } else {
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ...
    if (populateConfiguration.isOffer == 0) {
      ...
      pSourceAttribute =
          FindAttributeName(pRemoteMediaDescription->attributes,
    ...
    if (pSourceAttribute != NULL) {
      ...
      pTargetAttribute->attributeValueLength =
          pSourceAttribute->attributeValueLength;
      *pTargetAttributeCount += 1;
    } else {
  ...
  if ((ret == SDP_CONTROLLER_RESULT_OK) &&
      (trackKind != TRANSCEIVER_TRACK_KIND_DATA_CHANNEL)) {
    TransceiverDirection_t targetDirection =
        TRANSCEIVER_TRACK_DIRECTION_UNKNOWN;
    ...
    if (populateConfiguration.isOffer != 0) {
      ...
      // in case of a missing m-line, we respond with the same m-line but
      // direction set to inactive
      if (populateConfiguration.pTransceiver->direction ==
        ...
        for (i = 0; i < pRemoteMediaDescription->mediaAttributesCount; i++) {
          if ((pRemoteMediaDescription->attributes[i].attributeNameLength ==
               SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_SENDRECV_LENGTH) &&
              (strncmp(pRemoteMediaDescription->attributes[i].pAttributeName,
                       SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_SENDRECV,
                       SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_SENDRECV_LENGTH) ==
               0)) {
            targetDirection = TRANSCEIVER_TRACK_DIRECTION_SENDRECV;
            break;
          } else if (
              (pRemoteMediaDescription->attributes[i].attributeNameLength ==
               SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_SENDONLY_LENGTH) &&
              (strncmp(pRemoteMediaDescription->attributes[i].pAttributeName,
                       SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_SENDONLY,
                       SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_SENDONLY_LENGTH) ==
               0)) {
            ...
            break;
              ...
              (pRemoteMediaDescription->attributes[i].attributeNameLength ==
               SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_RECVONLY_LENGTH) &&
              (strncmp(pRemoteMediaDescription->attributes[i].pAttributeName,
                       SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_RECVONLY,
                       SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_RECVONLY_LENGTH) ==
               0)) {
            ...
            break;
              ...
              (pRemoteMediaDescription->attributes[i].attributeNameLength ==
               SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_INACTIVE_LENGTH) &&
              (strncmp(pRemoteMediaDescription->attributes[i].pAttributeName,
                       SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_INACTIVE,
                       SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_INACTIVE_LENGTH) ==
               0)) {
            targetDirection = TRANSCEIVER_TRACK_DIRECTION_INACTIVE;
            break;
    ...
    default:
      ...
      LogWarn(("Incorrect/no transceiver direction set...this attribute will "
               "be set to inactive, target: %d",
               targetDirection));
      pTargetAttribute->pAttributeName =
          SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_INACTIVE;
      pTargetAttribute->attributeNameLength =
  ...
  if ((ret == SDP_CONTROLLER_RESULT_OK) &&
    ...
    pTargetAttribute =
        &pLocalMediaDescription->attributes[*pTargetAttributeCount];
    pTargetAttribute->pAttributeName =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_RTCP_MUX;
    pTargetAttribute->attributeNameLength =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_RTCP_MUX_LENGTH;
    pTargetAttribute->pAttributeValue = NULL;
    pTargetAttribute->attributeValueLength = 0;
    ...
    *pTargetAttributeCount += 1;
    ...
    pTargetAttribute =
        &pLocalMediaDescription->attributes[*pTargetAttributeCount];
    pTargetAttribute->pAttributeName =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_RTCP_RSIZE;
    pTargetAttribute->attributeNameLength =
        SDP_CONTROLLER_MEDIA_ATTRIBUTE_NAME_RTCP_RSIZE_LENGTH;
    pTargetAttribute->pAttributeValue = NULL;
    pTargetAttribute->attributeValueLength = 0;
  ...
  if ((ret == SDP_CONTROLLER_RESULT_OK) &&
    ...
    ret = PopulateCodecAttributes(pRemoteMediaDescription,
                                  populateConfiguration, &pCurBuffer,
                                  &remainSize, pLocalMediaDescription);
  }
  ...
  if ((ret == SDP_CONTROLLER_RESULT_OK) &&
    ...
    pTargetAttribute =
        &pLocalMediaDescription->attributes[*pTargetAttributeCount];
    pTargetAttribute->pAttributeName =
        SDP_CONTROLLER_DATA_CHANNEL_ATTRIBUTE_NAME_SCTP_PORT;
    pTargetAttribute->attributeNameLength =
        SDP_CONTROLLER_DATA_CHANNEL_ATTRIBUTE_NAME_SCTP_PORT_LENGTH;
    pTargetAttribute->pAttributeValue =
        SDP_CONTROLLER_DATA_CHANNEL_ATTRIBUTE_VALUE_SCTP_PORT;
    pTargetAttribute->attributeValueLength =
        SDP_CONTROLLER_DATA_CHANNEL_ATTRIBUTE_VALUE_SCTP_PORT_LENGTH;
    *pTargetAttributeCount += 1;
  }
...
SdpControllerResult_t SdpController_PopulateSessionDescription(
    SdpControllerSdpDescription_t *pRemoteSessionDescription,
    SdpControllerPopulateSessionConfiguration_t populateConfiguration,
    SdpControllerSdpDescription_t *pLocalSessionDescription, char **ppBuffer,
    size_t *pBufferLength) {
  SdpControllerResult_t ret = SDP_CONTROLLER_RESULT_OK;

  if ((pLocalSessionDescription == NULL) || (ppBuffer == NULL) ||
      (pBufferLength == NULL)) {
    LogError(("Invalid input, pLocalSessionDescription: %p, ppBuffer: %p, "
              "pBufferLength: %p",
              pLocalSessionDescription, ppBuffer, pBufferLength));
    ret = SDP_CONTROLLER_RESULT_BAD_PARAMETER;
  }
  ...
  if (ret == SDP_CONTROLLER_RESULT_OK) {
    ...
    /* Session origin. */
    ret = PopulateSessionOrigin(ppBuffer, pBufferLength,
                                &pLocalSessionDescription->origin);
  }

  if (ret == SDP_CONTROLLER_RESULT_OK) {
    /* Session name. */
    pLocalSessionDescription->pSessionName =
        SDP_CONTROLLER_ORIGIN_DEFAULT_SESSION_NAME;
    pLocalSessionDescription->sessionNameLength =
        strlen(SDP_CONTROLLER_ORIGIN_DEFAULT_SESSION_NAME);

    /* Session timing description. */
    pLocalSessionDescription->timingDescription.startTime =
        populateConfiguration.timingDescription.startTime;
    pLocalSessionDescription->timingDescription.stopTime =
        populateConfiguration.timingDescription.stopTime;

    ret = PopulateSessionAttributes(
        pRemoteSessionDescription, populateConfiguration,
        pLocalSessionDescription, ppBuffer, pBufferLength);
  }
