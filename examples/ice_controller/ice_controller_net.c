...
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_BINDING_REQUEST                \
  "BINDING_REQUEST"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_BINDING_SUCCESS                \
  "BINDING_SUCCESS_RESPONSE"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_BINDING_FAILURE                \
  "BINDING_FAILURE_RESPONSE"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_BINDING_INDICATION             \
  "BINDING_INDICATION"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_ALLOCATE_REQUEST               \
  "ALLOCATE_REQUEST"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_ALLOCATE_SUCCESS               \
  "ALLOCATE_SUCCESS_RESPONSE"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_ALLOCATE_FAILURE               \
  "ALLOCATE_FAILURE_RESPONSE"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_REFRESH_REQUEST                \
  "REFRESH_REQUEST"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_REFRESH_SUCCESS                \
  "REFRESH_SUCCESS_RESPONSE"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_REFRESH_FAILURE                \
  "REFRESH_FAILURE_RESPONSE"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_CREATE_PERMISSION_REQUEST      \
  "CREATE_PERMISSION_REQUEST"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_CREATE_PERMISSION_SUCCESS      \
  "CREATE_PERMISSION_SUCCESS_RESPONSE"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_CREATE_PERMISSION_FAILURE      \
  "CREATE_PERMISSION_FAILURE_RESPONSE"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_CHANNEL_BIND_REQUEST           \
  "CHANNEL_BIND_REQUEST"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_CHANNEL_BIND_SUCCESS           \
  "CHANNEL_BIND_SUCCESS_RESPONSE"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_CHANNEL_BIND_FAILURE           \
  "CHANNEL_BIND_FAILURE_RESPONSE"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_SEND_INDICATION                \
  "SEND_INDICATION"
#define ICE_CONTROLLER_STUN_MESSAGE_TYPE_STRING_DATA_INDICATION                \
  "DATA_INDICATION"
...
static void GetLocalIPAdresses(IceEndpoint_t *pLocalIpAddresses,
  ...
  for (pIfAddr = pIfAddrs;
       pIfAddr && localIpAddressesNum < localIpAddressesSize;
    ...
    {
      pLocalIpAddresses[localIpAddressesNum].transportAddress.family =
          STUN_ADDRESS_IPv4;
      ...
      memcpy(pLocalIpAddresses[localIpAddressesNum].transportAddress.address,
             &pIpv4Addr->sin_addr, STUN_IPV4_ADDRESS_SIZE);

      /* When the host is using VPN, the IFF_POINTOPOINT flag of the interface
       * is set. */
      pLocalIpAddresses[localIpAddressesNum].isPointToPoint =
          ((pIfAddr->ifa_flags & IFF_POINTOPOINT) != 0);
      localIpAddressesNum++;
    } else if (pIfAddr->ifa_addr &&
               (pIfAddr->ifa_addr->sa_family == AF_INET6)) {
      /* TODO: skip IPv6 for now. */
      // getnameinfo( pIfAddr->ifa_addr, sizeof(struct sockaddr_in6), ipv6Addr,
      // sizeof(ipv6Addr), NULL, 0, NI_NUMERICHOST ); pLocalIpAddresses[
      // localIpAddressesNum ].transportAddress.family = STUN_ADDRESS_IPv6;
      // pLocalIpAddresses[ localIpAddressesNum ].transportAddress.port = 0;
      // pIpv6Addr = ( struct sockaddr_in6* ) pIfAddr->ifa_addr;
      // memcpy( pLocalIpAddresses[ localIpAddressesNum
      // ].transportAddress.address , &pIpv6Addr->sin6_addr,
      // STUN_IPV6_ADDRESS_SIZE ); pLocalIpAddresses[ localIpAddressesNum
      // ].isPointToPoint = ( ( pIfAddr->ifa_flags & IFF_POINTOPOINT ) != 0 );
      // localIpAddressesNum++;
    }
  }
...
void IceControllerNet_UpdateSocketContext(
    IceControllerContext_t *pCtx, IceControllerSocketContext_t *pSocketContext,
    IceControllerSocketContextState_t newState, IceCandidate_t *pLocalCandidate,
    IceCandidate_t *pRemoteCandidate, IceControllerIceServer_t *pIceServer) {
  IceControllerResult_t ret = ICE_CONTROLLER_RESULT_OK;

  if ((pCtx == NULL) || (pSocketContext == NULL)) {
    LogError(
        ("Invalid input, pCtx: %p, pSocketContext: %p, pLocalCandidate: %p",
         pCtx, pSocketContext, pLocalCandidate));
...
static IceControllerResult_t CreateSocketContextUdp(
    IceControllerContext_t *pCtx, uint16_t family, IceEndpoint_t *pBindEndpoint,
    IceEndpoint_t *pConnectEndpoint, IceSocketProtocol_t protocol,
  ...
  if (pCtx->socketsContextsCount < ICE_CONTROLLER_MAX_LOCAL_CANDIDATE_COUNT) {
    ...
    LogWarn(
        ("No socket context available for ice controller. Current number: %lu",
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    pSocketContext->socketFd =
        socket(family == STUN_ADDRESS_IPv4 ? AF_INET : AF_INET6, SOCK_DGRAM, 0);

    if (pSocketContext->socketFd == -1) {
      LogError(
          ("socket() failed to create socket with errno: %s", strerror(errno)));
  ...
  if ((ret == ICE_CONTROLLER_RESULT_OK) && needBinding) {
    if (pBindEndpoint->transportAddress.family == STUN_ADDRESS_IPv4) {
      ...
      memcpy(&ipv4Address.sin_addr, pBindEndpoint->transportAddress.address,
      ...
      // memcpy(&ipv6Addr.sin6_addr, pBindEndpoint->transportAddress.address,
      // STUN_IPV4_ADDRESS_SIZE); pSockAddress = (struct sockaddr*) &ipv6Addr;
  ...
  if ((ret == ICE_CONTROLLER_RESULT_OK) && needBinding) {
    if (bind(pSocketContext->socketFd, pSockAddress, addressLength) < 0) {
      LogError(
          ("socket() failed to bind socket with errno: %s", strerror(errno)));
  ...
  if ((ret == ICE_CONTROLLER_RESULT_OK) && needBinding) {
    if (getsockname(pSocketContext->socketFd, pSockAddress, &addressLength) <
        0) {
      ...
      pBindEndpoint->transportAddress.port =
          (uint16_t)pBindEndpoint->transportAddress.family == STUN_ADDRESS_IPv4
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    setsockopt(pSocketContext->socketFd, SOL_SOCKET, SO_SNDBUF, &sendBufferSize,
               sizeof(sendBufferSize));
    setsockopt(pSocketContext->socketFd, SOL_SOCKET, SO_RCVTIMEO, &tv,
               sizeof(struct timeval));
    setsockopt(pSocketContext->socketFd, SOL_SOCKET, SO_SNDTIMEO, &tv,
               sizeof(struct timeval));
  }
...
static IceControllerResult_t CreateSocketContextTcp(
    IceControllerContext_t *pCtx, uint16_t family, IceEndpoint_t *pBindEndpoint,
    IceEndpoint_t *pConnectEndpoint, IceSocketProtocol_t protocol,
    IceControllerSocketContext_t **ppOutSocketContext) {
  IceControllerResult_t ret = ICE_CONTROLLER_RESULT_OK;
  ...
  pRemoteIpPos = inet_ntop(AF_INET, pConnectEndpoint->transportAddress.address,
                           remoteIpAddr, INET_ADDRSTRLEN);
  LogInfo(("Start TLS handshaking with %s:%d",
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    ...
    pSocketContext->tlsSession.xTlsNetworkContext.pParams =
        &pSocketContext->tlsSession.xTlsTransportParams;

    LogInfo(("Establishing a TLS session with %s:%d.", pRemoteIpPos,
             pConnectEndpoint->transportAddress.port));

    /* Attempt to create a server-authenticated TLS connection. */
    xNetworkStatus = TLS_FreeRTOS_Connect(
        &pSocketContext->tlsSession.xTlsNetworkContext, pRemoteIpPos,
        pConnectEndpoint->transportAddress.port, &credentials, 0U, 0U,
    ...
    if (xNetworkStatus == TLS_TRANSPORT_HANDSHAKE_IN_PROGRESS) {
      ...
      LogError(("Connection with TLS/TCP TURN server failed with return %d",
                xNetworkStatus));
      pCtx->socketsContextsCount--;
      pSocketContext->socketFd = -1;
  ...
  if ((ret == ICE_CONTROLLER_RESULT_OK) ||
      (ret == ICE_CONTROLLER_RESULT_CONNECTION_IN_PROGRESS)) {
    pSocketContext->socketFd = TLS_FreeRTOS_GetSocketFd(
...
static IceControllerResult_t CreateSocketContext(
    IceControllerContext_t *pCtx, uint16_t family, IceEndpoint_t *pBindEndpoint,
    IceEndpoint_t *pConnectEndpoint, IceSocketProtocol_t protocol,
    IceControllerSocketContext_t **ppOutSocketContext) {
  IceControllerResult_t ret = ICE_CONTROLLER_RESULT_OK;
  ...
  if ((pCtx == NULL) || (ppOutSocketContext == NULL)) {
    LogError(("Invalid input, pCtx: %p, ppOutSocketContext: %p", pCtx,
              ppOutSocketContext));
    ret = ICE_CONTROLLER_RESULT_BAD_PARAMETER;
  }
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    if (protocol == ICE_SOCKET_PROTOCOL_UDP) {
      ret =
          CreateSocketContextUdp(pCtx, family, pBindEndpoint, pConnectEndpoint,
                                 protocol, ppOutSocketContext);
    } else if (protocol == ICE_SOCKET_PROTOCOL_TCP) {
      ret =
          CreateSocketContextTcp(pCtx, family, pBindEndpoint, pConnectEndpoint,
                                 protocol, ppOutSocketContext);
    } else {
...
static IceControllerResult_t
SendSocketPacket(IceControllerSocketContext_t *pSocketContext,
                 const uint8_t *pBuffer, size_t length, int flags,
                 struct sockaddr *pDestinationAddress, socklen_t addressLength,
  ...
  while (sendTotalBytes < length) {
    if (pSocketContext->socketType == ICE_CONTROLLER_SOCKET_TYPE_UDP) {
      sentBytes = sendto(pSocketContext->socketFd, pBuffer + sendTotalBytes,
                         length - sendTotalBytes, flags, pDestinationAddress,
                         addressLength);
    } else if (pSocketContext->socketType == ICE_CONTROLLER_SOCKET_TYPE_TLS) {
      sentBytes =
          TLS_FreeRTOS_send(&pSocketContext->tlsSession.xTlsNetworkContext,
                            pBuffer + sendTotalBytes, length - sendTotalBytes);
    } else {
      /* The socket type is checked before invoking this function, so this
       * condition should never happen. */
      LogError(("Fail to send because of unknown socket type %d",
                pSocketContext->socketType));
    ...
    if (sentBytes < 0) {
      if ((errno == EAGAIN) || (errno == EWOULDBLOCK)) {
        ...
        if (ICE_CONTROLLER_RESEND_TIMEOUT_MS <= totalDelayMs) {
          LogWarn(("Fail to send before timeout: %dms",
                   ICE_CONTROLLER_RESEND_TIMEOUT_MS));
          ret = ICE_CONTROLLER_RESULT_FAIL_SOCKET_SENDTO;
          break;
        }
      } else {
        LogWarn(("Failed to send to socket fd: %d error, errno(%d): %s",
                 pSocketContext->socketFd, errno, strerror(errno)));
        LogVerbose(
            ("Source family: %d, IP:port: %s:%u",
             pSocketContext->pLocalCandidate->endpoint.transportAddress.family,
             IceControllerNet_LogIpAddressInfo(
                 &pSocketContext->pLocalCandidate->endpoint, ipBuffer,
                 sizeof(ipBuffer)),
        ...
        LogVerbose(("Dest family: %d, IP:port: %s:%u",
                    ...
                    IceControllerNet_LogIpAddressInfo(
                        pDestinationEndpoint, ipBuffer, sizeof(ipBuffer)),
...
void IceControllerNet_FreeSocketContext(
    IceControllerContext_t *pCtx,
  ...
  if (pSocketContext && (pSocketContext->socketFd != -1)) {
    if (pthread_mutex_lock(&(pCtx->socketMutex)) == 0) {
      if (pSocketContext->socketType == ICE_CONTROLLER_SOCKET_TYPE_TLS) {
        retTlsTransport = TLS_FreeRTOS_Disconnect(
            &pSocketContext->tlsSession.xTlsNetworkContext);
        if (retTlsTransport != TLS_TRANSPORT_SUCCESS) {
          LogWarn(("Fail to disconnect TLS session with return %d",
                   retTlsTransport));
        }
      }
...
static void AddHostCandidate(IceControllerContext_t *pCtx,
  ...
  ret = CreateSocketContext(pCtx, pLocalIceEndpoint->transportAddress.family,
                            pLocalIceEndpoint, NULL, ICE_SOCKET_PROTOCOL_UDP,
                            &pSocketContext);
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    pCandidate =
        &(pCtx->iceContext
              .pLocalCandidates[pCtx->iceContext.numLocalCandidates - 1]);
    if (pCtx->onIceEventCallbackFunc) {
      localCandidateReadyContent.iceControllerCallbackContent
          .localCandidateReadyMsg.pLocalCandidate = pCandidate;
      localCandidateReadyContent.iceControllerCallbackContent
          .localCandidateReadyMsg.localCandidateIndex =
          pCtx->candidateFoundationCounter;
      retLocalCandidateReady = pCtx->onIceEventCallbackFunc(
          pCtx->pOnIceEventCustomContext,
          ICE_CONTROLLER_CB_EVENT_LOCAL_CANDIDATE_READY,
          &localCandidateReadyContent);
      if (retLocalCandidateReady == 0) {
        ...
        IceControllerNet_FreeSocketContext(pCtx, pSocketContext);
        LogError(
            ("Fail to send local candidate, ret: %d.", retLocalCandidateReady));
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    IceControllerNet_UpdateSocketContext(
        pCtx, pSocketContext, ICE_CONTROLLER_SOCKET_CONTEXT_STATE_READY,
        pCandidate, NULL, NULL);

    LogInfo(("Created host candidate with fd %d, ID: 0x%04x",
             pSocketContext->socketFd, pCandidate->candidateId));
    LogVerbose(("host candidate's local IP/port: %s/%d",
                IceControllerNet_LogIpAddressInfo(pLocalIceEndpoint, ipBuffer,
                                                  sizeof(ipBuffer)),
...
static void AddSrflxCandidate(IceControllerContext_t *pCtx,
  ...
  for (i = 0; i < pCtx->iceServersCount; i++) {
    ...
    if (pCtx->iceServers[i].serverType != ICE_CONTROLLER_ICE_SERVER_TYPE_STUN) {
      ...
      continue;
    } else if (pCtx->iceServers[i].iceEndpoint.transportAddress.family !=
               STUN_ADDRESS_IPv4) {
    ...
    if ((pCtx->iceServers[i].iceEndpoint.transportAddress.family ==
         STUN_ADDRESS_IPv4) &&
        (pLocalIceEndpoint->transportAddress.family ==
         pCtx->iceServers[i].iceEndpoint.transportAddress.family)) {
      ret = CreateSocketContext(
          pCtx, pLocalIceEndpoint->transportAddress.family, pLocalIceEndpoint,
          NULL, ICE_SOCKET_PROTOCOL_UDP, &pSocketContext);
    }
    ...
    if (ret == ICE_CONTROLLER_RESULT_OK) {
      if (pthread_mutex_lock(&(pCtx->iceMutex)) == 0) {
        ...
        if (iceResult != ICE_RESULT_OK) {
          /* Free resource that already created. */
          LogError(
              ("Ice_AddServerReflexiveCandidate fail, result: %d", iceResult));
          IceControllerNet_FreeSocketContext(pCtx, pSocketContext);
          ret = ICE_CONTROLLER_RESULT_FAIL_ADD_HOST_CANDIDATE;
          break;
        }
      } else {
        LogError(("Failed to add server reflexive candidate: mutex lock "
                  "acquisition."));
        ret = ICE_CONTROLLER_RESULT_FAIL_MUTEX_TAKE;
      }
    }
    ...
    if (ret == ICE_CONTROLLER_RESULT_OK) {
      IceControllerNet_UpdateSocketContext(
          pCtx, pSocketContext, ICE_CONTROLLER_SOCKET_CONTEXT_STATE_CREATE,
          &pCtx->iceContext
               .pLocalCandidates[pCtx->iceContext.numLocalCandidates - 1],
          NULL, &pCtx->iceServers[i]);
...
static void AddRelayCandidates(IceControllerContext_t *pCtx) {
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    /* Loop through all ICE server configs and start allocate TURN with UDP and
     * TLS TURN servers. */
    for (i = 0; i < pCtx->iceServersCount; i++) {
      /* Reset ret for every round. */
      ret = ICE_CONTROLLER_RESULT_OK;
      ...
      if (pCtx->iceServers[i].iceEndpoint.transportAddress.family !=
        ...
        continue;
      } else if ((pCtx->iceServers[i].serverType !=
        ...
        LogInfo(("Unknown TURN Server, protocol: %d, Server URL: %.*s",
                 ...
                 (int)pCtx->iceServers[i].urlLength, pCtx->iceServers[i].url));
        continue;
      } else if ((pCtx->iceServers[i].protocol == ICE_SOCKET_PROTOCOL_UDP) &&
                 (pCtx->iceServers[i].serverType !=
                  ICE_CONTROLLER_ICE_SERVER_TYPE_TURN)) {
        /* For now we do not support DTLS connection over TURN server. */
        LogInfo(("Only pure UDP TURN server is supported, serverType: %d, "
                 "Server URL: %.*s",
                 pCtx->iceServers[i].serverType,
                 (int)pCtx->iceServers[i].urlLength, pCtx->iceServers[i].url));
        continue;
        ...
        LogInfo(("Only TLS/TCP TURN server is supported, serverType: %d, "
                 "Server URL: %.*s",
                 pCtx->iceServers[i].serverType,
                 (int)pCtx->iceServers[i].urlLength, pCtx->iceServers[i].url));
        continue;
      ...
      ret = CreateSocketContext(pCtx, STUN_ADDRESS_IPv4, NULL,
                                &pCtx->iceServers[i].iceEndpoint,
                                pCtx->iceServers[i].protocol, &pSocketContext);

      if (ret == ICE_CONTROLLER_RESULT_OK) {
        if (pthread_mutex_lock(&(pCtx->iceMutex)) == 0) {
          iceResult = Ice_AddRelayCandidate(
              &pCtx->iceContext, &pCtx->iceServers[i].iceEndpoint,
              pCtx->iceServers[i].userName, pCtx->iceServers[i].userNameLength,
              pCtx->iceServers[i].password, pCtx->iceServers[i].passwordLength);
          pthread_mutex_unlock(&(pCtx->iceMutex));
      ...
      if (ret == ICE_CONTROLLER_RESULT_OK) {
        ...
        LogInfo(("Created relay candidate with fd %d, ID: 0x%04x",
                 ...
                 pCtx->iceContext
                     ...
                     .candidateId));
        LogVerbose(
            ("relay candidate's local IP/port: %s/%d",
             IceControllerNet_LogIpAddressInfo(&pCtx->iceServers[i].iceEndpoint,
        ...
        IceControllerNet_UpdateSocketContext(
            pCtx, pSocketContext,
            ICE_CONTROLLER_SOCKET_CONTEXT_STATE_CONNECTION_IN_PROGRESS, NULL,
            NULL, &(pCtx->iceServers[i]));

        LogVerbose(
            ("Connection in-progress with TURN server for socket fd %d...",
             pSocketContext->socketFd));
...
static IceControllerResult_t SendBindingResponse(
    IceControllerContext_t *pCtx, IceControllerSocketContext_t *pSocketContext,
    IceCandidatePair_t *pCandidatePair, uint8_t *pTransactionIdBuffer) {
  IceControllerResult_t ret = ICE_CONTROLLER_RESULT_OK;
  IceResult_t iceResult;
  ...
  if (pthread_mutex_lock(&(pCtx->iceMutex)) == 0) {
    iceResult = Ice_CreateResponseForRequest(
        &pCtx->iceContext, pCandidatePair, pTransactionIdBuffer, sentStunBuffer,
        &sentStunBufferLength);
    pthread_mutex_unlock(&(pCtx->iceMutex));

    if (iceResult != ICE_RESULT_OK) {
      LogWarn(
          ("Unable to create STUN binding response, result: %d", iceResult));
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    ...
    if (pSocketContext->pLocalCandidate->candidateType ==
    ...
    if (IceControllerNet_SendPacket(pCtx, pSocketContext, pDestEndpoint,
                                    sentStunBuffer, sentStunBufferLength) !=
        ICE_CONTROLLER_RESULT_OK) {
      LogWarn(("Unable to send STUN response for nomination"));
      ret = ICE_CONTROLLER_RESULT_FAIL_SEND_BIND_RESPONSE;
    } else {
      LogDebug(("Sending STUN bind response back to remote, local/remote "
                "candidate ID: 0x%04x / 0x%04x",
...
static IceControllerResult_t
CheckNomination(IceControllerContext_t *pCtx,
                IceControllerSocketContext_t *pSocketContext,
  ...
  if ((pCtx == NULL) || (pSocketContext == NULL) || (pCandidatePair == NULL)) {
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    if ((pCandidatePair->state == ICE_CANDIDATE_PAIR_STATE_SUCCEEDED) &&
...
#endif
      LogInfo(
          ("Found nomination pair, local/remote candidate ID: 0x%04x / 0x%04x",
           pCandidatePair->pLocalCandidate->candidateId,
           pCandidatePair->pRemoteCandidate->candidateId));

      LogVerbose(
          ("Candidiate pair is nominated, local IP/port: %s/%u, remote "
           "IP/port: %s/%u",
           IceControllerNet_LogIpAddressInfo(
               &pCandidatePair->pLocalCandidate->endpoint, ipBuffer,
               sizeof(ipBuffer)),
           pCandidatePair->pLocalCandidate->endpoint.transportAddress.port,
           IceControllerNet_LogIpAddressInfo(
               &pCandidatePair->pRemoteCandidate->endpoint, ipBuffer2,
               sizeof(ipBuffer2)),
      ...
      if (pthread_mutex_lock(&(pCtx->socketMutex)) == 0) {
        ...
        pCtx->pNominatedSocketContext->pRemoteCandidate =
            pCandidatePair->pRemoteCandidate;
...
IceControllerNet_ConvertIpString(const char *pIpAddr, size_t ipAddrLength,
  ...
  if (ipAddrLength > ICE_CONTROLLER_IP_ADDR_STRING_BUFFER_LENGTH) {
    LogWarn(
        ("invalid IP address detected, IP: %.*s", (int)ipAddrLength, pIpAddr));
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    ...
    if (inet_pton(AF_INET, ipAddress,
                  pDestinationIceEndpoint->transportAddress.address) == 1) {
      pDestinationIceEndpoint->transportAddress.family = STUN_ADDRESS_IPv4;
    } else if (inet_pton(AF_INET6, ipAddress,
...
IceControllerResult_t
IceControllerNet_SendPacket(IceControllerContext_t *pCtx,
                            IceControllerSocketContext_t *pSocketContext,
  ...
  if ((pCtx == NULL) || (pSocketContext == NULL) || (pRemoteEndpoint == NULL) ||
      (pBuffer == NULL)) {
    LogError(("Invalid input, pCtx: %p, pSocketContext: %p, pRemoteEndpoint: "
              "%p, pBuffer: %p",
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    /* Set socket destination address, including IP type (v4/v6), IP address and
     * port. */
    if (pSocketContext->pLocalCandidate->endpoint.transportAddress.family !=
        pRemoteEndpoint->transportAddress.family) {
      LogWarn((
          "The sending IP family: %d is different from receiving IP family: %d",
          pSocketContext->pLocalCandidate->endpoint.transportAddress.family,
          pRemoteEndpoint->transportAddress.family));
      ret = ICE_CONTROLLER_RESULT_FAIL_SOCKET_SENDTO;
    }
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    if (pRemoteEndpoint->transportAddress.family == STUN_ADDRESS_IPv4) {
      ...
      memcpy(&ipv4Address.sin_addr, pRemoteEndpoint->transportAddress.address,
             STUN_IPV4_ADDRESS_SIZE);
      ...
      memcpy(&ipv6Address.sin6_addr, pRemoteEndpoint->transportAddress.address,
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    if ((pSocketContext->socketType == ICE_CONTROLLER_SOCKET_TYPE_UDP) ||
        (pSocketContext->socketType == ICE_CONTROLLER_SOCKET_TYPE_TLS)) {
      ret =
          SendSocketPacket(pSocketContext, pBuffer, bufferLength, 0,
                           pDestinationAddress, addressLength, pRemoteEndpoint);
    } else {
      LogError(("Internal error, invalid socket type %d",
                pSocketContext->socketType));
  ...
  if (ret == ICE_CONTROLLER_RESULT_FAIL_SOCKET_SENDTO) {
     ...
     * Action required: Close the local socket to properly terminate the
     * connection.
     */
    (void)Ice_CloseCandidate(&pCtx->iceContext,
    ...
    if (pSocketContext == pCtx->pNominatedSocketContext) {
      ...
      LogWarn(("Unable to send packet through nominated socket, closing "
               "session: %.*s",
...
void IceControllerNet_AddLocalCandidates(IceControllerContext_t *pCtx) {
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    ...
    for (i = 0; i < pCtx->localIceEndpointsCount; i++) {
      if (ICE_CONTROLLER_IS_NAT_CONFIG_SET(
              pCtx, ICE_CANDIDATE_NAT_TRAVERSAL_CONFIG_SEND_HOST)) {
#if METRIC_PRINT_ENABLED
      ...
      if (ICE_CONTROLLER_IS_NAT_CONFIG_SET(
              pCtx, ICE_CANDIDATE_NAT_TRAVERSAL_CONFIG_SEND_SRFLX)) {
#if METRIC_PRINT_ENABLED
    ...
    if (ICE_CONTROLLER_IS_NAT_CONFIG_SET(
            pCtx, ICE_CANDIDATE_NAT_TRAVERSAL_CONFIG_SEND_RELAY)) {
#if METRIC_PRINT_ENABLED
...
IceControllerResult_t IceControllerNet_ExecuteTlsHandshake(
    IceControllerContext_t *pCtx, IceControllerSocketContext_t *pSocketContext,
  ...
  if ((pCtx == NULL) || (pSocketContext == NULL)) {
    LogError(
        ("Invalid input, pCtx: %p, pSocketContext: %p", pCtx, pSocketContext));
    ret = ICE_CONTROLLER_RESULT_BAD_PARAMETER;
  } else if (pSocketContext->socketType != ICE_CONTROLLER_SOCKET_TYPE_TLS) {
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    if (pthread_mutex_lock(&(pCtx->socketMutex)) == 0) {
      transportResult = TLS_FreeRTOS_ContinueHandshake(
          &(pSocketContext->tlsSession.xTlsNetworkContext));
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    if (transportResult == TLS_TRANSPORT_SUCCESS) {
      LogVerbose(("Connection with TURN server successful for socket fd %d",
                  pSocketContext->socketFd));

      if ((isIceLockTakenBeforeCall != 0U) ||
          (pthread_mutex_lock(&(pCtx->iceMutex)) == 0)) {
        iceResult = Ice_AddRelayCandidate(
            &(pCtx->iceContext), &(pSocketContext->pIceServer->iceEndpoint),
      ...
      if (ret == ICE_CONTROLLER_RESULT_OK) {
        if (iceResult != ICE_RESULT_OK) {
          ...
          LogInfo(
              ("Created relay candidate with fd %d, ID: 0x%04x",
               pSocketContext->socketFd,
               pCtx->iceContext
                   .pLocalCandidates[pCtx->iceContext.numLocalCandidates - 1]
                   .candidateId));
          ...
          IceControllerNet_UpdateSocketContext(
              ...
              &(pCtx->iceContext
                    .pLocalCandidates[pCtx->iceContext.numLocalCandidates - 1]),
              NULL, pSocketContext->pIceServer);
        }
      }
    } else if (transportResult == TLS_TRANSPORT_HANDSHAKE_IN_PROGRESS) {
      LogVerbose(
          ("Connection still in-progress with TURN server for socket fd %d...",
           pSocketContext->socketFd));
    } else {
      LogError(("Connection with TURN server failed for socket fd %d",
                pSocketContext->socketFd));
      IceControllerNet_FreeSocketContext(pCtx, pSocketContext);
    }
  }
...
IceControllerResult_t IceControllerNet_HandleStunPacket(
    IceControllerContext_t *pCtx, IceControllerSocketContext_t *pSocketContext,
    uint8_t *pReceiveBuffer, size_t receiveBufferLength,
    IceEndpoint_t *pRemoteIceEndpoint, IceCandidatePair_t *pCandidatePair) {
  IceControllerResult_t ret = ICE_CONTROLLER_RESULT_OK;
  ...
  if ((pCtx == NULL) || (pReceiveBuffer == NULL) ||
      (pRemoteIceEndpoint == NULL)) {
    LogError(
        ("Invalid input, pCtx: %p, pReceiveBuffer: %p, pRemoteIceEndpoint: %p",
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    if (pthread_mutex_lock(&(pCtx->iceMutex)) == 0) {
      iceHandleStunResult = Ice_HandleStunPacket(
          &pCtx->iceContext, pReceiveBuffer, (size_t)receiveBufferLength,
          pSocketContext->pLocalCandidate, pRemoteIceEndpoint,
          currentTimeSeconds, &pTransactionIdBuffer, &pCandidatePair);
      pthread_mutex_unlock(&(pCtx->iceMutex));
    } else {
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    if (iceHandleStunResult != ICE_HANDLE_STUN_PACKET_RESULT_NOT_STUN_PACKET) {
      ...
      if (pCandidatePair != NULL) {
        LogDebug((
            "Receiving STUN packet, local/remote candidate ID: 0x%04x / 0x%04x",
            pCandidatePair->pLocalCandidate->candidateId,
            pCandidatePair->pRemoteCandidate->candidateId));
      }
    }
    ...
    case ICE_HANDLE_STUN_PACKET_RESULT_UPDATED_SERVER_REFLEXIVE_CANDIDATE_ADDRESS:
      if (pCtx->onIceEventCallbackFunc) {
        ...
        IceControllerNet_UpdateSocketContext(
            pCtx, pSocketContext, ICE_CONTROLLER_SOCKET_CONTEXT_STATE_READY,
            pSocketContext->pLocalCandidate, pSocketContext->pRemoteCandidate,
            pSocketContext->pIceServer);

        localCandidateReadyContent.iceControllerCallbackContent
            .localCandidateReadyMsg.pLocalCandidate =
            pSocketContext->pLocalCandidate;
        localCandidateReadyContent.iceControllerCallbackContent
            .localCandidateReadyMsg.localCandidateIndex =
            pCtx->candidateFoundationCounter;
        ...
        if (retLocalCandidateReady == 0) {
          ...
          /* Free resource that already created. */
          LogWarn(("Fail to send server reflexive candidate to remote peer, "
                   "ret: %d.",
                   retLocalCandidateReady));
        }
      } else {
    ...
    case ICE_HANDLE_STUN_PACKET_RESULT_UPDATED_RELAY_CANDIDATE_ADDRESS:
      if (pCtx->onIceEventCallbackFunc) {
        ...
        if (retLocalCandidateReady == 0) {
          ...
          /* Free resource that already created. */
          LogWarn(("Fail to send relay candidate to remote peer, ret: %d.",
                   retLocalCandidateReady));
        }
    ...
    case ICE_HANDLE_STUN_PACKET_RESULT_SEND_RESPONSE_FOR_REMOTE_REQUEST:
      ret = SendBindingResponse(pCtx, pSocketContext, pCandidatePair,
                                pTransactionIdBuffer);

      if (ret == ICE_CONTROLLER_RESULT_OK) {
        ret = CheckNomination(pCtx, pSocketContext, pCandidatePair);
      }
      break;
    case ICE_HANDLE_STUN_PACKET_RESULT_SEND_CHANNEL_BIND_REQUEST:
      if (pthread_mutex_lock(&(pCtx->iceMutex)) == 0) {
        iceResult = Ice_CreateNextPairRequest(
            &pCtx->iceContext, pCandidatePair, currentTimeSeconds,
            sentStunBuffer, &sentStunBufferLength);
        pthread_mutex_unlock(&(pCtx->iceMutex));

        if (iceResult != ICE_RESULT_OK) {
          LogWarn(("Unable to create channel binding message, result: %d",
                   iceResult));
        } else {
          LogDebug(("Sending channel binding request, local/remote candidate "
                    "ID: 0x%04x / 0x%04x",
                    pCandidatePair->pLocalCandidate->candidateId,
                    pCandidatePair->pRemoteCandidate->candidateId));
          IceControllerNet_LogStunPacket(sentStunBuffer, sentStunBufferLength);

          if (IceControllerNet_SendPacket(
                  pCtx, pSocketContext,
                  &(pSocketContext->pIceServer->iceEndpoint), sentStunBuffer,
                  sentStunBufferLength) != ICE_CONTROLLER_RESULT_OK) {
            LogWarn(("Unable to send channel binding message"));
          }
        }
      } else {
        LogError(("Failed to create channel binding request: mutex lock "
                  "acquisition."));
        ret = ICE_CONTROLLER_RESULT_FAIL_MUTEX_TAKE;
      }
      break;
    case ICE_HANDLE_STUN_PACKET_RESULT_SEND_CONNECTIVITY_CHECK_REQUEST:
      if (pthread_mutex_lock(&(pCtx->iceMutex)) == 0) {
        ...
        if (iceResult != ICE_RESULT_OK) {
          LogWarn(("Unable to STUN binding request  message, result: %d",
                   iceResult));
        } else {
          LogDebug(("Sending STUN binding request, local/remote candidate ID: "
                    "0x%04x / 0x%04x",
                    pCandidatePair->pLocalCandidate->candidateId,
                    pCandidatePair->pRemoteCandidate->candidateId));
          IceControllerNet_LogStunPacket(sentStunBuffer, sentStunBufferLength);
        ...
        LogError(("Failed to create connectivity check binding request: mutex "
                  "lock acquisition."));
        ret = ICE_CONTROLLER_RESULT_FAIL_MUTEX_TAKE;
      }
      break;
    ...
    case ICE_HANDLE_STUN_PACKET_RESULT_CANDIDATE_PAIR_NOT_FOUND:
      LogDebug(("Valid Candidate Pair is not found, it might be a duplicate "
                "response, local candidate ID: 0x%04x",
                pSocketContext->pLocalCandidate->candidateId));
      break;
    case ICE_HANDLE_STUN_PACKET_RESULT_CANDIDATE_NOT_FOUND:
      LogError(("Error : Valid Server Reflexive Candidate is not found, local "
                "candidate ID: 0x%04x",
                pSocketContext->pLocalCandidate->candidateId));
      break;
    ...
    case ICE_HANDLE_STUN_PACKET_RESULT_SEND_ALLOCATION_REQUEST:
      /* Received TURN allocation error response, get the nonce/realm from the
       * message. Send the TURN allocation request again. */
      if (pthread_mutex_lock(&(pCtx->iceMutex)) == 0) {
        sentStunBufferLength = ICE_CONTROLLER_STUN_MESSAGE_BUFFER_SIZE;
        iceResult = Ice_CreateNextCandidateRequest(
            &pCtx->iceContext, pSocketContext->pLocalCandidate,
            currentTimeSeconds, sentStunBuffer, &sentStunBufferLength);
        pthread_mutex_unlock(&(pCtx->iceMutex));

        if (iceResult == ICE_RESULT_OK) {
          LogDebug(
              ("Sending TURN allocation request, local candidate ID: 0x%04x",
               pSocketContext->pLocalCandidate->candidateId));
          IceControllerNet_LogStunPacket(sentStunBuffer, sentStunBufferLength);
          ...
          LogWarn(("Not able to create candidate request with return: %d",
                   iceResult));
        }
      } else {
        LogError(
            ("Failed to create allocation request: mutex lock acquisition."));
        ret = ICE_CONTROLLER_RESULT_FAIL_MUTEX_TAKE;
      }
    ...
    case ICE_HANDLE_STUN_PACKET_RESULT_UNEXPECTED_RESPONSE:
      LogDebug((
          "Unexpected response. pair state is %d, local candidate ID: 0x%04x",
          pCandidatePair->state, pSocketContext->pLocalCandidate->candidateId));
      break;
    ...
    case ICE_HANDLE_STUN_PACKET_RESULT_MATCHING_TRANSACTION_ID_NOT_FOUND:
      LogVerbose(
          ("Transaction ID not matching, might be a duplicate response"));
      break;
    case ICE_HANDLE_STUN_PACKET_RESULT_FRESH_CHANNEL_BIND_COMPLETE:
      LogVerbose(("Channel binding success response, this might be a duplicate "
                  "response"));
      break;
    default:
      LogWarn(("Unknown case: %d, packet length: %lu, first two bytes: 0x%02x "
               "0x%02x",
               iceHandleStunResult, receiveBufferLength, pReceiveBuffer[0],
               pReceiveBuffer[1]));
      break;
    }
  }
...
IceControllerResult_t
IceControllerNet_DnsLookUp(char *pUrl,
  ...
  if (ret == ICE_CONTROLLER_RESULT_OK) {
    for (pIterator = pResult; pIterator; pIterator = pIterator->ai_next) {
      if (pIterator->ai_family == AF_INET) {
        ...
        memcpy(pIceTransportAddress->address, &ipv4Address->sin_addr,
               STUN_IPV4_ADDRESS_SIZE);
        break;
        ...
        memcpy(pIceTransportAddress->address, &ipv6Address->sin6_addr,
               STUN_IPV6_ADDRESS_SIZE);
        break;
      }
    }
...
const char *IceControllerNet_LogIpAddressInfo(const IceEndpoint_t *pIceEndpoint,
  ...
  if ((pIceEndpoint != NULL) && (pIpBuffer != NULL)) {
    ...
    case STUN_ADDRESS_IPv4:
      ret = inet_ntop(AF_INET, pIceEndpoint->transportAddress.address,
                      pIpBuffer, ipBufferLength);
      if (ret == NULL) {
        /*
         * IP address to string conversion failed due to insufficient buffer
         * size. Required: IPv4 (16 bytes), IPv6 (46 bytes)
         */
        ret = "Masked IPv4 Address";
      }
      break;
    case STUN_ADDRESS_IPv6:
      ret = inet_ntop(AF_INET6, pIceEndpoint->transportAddress.address,
                      pIpBuffer, ipBufferLength);
      if (ret == NULL) {
        /*
         * IP address to string conversion failed due to insufficient buffer
...
#define SWAP_BYTES_16(value) ((((value) >> 8) & 0xFF) | (((value) & 0xFF) << 8))
...
void IceControllerNet_LogStunPacket(uint8_t *pStunPacket,
  ...
  IceControllerStunMsgHeader_t *pStunMsgHeader =
      (IceControllerStunMsgHeader_t *)pStunMsgContent;

  if ((pStunPacket == NULL) ||
      (stunPacketSize < sizeof(IceControllerStunMsgHeader_t))) {
    // invalid STUN packet, ignore it
  } else {
    do {
      if ((pStunPacket[0] & 0xF0) == 0x40) {
        LogVerbose(("TURN channel number: 0x%02x%02x, TURN application data "
                    "length: 0x%02x%02x",
                    pStunPacket[0], pStunPacket[1], pStunPacket[2],
                    pStunPacket[3]));
        pStunMsgContent =
            &pStunPacket[ICE_TURN_CHANNEL_DATA_MESSAGE_HEADER_LENGTH];
        pStunMsgHeader = (IceControllerStunMsgHeader_t *)pStunMsgContent;
        if (stunPacketSize < sizeof(IceControllerStunMsgHeader_t) +
                                 ICE_TURN_CHANNEL_DATA_MESSAGE_HEADER_LENGTH) {
          // invalid STUN packet, ignore it.
      ...
      if (pStunMsgContent[0] < 2) {
        LogVerbose(
            ("Dumping STUN packets: STUN type: %s, content length:: "
             "0x%02x%02x, transaction ID: "
             "0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
             convertStunMsgTypeToString(pStunMsgHeader->msgType),
             pStunMsgHeader->contentLength[0], pStunMsgHeader->contentLength[1],
             pStunMsgHeader->transactionId[0], pStunMsgHeader->transactionId[1],
             pStunMsgHeader->transactionId[2], pStunMsgHeader->transactionId[3],
             pStunMsgHeader->transactionId[4], pStunMsgHeader->transactionId[5],
             pStunMsgHeader->transactionId[6], pStunMsgHeader->transactionId[7],
             pStunMsgHeader->transactionId[8], pStunMsgHeader->transactionId[9],
             pStunMsgHeader->transactionId[10],
             pStunMsgHeader->transactionId[11]));
      }
    } while (0U);
  }
#endif /* #if LIBRARY_LOG_LEVEL >= LOG_VERBOSE  */
