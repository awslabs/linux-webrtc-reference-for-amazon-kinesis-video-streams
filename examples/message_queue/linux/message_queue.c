...
void MessageQueue_Destroy(MessageQueueHandler_t *pMessageQueueHandler,
  ...
  if (pMessageQueueHandler != NULL) {
    if (mq_close(pMessageQueueHandler->messageQueue) == -1) {
      LogError(("mq_close error, errno: %s, queue name: %s", strerror(errno),
                pMessageQueueHandler->pQueueName));
    }

    if (mq_unlink(pMessageQueueHandler->pQueueName) == -1) {
      LogError(("mq_unlink error, errno: %s, queue name: %s", strerror(errno),
                pMessageQueueHandler->pQueueName));
    }
  } else {
...
MessageQueue_Create(MessageQueueHandler_t *pMessageQueueHandler,
                    const char *pQueueName, size_t messageMaxLength,
  ...
  if (ret == MESSAGE_QUEUE_RESULT_OK) {
    ...
    pMessageQueueHandler->messageQueue =
        mq_open(pQueueName, O_RDWR | O_CREAT, 0666, &attr);
...
MessageQueueResult_t
MessageQueue_Send(MessageQueueHandler_t *pMessageQueueHandler, void *pMessage,
  ...
  if (ret == MESSAGE_QUEUE_RESULT_OK) {
    if (mq_send(pMessageQueueHandler->messageQueue, pMessage, messageLength,
                0) == -1) {
...
MessageQueueResult_t
MessageQueue_Recv(MessageQueueHandler_t *pMessageQueueHandler, void *pMessage,
  ...
  if (ret == MESSAGE_QUEUE_RESULT_OK) {
    recvLength = mq_receive(pMessageQueueHandler->messageQueue, pMessage,
                            *pMessageLength, &recvPriority);
    if (recvLength == -1) {
...
MessageQueueResult_t
MessageQueue_IsEmpty(MessageQueueHandler_t *pMessageQueueHandler) {
  MessageQueueResult_t ret = MESSAGE_QUEUE_RESULT_OK;
  struct mq_attr attr;
...
MessageQueueResult_t
MessageQueue_IsFull(MessageQueueHandler_t *pMessageQueueHandler) {
  MessageQueueResult_t ret = MESSAGE_QUEUE_RESULT_OK;
  struct mq_attr attr;
...
MessageQueueResult_t
MessageQueue_AttachPoll(MessageQueueHandler_t *pMessageQueueHandler,
                        struct pollfd *pPollFd, uint32_t PollEvents) {
  MessageQueueResult_t ret = MESSAGE_QUEUE_RESULT_OK;
