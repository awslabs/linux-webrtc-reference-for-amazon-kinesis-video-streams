...
mbedtls_ssl_srtp_profile DTLS_SRTP_SUPPORTED_PROFILES[] = {
     ...
     MBEDTLS_VERSION_NUMBER == 0x03020100)
 ...
 */
static DtlsTransportStatus_t
dtlsSetup(DtlsNetworkContext_t *pDtlsNetworkContext,
          DtlsNetworkCredentials_t *pNetworkCredentials);
 ...
 */
static DtlsTransportStatus_t
initMbedtls(mbedtls_entropy_context *pEntropyContext,
            mbedtls_ctr_drbg_context *pCtrDrbgContext);

/*-----------------------------------------------------------*/

#ifdef MBEDTLS_DEBUG_C
void dtls_mbedtls_string_printf(void *sslContext, int level, const char *file,
...
static int DtlsUdpSendWrap(void *pCustomCtx, const unsigned char *pBuf,
                           size_t len) {
  int ret = 0;
  DtlsTransportParams_t *pDtlsTransportParams =
      (DtlsTransportParams_t *)pCustomCtx;
  ...
  if (ret == 0) {
    if (pDtlsTransportParams->onDtlsSendHook != NULL) {
      ret = pDtlsTransportParams->onDtlsSendHook(
          pDtlsTransportParams->pOnDtlsSendCustomContext, pBuf, len);
    } else {
...
static int DtlsUdpRecvWrap(void *pCustomCtx, unsigned char *pBuf, size_t len) {
  int ret = 0;
  DtlsTransportParams_t *pDtlsTransportParams =
      (DtlsTransportParams_t *)pCustomCtx;
  ...
  if (ret == 0) {
    ...
    memcpy(pBuf,
           pDtlsTransportParams->pReceivedPacket +
               pDtlsTransportParams->receivedPacketOffset,
           ret);

    if (ret + pDtlsTransportParams->receivedPacketOffset >=
        pDtlsTransportParams->receivedPacketLength) {
...
static void DtlsSslContextInit(DtlsSSLContext_t *pSslContext) {
  ...
  mbedtls_ssl_conf_dbg(&(pSslContext->config), dtls_mbedtls_string_printf,
                       NULL);
#endif /* MBEDTLS_DEBUG_C */
}
/*-----------------------------------------------------------*/
...
int dtlsSessionKeyDerivationCallback(
    void *customData, const unsigned char *pMasterSecret,
    const unsigned char *pKeyBlock, size_t maclen, size_t keylen, size_t ivlen,
  ...
  memcpy(pKeys->randBytes + MAX_DTLS_RANDOM_BYTES_LEN, serverRandom,
...
static int32_t setCredentials(DtlsSSLContext_t *pSslContext,
  ...
  mbedtls_ssl_conf_rng(&(pSslContext->config), mbedtls_ctr_drbg_random,
    ...
    if (pNetworkCredentials->pPrivateKey != NULL) {
      if (mbedtlsError == 0) {
        mbedtlsError = mbedtls_ssl_conf_own_cert(
            &(pSslContext->config), pNetworkCredentials->pClientCert,
      ...
      if (mbedtlsError == 0) {
        mbedtlsError = mbedtls_ssl_conf_dtls_srtp_protection_profiles(
            &pSslContext->config, DTLS_SRTP_SUPPORTED_PROFILES);
        if (mbedtlsError != 0) {
...
static DtlsTransportStatus_t
dtlsSetup(DtlsNetworkContext_t *pNetworkContext,
          DtlsNetworkCredentials_t *pNetworkCredentials) {
  ...
  mbedtlsError = mbedtls_ssl_config_defaults(
      &(pDtlsTransportParams->dtlsSslContext.config), MBEDTLS_SSL_IS_CLIENT,
  ...
  if (returnStatus == DTLS_SUCCESS) {
    ...
    mbedtlsError =
        mbedtls_ssl_setup(&(pDtlsTransportParams->dtlsSslContext.context),
                          &(pDtlsTransportParams->dtlsSslContext.config));

    if (mbedtlsError != 0) {
      LogError((
          "Failed to set up mbed DTLS SSL context: mbedTLSError=-0x%x %s : %s.",
          mbedtlsError, mbedtlsHighLevelCodeOrDefault(mbedtlsError),
          mbedtlsLowLevelCodeOrDefault(mbedtlsError)));
...
initMbedtls(mbedtls_entropy_context *pEntropyContext,
  ...
  if (returnStatus == DTLS_SUCCESS) {
    ...
    if (mbedtlsError != PSA_SUCCESS) {
      LogError(("Failed to initialize PSA Crypto implementation: %d",
                (int)mbedtlsError));
      returnStatus = DTLS_TRANSPORT_INTERNAL_ERROR;
    }
  }
  ...
  if (returnStatus == DTLS_SUCCESS) {
    ...
    mbedtlsError = mbedtls_ctr_drbg_seed(pCtrDrbgContext, mbedtls_entropy_func,
...
void DTLS_Disconnect(DtlsNetworkContext_t *pNetworkContext) {
  ...
  if ((pNetworkContext != NULL) && (pNetworkContext->pParams != NULL)) {
    ...
    dtlsStatus = (int32_t)mbedtls_ssl_close_notify(
        &(pDtlsTransportParams->dtlsSslContext.context));

    /* Ignore the WANT_READ and WANT_WRITE return values. */
    if ((dtlsStatus != (int32_t)MBEDTLS_ERR_SSL_WANT_READ) &&
        (dtlsStatus != (int32_t)MBEDTLS_ERR_SSL_WANT_WRITE)) {
      if (dtlsStatus == 0) {
        ...
        LogError(("(Network connection %p) Failed to send DTLS close-notify: "
                  "mbedTLSError= %s : %s.",
                  pNetworkContext, mbedtlsHighLevelCodeOrDefault(dtlsStatus),
      ...
      LogInfo(("(Network connection %p) TLS close-notify sent; "
               ...
               pNetworkContext,
               (dtlsStatus == MBEDTLS_ERR_SSL_WANT_READ) ? "WANT_READ"
                                                         : "WANT_WRITE"));
    }
...
int32_t DTLS_Send(DtlsNetworkContext_t *pNetworkContext, const void *pBuffer,
                  size_t bytesToSend) {
  DtlsTransportParams_t *pDtlsTransportParams = NULL;
  int32_t dtlsStatus = 0;
  ...
  if ((pNetworkContext == NULL) || (pNetworkContext->pParams == NULL)) {
    ...
    dtlsStatus = (int32_t)mbedtls_ssl_write(
        &(pDtlsTransportParams->dtlsSslContext.context), pBuffer, bytesToSend);

    if ((dtlsStatus == MBEDTLS_ERR_SSL_TIMEOUT) ||
        (dtlsStatus == MBEDTLS_ERR_SSL_WANT_READ) ||
        (dtlsStatus == MBEDTLS_ERR_SSL_WANT_WRITE)) {
...
int32_t dtlsFillPseudoRandomBits(uint8_t *pBuf, size_t bufSize) {
  int32_t retStatus = 0;
  uint32_t i;

  if ((bufSize >= DTLS_CERT_MIN_SERIAL_NUM_SIZE) &&
...
int32_t DTLS_CreateCertificateFingerprint(const mbedtls_x509_crt *pCert,
                                          char *pBuff, const size_t bufLen) {
  int32_t retStatus = 0;
  ...
  if (retStatus == 0) {
    ...
    if (sslRet != 0) {
      LogError(
          ("Failed to calculate the SHA-256 checksum: mbedTLSError= %s : %s.",
  ...
  if (retStatus == 0) {
    ...
    if (bufLen < 3 * size) {
      LogError(("buffer to store fingerprint too small buffer: %lu size: %d",
                bufLen, size));
      retStatus = -1;
    } else {
      /* Empty else marker. */
...
int32_t DTLS_GetLocalCertificateFingerprint(DtlsSSLContext_t *pSslContext,
  ...
  DTLS_CreateCertificateFingerprint(&pSslContext->clientCert, pBuff, buffLen);
...
int32_t
DTLS_VerifyRemoteCertificateFingerprint(DtlsSSLContext_t *pSslContext,
  ...
  if ((pSslContext == NULL) || (pExpectedFingerprint == NULL) ||
      (CERTIFICATE_FINGERPRINT_LENGTH < fingerprintMaxLen)) {
    LogError(("invalid input, pSslContext || pExpectedFingerprint == NULL || "
              "CERTIFICATE_FINGERPRINT_LENGTH < fingerprintMaxLen(%lu)",
              fingerprintMaxLen));
    retStatus = -1;
  } else {
    /* Empty else marker. */
  ...
  pRemoteCertificate =
      (mbedtls_x509_crt *)mbedtls_ssl_get_peer_cert(&pSslContext->context);
  ...
  if (DTLS_CreateCertificateFingerprint(pRemoteCertificate, actualFingerprint,
  ...
  if (strncmp(pExpectedFingerprint, actualFingerprint, fingerprintMaxLen) !=
      0) {
    LogError(("DTLS_SSL_REMOTE_CERTIFICATE_VERIFICATION_FAILED \nexpected "
              "fingerprint:\n %s \nactual fingerprint:\n %s",
              pExpectedFingerprint, actualFingerprint));
...
int32_t DTLS_PopulateKeyingMaterial(DtlsSSLContext_t *pSslContext,
  ...
  TlsKeys *pKeys = NULL;
  uint8_t keyingMaterialBuffer[MAX_SRTP_MASTER_KEY_LEN * 2 +
                               MAX_SRTP_SALT_KEY_LEN * 2];
  ...
  retStatus = mbedtls_ssl_tls_prf(
      pKeys->tlsProfile, pKeys->masterSecret, ARRAY_SIZE(pKeys->masterSecret),
      KEYING_EXTRACTOR_LABEL, pKeys->randBytes, ARRAY_SIZE(pKeys->randBytes),
      keyingMaterialBuffer, ARRAY_SIZE(keyingMaterialBuffer));
  if (retStatus != 0) {
    LogError(("Failed TLS-PRF function for key derivation, funct: %d",
              pKeys->tlsProfile));
  ...
  if (retStatus == 0) {
    pDtlsKeyingMaterial->key_length =
        MAX_SRTP_MASTER_KEY_LEN + MAX_SRTP_SALT_KEY_LEN;

    memcpy(pDtlsKeyingMaterial->clientWriteKey, &keyingMaterialBuffer[offset],
           MAX_SRTP_MASTER_KEY_LEN);
    offset += MAX_SRTP_MASTER_KEY_LEN;

    memcpy(pDtlsKeyingMaterial->serverWriteKey, &keyingMaterialBuffer[offset],
           MAX_SRTP_MASTER_KEY_LEN);
    offset += MAX_SRTP_MASTER_KEY_LEN;

    memcpy(pDtlsKeyingMaterial->clientWriteKey + MAX_SRTP_MASTER_KEY_LEN,
           &keyingMaterialBuffer[offset], MAX_SRTP_SALT_KEY_LEN);
...
#else  /* #if ( MBEDTLS_VERSION_NUMBER > 0x02100600 ) */
    negotiatedSRTPProfile =
        mbedtls_ssl_get_dtls_srtp_protection_profile(&pSslContext->context);
    switch (negotiatedSRTPProfile) {
    case MBEDTLS_TLS_SRTP_AES128_CM_HMAC_SHA1_80:
#endif /* #if ( MBEDTLS_VERSION_NUMBER > 0x02100600 ) */
      pDtlsKeyingMaterial->srtpProfile =
          KVS_SRTP_PROFILE_AES128_CM_HMAC_SHA1_80;
      break;

#if (MBEDTLS_VERSION_NUMBER == 0x03000000 ||                                   \
     MBEDTLS_VERSION_NUMBER == 0x03020100)
    case MBEDTLS_TLS_SRTP_AES128_CM_HMAC_SHA1_32:
#else  /* #if ( MBEDTLS_VERSION_NUMBER == 0x03000000 || MBEDTLS_VERSION_NUMBER \
          == 0x03020100 ) */
    case MBEDTLS_TLS_SRTP_AES128_CM_HMAC_SHA1_32:
#endif /* #if ( MBEDTLS_VERSION_NUMBER == 0x03000000 || MBEDTLS_VERSION_NUMBER \
          == 0x03020100 ) */
      pDtlsKeyingMaterial->srtpProfile =
          KVS_SRTP_PROFILE_AES128_CM_HMAC_SHA1_32;
      break;
    default:
...
int32_t DTLS_CreateCertificateAndKey(int32_t certificateBits,
  ...
  char *pCertBuf = NULL;
  char notBeforeBuf[MBEDTLS_X509_RFC5280_UTC_TIME_LEN + 1],
      notAfterBuf[MBEDTLS_X509_RFC5280_UTC_TIME_LEN + 1];
  ...
  if ((pCert != NULL) && (pKey != NULL)) {
    if ((pCertBuf = (char *)malloc(GENERATED_CERTIFICATE_MAX_SIZE))) {
      if ((NULL != (pEntropy = (mbedtls_entropy_context *)malloc(
                        sizeof(mbedtls_entropy_context))))) {
        if ((NULL != (pCtrDrbg = (mbedtls_ctr_drbg_context *)malloc(
                          sizeof(mbedtls_ctr_drbg_context))))) {
          if ((NULL != (pWriteCert = (mbedtls_x509write_cert *)malloc(
                            sizeof(mbedtls_x509write_cert))))) {
            if (dtlsFillPseudoRandomBits(certSn, sizeof(certSn)) == 0) {
              ...
              initialized = 1;
              if (mbedtls_ctr_drbg_seed(pCtrDrbg, mbedtls_entropy_func,
                                        pEntropy, NULL, 0) == 0) {
                ...
                if (generateRSACertificate) {
                  LogWarn(
                      ("generateRSACertificate this will take about 10mins"));

                  if (mbedtls_pk_setup(pKey, mbedtls_pk_info_from_type(
                                                 MBEDTLS_PK_RSA)) == 0) {
                    LogDebug(("mbedtls_pk_setup successful"));
                    if (mbedtls_rsa_gen_key(
                            mbedtls_pk_rsa(*pKey), mbedtls_ctr_drbg_random,
                            pCtrDrbg, certificateBits, DTLS_RSA_F4) == 0) {
                ...
                } else // generate ECDSA
                {
                  ...
                  if ((mbedtls_pk_setup(pKey, mbedtls_pk_info_from_type(
                                                  MBEDTLS_PK_ECKEY)) == 0) &&
                      (mbedtls_ecp_gen_key(
                           MBEDTLS_ECP_DP_SECP256R1, mbedtls_pk_ec(*pKey),
                           mbedtls_ctr_drbg_random, pCtrDrbg) == 0)) {
                    LogDebug(
                        ("mbedtls_pk_setup && mbedtls_ecp_gen_key successful"));
                  } else {
                    retStatus = DTLS_GENERATE_KEY_FAILURE;
                    LogError(
                        ("mbedtls_pk_setup or mbedtls_ecp_gen_key failed"));
                  }
                }
              }
              ...
              int mbedtlsRet =
                  mbedtls_mpi_read_binary(&serial, certSn, sizeof(certSn));
              if (mbedtlsRet == 0) {
                ...
                if (strftime(notBeforeBuf, sizeof(notBeforeBuf), "%Y%m%d%H%M%S",
                             gmtime(&timeT)) !=
                    MBEDTLS_X509_RFC5280_UTC_TIME_LEN) {
                  LogDebug(("notBefore: %s", notBeforeBuf));

                  timeT = nowTime.tv_sec +
                          (GENERATED_CERTIFICATE_DAYS * DTLS_SECONDS_IN_A_DAY);
                  if (strftime(notAfterBuf, sizeof(notAfterBuf), "%Y%m%d%H%M%S",
                               gmtime(&timeT)) !=
                      MBEDTLS_X509_RFC5280_UTC_TIME_LEN) {
                    LogDebug(("notAfter: %s", notAfterBuf));

                    if (mbedtls_x509write_crt_set_serial(pWriteCert, &serial) ==
                        0) {
                      if (mbedtls_x509write_crt_set_validity(
                              pWriteCert, notBeforeBuf, notAfterBuf) == 0) {
                        if (mbedtls_x509write_crt_set_subject_name(
                          ...
                          if (mbedtls_x509write_crt_set_issuer_name(
                            ...
                            LogError(("mbedtls_x509write_crt_set_issuer_name "
                                      "failed"));
                          }
                        }
                      } else {
                      ...
                      mbedtls_x509write_crt_set_version(
                          pWriteCert, MBEDTLS_X509_CRT_VERSION_3);
                      ...
                      len = mbedtls_x509write_crt_der(
                          pWriteCert, (void *)pCertBuf,
                      ...
                      if (mbedtls_x509_crt_parse_der(
                              pCert,
                              (void *)(pCertBuf +
                                       GENERATED_CERTIFICATE_MAX_SIZE - len),
                              len) != 0) {
                ...
                LogError(
                    ("mbedtls_mpi_read_binary failed, ret: %d", mbedtlsRet));
  ...
  if (initialized && (0 != retStatus)) {
    ...
    if (0 != retStatus) {
      DTLS_FreeCertificateAndKey(pCert, pKey);
    }
  }
  free(pCertBuf);
  free(pEntropy);
...
DtlsTransportStatus_t DTLS_Init(DtlsNetworkContext_t *pNetworkContext,
  ...
  if ((pNetworkContext == NULL) || (pNetworkCredentials == NULL)) {
    LogError(("Invalid input parameter(s): Arguments cannot be NULL. "
              ...
              pNetworkContext, pNetworkCredentials));
  ...
  if (returnStatus == DTLS_SUCCESS) {
    ...
    returnStatus =
        initMbedtls(&(pDtlsTransportParams->dtlsSslContext.entropyContext),
                    &(pDtlsTransportParams->dtlsSslContext.ctrDrbgContext));
  }

  /* Initialize DTLS contexts and set credentials. */
  if (returnStatus == DTLS_SUCCESS) {
    returnStatus = dtlsSetup(pNetworkContext, pNetworkCredentials);
  }

  if (returnStatus == DTLS_SUCCESS) {
    memset(&pNetworkContext->pParams->mbedtlsTimer, 0,
    ...
    mbedtls_ssl_set_bio(&(pDtlsTransportParams->dtlsSslContext.context),
                        (void *)pDtlsTransportParams, DtlsUdpSendWrap,
                        DtlsUdpRecvWrap, NULL);
  }

  if (returnStatus != DTLS_SUCCESS) {
    LogWarn(("Fail to initialize DTLS Context: %p with return: %d",
             pNetworkContext, returnStatus));
  } else {
...
DtlsTransportStatus_t DTLS_ProcessPacket(DtlsNetworkContext_t *pNetworkContext,
  ...
  if ((pNetworkContext == NULL) || (pDtlsPacket == NULL) ||
      (readBuffer == NULL) || (pReadBufferSize == NULL)) {
    LogError(("Invalid input parameter(s): Arguments cannot be NULL. "
              "pNetworkContext=%p, pDtlsPacket=%p, readBuffer=%p, "
              "pReadBufferSize=%p.",
              pNetworkContext, pDtlsPacket, readBuffer, pReadBufferSize));
    returnStatus = DTLS_INVALID_PARAMETER;
  }
  ...
  if (returnStatus == DTLS_SUCCESS) {
    ...
    while (mbedtlsError == MBEDTLS_ERR_SSL_WANT_READ &&
           pDtlsTransportParams->pReceivedPacket != NULL) {
      /* Perform read function. Mbedtls would execute mbedtls_ssl_handshake
       * inside if the handshake is not done. */
      mbedtlsError = mbedtls_ssl_read(
          &(pDtlsTransportParams->dtlsSslContext.context),
          readBuffer + readOffset, *pReadBufferSize - readOffset);

      if ((mbedtlsError == MBEDTLS_ERR_SSL_TIMEOUT) ||
        ...
        LogWarn(("Failed to read data: mbedTLSError=-0x%x %s : %s.",
                 -mbedtlsError, mbedtlsHighLevelCodeOrDefault(mbedtlsError),
                 mbedtlsLowLevelCodeOrDefault(mbedtlsError)));
  ...
  if (returnStatus == DTLS_SUCCESS) {
    ...
    *pReadBufferSize = (size_t)readOffset;
    ...
    if (pNetworkContext->state == DTLS_STATE_HANDSHAKING) {
      ...
      if (pDtlsTransportParams->dtlsSslContext.context.state ==
...
DTLS_ExecuteHandshake(DtlsNetworkContext_t *pNetworkContext) {
  DtlsTransportStatus_t returnStatus = DTLS_SUCCESS;
  DtlsTransportParams_t *pDtlsTransportParams = NULL;
  int32_t mbedtlsError = 0;
  ...
  if (returnStatus == DTLS_SUCCESS) {
    ...
    if (mbedtlsError == MBEDTLS_ERR_SSL_WANT_READ) {
      ...
      LogError(
          ("Unexpected error during DTLS handshaking, error=-0x%x %s : %s.",
           -mbedtlsError, mbedtlsHighLevelCodeOrDefault(mbedtlsError),
           mbedtlsLowLevelCodeOrDefault(mbedtlsError)));
