...
#define MBEDTLS_ERROR_DESCRIPTION( err )                                 \
    ...
    {                                                                    \
        ...
        mbedtls_strerror( err, _error_string, sizeof( _error_string ) ); \
...
typedef int32_t ( *OnTransportDtlsSendHook_t )( void * pCustomContext,
    const uint8_t * pInputBuffer, size_t inputBufferLength );
...
typedef void ( *mbedtls_set_delay_fptr )( void *, uint32_t, uint32_t );
...
// DtlsKeyingMaterial is information extracted via
// https://tools.ietf.org/html/rfc5705 also includes the use_srtp value from
// Handshake
typedef struct
{
...
typedef enum DtlsTransportStatus
{
    ...
    DTLS_TRANSPORT_INSUFFICIENT_MEMORY, /**< Insufficient memory required to
    ...
    DTLS_HANDSHAKE_ALREADY_COMPLETE, /**< DTLS handshaking is done before
                                        calling. */
...
DtlsTransportStatus_t DTLS_Init( DtlsNetworkContext_t * pNetworkContext,
    DtlsNetworkCredentials_t * pNetworkCredentials, uint8_t isServer );
...
DtlsTransportStatus_t DTLS_ProcessPacket(
    DtlsNetworkContext_t * pNetworkContext, void * pDtlsPacket,
    size_t dtlsPacketLength, uint8_t * readBuffer, size_t * pReadBufferSize );
 ...
 *         - DTLS_HANDSHAKE_ALREADY_COMPLETE if handshake has been completed
 * before invoking.
 *         - Other specific error codes in case of failure
 */
DtlsTransportStatus_t DTLS_ExecuteHandshake(
...
int32_t DTLS_CreateCertificateAndKey( int32_t certificateBits,
    int generateRSACertificate, mbedtls_x509_crt * pCert,
 ...
 *         - Other specific error codes in case of failure
 */
int32_t DTLS_FreeCertificateAndKey(
    mbedtls_x509_crt * pCert, mbedtls_pk_context * pKey );
 ...
 *         - Other specific error codes in case of failure
 */
int32_t DTLS_CreateCertificateFingerprint(
    const mbedtls_x509_crt * pCert, char * pBuff, const size_t bufLen );
 ...
 *
 * @return DtlsTransportStatus_t Returns the status of the initialization:
 *         - DTLS_SUCCESS if the fingerprint of remote certificate matches the
 ...
 *         - Other specific error codes in case of failure
 */
int32_t DTLS_PopulateKeyingMaterial(
    DtlsSSLContext_t * pSslContext, pDtlsKeyingMaterial_t pDtlsKeyingMaterial );
