 ...
 */
static void
setOptionalConfigurations(SSLContext_t *pSslContext, const char *pHostName,
                          const NetworkCredentials_t *pNetworkCredentials);
...
tlsSetup(TlsNetworkContext_t *pTlsNetworkContext, const char *pHostName,
         const NetworkCredentials_t *pNetworkCredentials);
 ...
 */
static TlsTransportStatus_t
tlsHandshake(TlsNetworkContext_t *pTlsNetworkContext,
             const NetworkCredentials_t *pNetworkCredentials, uint32_t flags);
 ...
 * numbers.
 * @param[out] ctrDrbgContext mbed TLS CTR DRBG context for generation of random
 * numbers.
 *
...
#ifdef MBEDTLS_DEBUG_C
void mbedtls_string_printf(void *sslContext, int level, const char *file,
...
static int32_t setRootCa(SSLContext_t *pSslContext, const uint8_t *pRootCa,
  ...
  mbedtlsError =
      mbedtls_x509_crt_parse(&(pSslContext->rootCa), pRootCa, rootCaSize);

  if (mbedtlsError != 0) {
    LogError(
        ("Failed to parse server root CA certificate: mbedTLSError= %s : %s.",
         mbedtlsHighLevelCodeOrDefault(mbedtlsError),
         mbedtlsLowLevelCodeOrDefault(mbedtlsError)));
  } else {
    mbedtls_ssl_conf_ca_chain(&(pSslContext->config), &(pSslContext->rootCa),
                              NULL);
  }
...
static int32_t setRootCaPath(SSLContext_t *pSslContext, const char *pRootCaPath,
  ...
  /* Parse the server root CA certificate into the SSL context. */
  mbedtlsError =
      mbedtls_x509_crt_parse_file(&(pSslContext->rootCa), caFilePath);
...
static int32_t setClientCertificate(SSLContext_t *pSslContext,
  ...
  mbedtlsError = mbedtls_x509_crt_parse(&(pSslContext->clientCert), pClientCert,
                                        clientCertSize);
...
static int32_t setCredentials(SSLContext_t *pSslContext,
  ...
  if (pNetworkCredentials->pRootCa != NULL) {
    mbedtlsError = setRootCa(pSslContext, pNetworkCredentials->pRootCa,
  ...
  if ((pNetworkCredentials->pClientCert != NULL) &&
    ...
    if (mbedtlsError == 0) {
      mbedtlsError =
          setClientCertificate(pSslContext, pNetworkCredentials->pClientCert,
                               pNetworkCredentials->clientCertSize);
    }

    if (mbedtlsError == 0) {
      mbedtlsError =
          setPrivateKey(pSslContext, pNetworkCredentials->pPrivateKey,
...
setOptionalConfigurations(SSLContext_t *pSslContext, const char *pHostName,
  ...
  if (pNetworkCredentials->pAlpnProtos != NULL) {
    ...
    if (mbedtlsError != 0) {
      LogError(("Failed to configure ALPN protocol in mbed TLS: mbedTLSError= "
                "%s : %s.",
                mbedtlsHighLevelCodeOrDefault(mbedtlsError),
                mbedtlsLowLevelCodeOrDefault(mbedtlsError)));
    }
...
tlsSetup(TlsNetworkContext_t *pTlsNetworkContext, const char *pHostName,
  ...
  assert(pTlsNetworkContext != NULL);
  assert(pTlsNetworkContext->pParams != NULL);
  assert(pHostName != NULL);
  assert(pNetworkCredentials != NULL);
  ...
  if (returnStatus == TLS_TRANSPORT_SUCCESS) {
    mbedtlsError =
        setCredentials(&(pTlsTransportParams->sslContext), pNetworkCredentials);
...
TLS_FreeRTOS_ContinueHandshake(TlsNetworkContext_t *pTlsNetworkContext) {
  TlsTransportParams_t *pTlsTransportParams = NULL;
  TlsTransportStatus_t returnStatus = TLS_TRANSPORT_SUCCESS;
  int32_t mbedtlsError = 0;
  ...
  if (returnStatus == TLS_TRANSPORT_SUCCESS) {
    pTlsTransportParams = pTlsNetworkContext->pParams;
    mbedtlsError =
        mbedtls_ssl_handshake(&(pTlsTransportParams->sslContext.context));
...
TlsTransportStatus_t TLS_FreeRTOS_Connect(
    TlsNetworkContext_t *pTlsNetworkContext, const char *pHostName,
    uint16_t port, const NetworkCredentials_t *pNetworkCredentials,
    uint32_t receiveTimeoutMs, uint32_t sendTimeoutMs, uint32_t flags) {
  TlsTransportParams_t *pTlsTransportParams = NULL;
  TlsTransportStatus_t returnStatus = TLS_TRANSPORT_SUCCESS;
  ...
  if ((pTlsNetworkContext == NULL) || (pTlsNetworkContext->pParams == NULL) ||
    ...
    LogError(("Invalid input parameter(s): Arguments cannot be NULL. "
              "pTlsNetworkContext=%p, "
              "pHostName=%p, pNetworkCredentials=%p.",
              pTlsNetworkContext, pHostName, pNetworkCredentials));
    returnStatus = TLS_TRANSPORT_INVALID_PARAMETER;
  ...
  if (returnStatus == TLS_TRANSPORT_SUCCESS) {
    ...
    socketStatus =
        TCP_Sockets_Connect(&(pTlsTransportParams->tcpSocket), pHostName, port,
                            receiveTimeoutMs, sendTimeoutMs);
  ...
  if (returnStatus == TLS_TRANSPORT_SUCCESS) {
    ...
    returnStatus =
        initMbedtls(&(pTlsTransportParams->sslContext.entropyContext),
...
TlsTransportStatus_t
TLS_FreeRTOS_Disconnect(TlsNetworkContext_t *pTlsNetworkContext) {
  TlsTransportParams_t *pTlsTransportParams = NULL;
  int32_t tlsStatus = 0;
  TlsTransportStatus_t returnStatus = TLS_TRANSPORT_SUCCESS;
  ...
  if ((pTlsNetworkContext != NULL) && (pTlsNetworkContext->pParams != NULL)) {
    ...
    tlsStatus = (int32_t)mbedtls_ssl_close_notify(
    ...
    if ((tlsStatus != (int32_t)MBEDTLS_ERR_SSL_WANT_READ) &&
      ...
      if (tlsStatus == 0) {
        ...
        LogError(("(Network connection %p) Failed to send TLS close-notify: "
                  "mbedTLSError= %s : %s.",
                  pTlsNetworkContext, mbedtlsHighLevelCodeOrDefault(tlsStatus),
...
int32_t TLS_FreeRTOS_recv(TlsNetworkContext_t *pTlsNetworkContext,
  ...
  if ((pTlsNetworkContext == NULL) || (pTlsNetworkContext->pParams == NULL)) {
    ...
    tlsStatus = (int32_t)mbedtls_ssl_read(
        &(pTlsTransportParams->sslContext.context), pBuffer, bytesToRecv);
