 ...
 */
int32_t TCP_Sockets_Connect(Socket_t *pTcpSocket, const char *pHostName,
  ...
  for (pxCur = pxAddrList; pxCur != NULL; pxCur = pxCur->ai_next) {
    xFd = socket(pxCur->ai_family, pxCur->ai_socktype, pxCur->ai_protocol);
    if (xFd < 0) {
  ...
  if (xRet == TCP_SOCKETS_ERRNO_NONE) {
    if ((receiveTimeoutMs == 0) && (sendTimeoutMs == 0)) {
      ...
      setsockopt(xFd, SOL_SOCKET, SO_RCVTIMEO, &receiveTimeoutMs,
                 sizeof(receiveTimeoutMs));
      setsockopt(xFd, SOL_SOCKET, SO_SNDTIMEO, &sendTimeoutMs,
 ...
 */
int32_t TCP_Sockets_Send(Socket_t xSocket, const void *pvBuffer,
 ...
 * * If an error occurred, a negative value is returned. @ref SocketsErrors
 */
int32_t TCP_Sockets_Recv(Socket_t xSocket, void *pvBuffer,
                         size_t xBufferLength) {
  int xReadRet;
  int xReturnStatus;
